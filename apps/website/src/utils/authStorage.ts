export const SHARED_AUTH_KEYS = {
  token: ['fhwh_token', 'token'],
  refreshToken: ['fhwh_refresh_token', 'refreshToken'],
  user: ['fhwh_user', 'user'],
} as const;
...
export function getSharedAuthSnapshot<T = any>() {
  const token = readSharedToken();
  const refreshToken = readSharedRefreshToken();
  const user = readSharedUser<T>();
