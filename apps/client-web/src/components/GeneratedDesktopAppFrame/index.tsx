...
const GeneratedDesktopAppFrame: React.FC<GeneratedDesktopAppFrameProps> = ({ app }) => {
  ...
  React.useEffect(() => {
    ...
    if (isAbsoluteWebUrl(app.route) || app.route.startsWith('data:')) {
      ...
      setStaticRouteMessage('外部页面已直接挂载。');
      setStaticRouteHtml('');
      return undefined;
    }
    ...
    const probeRoute = async () => {
      try {
        ...
        setStaticRouteMessage(`静态页面返回 ${response.status} ${response.statusText || 'Not Found'}。`);
      } catch (error) {
        if (cancelled) return;
  ...
  if (app.runtime === 'native-exe' && app.native) {
    ...
    return (
      <div className={`${styles.frameShell} ${styles.nativeFrameShell}`}>
        <div className={`${styles.metaBar} ${styles.nativeMetaBar}`}>
          <div className={styles.metaList}>
            ...
            <span className={`${styles.metaPill} ${styles.nativeMetaPill}`}>
              <Gauge size={12} />
              {app.native.launchArgs?.length ?? 0} 个启动参数
            </span>
          </div>

          <div className={`${styles.metaList} ${styles.nativeMetaInfo}`}>
            <span>已经注册成桌面应用入口</span>
            <span>在桌面里点击图标会直接启动本地程序</span>
          </div>
        </div>
        ...
        <div className={styles.nativeShell}>
          <div className={styles.nativeCard}>
            <div className={styles.nativeHeader}>
              ...
              <span className={styles.nativeBadge}>桌面启动器</span>
            </div>
            ...
            <div className={styles.nativeList}>
              ...
              <div className={styles.nativeRow}>
                <span>启动参数</span>
                <code>{app.native.launchArgs?.join(' ') || '无'}</code>
              </div>
            </div>
      ...
      <div className={styles.frameShell}>
        <div className={styles.metaBar}>
          <div className={styles.metaList}>
            ...
            <span className={styles.metaPill}>
              <Gauge size={12} />
              {app.bundle?.entryFile || '未提供 HTML 入口'}
            </span>
          </div>
          <div className={styles.metaList}>
            <span>已阻止自动打开空白页</span>
        ...
        <div className={styles.frameBody} ref={frameBodyRef} style={frameScaleStyle}>
          <div className={styles.empty}>
            <div className={styles.emptyCard}>
              <span className={styles.emptyBadge}>前端包缺失</span>
              <strong>{app.title}</strong>
              <p>这个应用暂时没有可运行的前端包。已停止自动打开，避免直接进入空白窗口或黑窗口。</p>
              <div className={styles.emptyMeta}>
                <span>入口：{app.bundle?.entryFile || '未配置'}</span>
    ...
    <div className={styles.frameShell}>
      <div className={styles.metaBar}>
        ...
        <div className={styles.metaList}>
          ...
          <span>{app.bundle.notes.length} 条转换说明</span>
        </div>
      </div>
