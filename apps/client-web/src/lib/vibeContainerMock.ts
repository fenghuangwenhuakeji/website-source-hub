...
export function onUserAction(callback: UserActionCallback): () => void {
  console.info('[MockVibe] onUserAction: registered new callback, total:', userActionCallbacks.size + 1);
  userActionCallbacks.add(callback);
  console.info('[MockVibe] onUserAction: after add, callbacks:', userActionCallbacks.size);
  return () => {
    console.info('[MockVibe] onUserAction: removing callback, before:', userActionCallbacks.size);
    userActionCallbacks.delete(callback);
    console.info('[MockVibe] onUserAction: after remove, callbacks:', userActionCallbacks.size);
  };
}
...
const mockManager = {
  ...
  sendAgentMessage: (event: unknown) => {
    ...
    console.info('[MockVibe] isReportUserActionsEnabled:', isReportUserActionsEnabled());
    ...
    if (!isReportUserActionsEnabled()) {
      ...
      if (evt.action_result === undefined) {
        console.info('[MockVibe] sendAgentMessage: blocked by reportUserActions=false, discarding user action');
        return;
      }
    }
    console.info('[MockVibe] sendAgentMessage: dispatching to', userActionCallbacks.size, 'callbacks');
    userActionCallbacks.forEach((cb, index) => {
      console.info('[MockVibe] sendAgentMessage: calling callback', index + 1);
      cb(event);
      console.info('[MockVibe] sendAgentMessage: callback', index + 1, 'finished');
    });
  },
  ...
  getSystemSettings: (): Promise<SystemSettingsResponse> =>
    Promise.resolve({
      language: { current: i18n.language || navigator.language, options: ['en', 'zh'] },
    }),
};
