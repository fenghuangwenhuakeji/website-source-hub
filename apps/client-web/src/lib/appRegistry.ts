...
import type { ExperienceProfileId, MobilePriority } from '@/types/desktopApp';
...
const APP_STATIC_REGISTRY: AppStaticDef[] = [
  { appId: 1, appName: 'os', route: '/home', displayName: '系统' },
  {
    appId: 4,
    appName: 'diary',
    route: '/diary',
  ...
  },
  {
    ...
    experienceProfileId: 'writing',
    activationKeywords: ['writing', 'novel', 'outline', 'chapter'],
  ...
  },
  {
    ...
    defaultSize: { width: 1200, height: 800 },
    experienceProfileId: 'writing',
    activationKeywords: ['writing', 'short-story', 'draft', 'chapter'],
    mobilePriority: 'primary',
  },
  {
    appId: 38,
    ...
    route: '/novelcomics',
    displayName: '小说漫剧',
    sourceDir: 'NovelComics',
    icon: 'Clapperboard',
    color: '#7660ff',
    defaultSize: { width: 1200, height: 800 },
    experienceProfileId: 'writing',
    activationKeywords: ['novel', 'comic', 'scene', 'storyboard'],
    mobilePriority: 'primary',
  },
  {
    appId: 28,
  ...
  },
  {
    ...
    route: '/codeEditor',
    displayName: '代码编辑器',
    sourceDir: 'CodeEditor',
    icon: 'Code2',
    color: '#007acc',
    defaultSize: { width: 1200, height: 800 },
    experienceProfileId: 'code-edit',
    activationKeywords: ['code', 'editor', 'debug', 'patch'],
  ...
  },
  {
    ...
    activationKeywords: ['chat', 'conversation', 'message', 'roleplay'],
    mobilePriority: 'primary',
  },
  {
    appId: 40,
  ...
  },
  {
    ...
    color: '#9B59B6',
    defaultSize: { width: 1200, height: 800 },
    experienceProfileId: 'fenghuang',
    activationKeywords: ['fenghuang', 'phoenix', 'suite', 'creative-hub'],
    mobilePriority: 'primary',
  },
  {
    appId: 42,
...
const OS_ACTIONS: AppActionDef[] = [
  {
    name: 'OPEN_APP',
    description: '打开指定的应用程序。传入 app_id 作为应用程序ID',
    params: [
      {
        name: 'app_id',
        type: 'string',
        description: `应用程序ID (${APP_STATIC_REGISTRY.filter((a) => a.appName !== 'os')
  ...
  {
    name: 'CLOSE_APP',
    description: '关闭指定的应用程序。传入 app_id 作为应用程序ID',
    params: [{ name: 'app_id', type: 'string', description: '应用程序ID', required: true }],
  },
  {
    name: 'SET_WALLPAPER',
    description:
      '更改桌面壁纸。wallpaper_url 必须是 https URL 或 data URL (data:image/...)。' +
      '你可以使用 generate_image 返回的 dataUrl，或任何 https 图片/视频 URL。',
    params: [
      {
        name: 'wallpaper_url',
        type: 'string',
        description: '壁纸的 https URL 或 data URL',
        required: true,
      },
    ],
  },
...
export function getDesktopApps(): Array<{
  ...
  return APP_STATIC_REGISTRY.filter((a) => a.appName !== 'os' && a.icon && a.color).map((a) => ({
    ...
    icon: a.icon!,
    color: a.color!,
    experienceProfileId: a.experienceProfileId,
    experienceHighlights: getExperienceHighlights(a.experienceProfileId),
    activationKeywords: a.activationKeywords,
    mobilePriority: a.mobilePriority,
  }));
}

export function getAppExperienceProfileId(appId: number): ExperienceProfileId | null {
  return APP_STATIC_REGISTRY.find((a) => a.appId === appId)?.experienceProfileId ?? null;
}
...
export function executeListApps(): string {
  ...
  return (
    `可用应用:\n${apps.join('\n')}\n\n` +
    '系统级操作 (使用 app_name="os"):\n' +
    '- OPEN_APP: 打开应用 (参数: app_id)\n' +
    '- CLOSE_APP: 关闭应用 (参数: app_id)\n' +
    '- SET_WALLPAPER: 更改壁纸 (参数: wallpaper_url)'
  );
}
