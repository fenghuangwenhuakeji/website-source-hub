...
// 3. 【博士最终实现版】formatTextForExport 函数现在也支持添加【导语】。
...
// 博士修改核心：此函数现在也接收导语和蓝图，并构建完整的导出文本
function formatTextForExport(prologue, chapters, blueprint = '', novelTitle = '未命名作品') {
    let fullContent = '';

    // 1. 添加故事蓝图
    const blueprintText = String(blueprint || '');
    if (blueprintText.trim().length > 0) {
        fullContent += `【故事蓝图】\n========================================\n${sanitizeForTemplate(blueprintText).trim()}\n========================================\n\n\n`;
    }

    // 2. 添加小说标题
    fullContent += `《${novelTitle}》\n\n`;

    // 3. 添加故事导语
    const prologueText = String(prologue || '');
    if (prologueText.trim().length > 0) {
        fullContent += `【正文内容】\n\n${sanitizeForTemplate(prologueText).trim()}\n\n\n`;
    }
    ...
    if (chapters && chapters.length > 0) {
        for (let i = 0; i < chapters.length; i++) {
            ...
            chaptersText += title + content + '\n\n\n';
        }
    }
    
    fullContent += chaptersText;
