...
const RAGSystem = {
    ...
    async _compressContext(results, maxTokens = 3000) {
        ...
        // 需要压缩：保留高优先级，对低优先级调用AI摘要
        const sorted = [...results].sort((a, b) => b.score - a.score);
        const keep = sorted.slice(0, Math.min(5, Math.ceil(sorted.length * 0.4)));
        const compress = sorted.slice(keep.length);
        let ctx = keep.map(r => `[${(this._SOURCES[r.source] || {}).label || r.source}] ${r.title}\n${r.content}`).join('\n---\n');
