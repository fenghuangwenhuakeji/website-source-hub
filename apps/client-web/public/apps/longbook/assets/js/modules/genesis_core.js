...
const GenesisCore = {
    ...
    MODE_CONFIG: {
        ...
        import: {
            label: '导入新书/续写', shortLabel: '导入', icon: 'fa-file-import', color: 'cyan', themeColor: '#06b6d4',
            bgGradient: 'from-cyan-900/30 to-blue-900/20',
	            desc: '快速导入，凤凰解析',
            stages: [
                { id: 'seed', label: '一句话开书', icon: 'fa-seedling' },
                { id: 'outline', label: '细纲入工作台', icon: 'fa-list-ol' },
                { id: 'write', label: '进入执笔', icon: 'fa-pen-nib' }
            ],
            entryModule: 'world_engine',
            dataSchema: { bookBrief: '', originalText: '', parsedStructure: {}, extractedEntities: [], styleFingerprint: '', continuationPoint: { chapterIndex: 0, position: 'end' }, importSummary: '', originalStats: { wordCount: 0, chapterCount: 0, genre: '' } },
    ...
    async getActiveProject() {
        ...
        const proj = await DB.get('projects', this._activeProjectId);
        if (!proj) {
    ...
    async requireActiveProject(message = '请先创建或选择一个项目') {
        const proj = await this.getActiveProject();
        if (!proj && typeof UI !== 'undefined') UI.toast(message, 'warning');
        return proj;
    },
    _itemBelongsToProject(item, projectId = this._activeProjectId) {
        if (!item || !projectId) return false;
        return !item.projectId || item.projectId === projectId;
    },
    filterProjectItems(items, projectId = this._activeProjectId) {
        if (!projectId) return [];
        return (items || []).filter(item => this._itemBelongsToProject(item, projectId));
    },
    stampProjectRecord(record, projectId = this._activeProjectId) {
        if (record && projectId && typeof record === 'object' && !Array.isArray(record)) record.projectId = record.projectId || projectId;
        return record;
    },
    setActiveProject(id) {
    ...
    clearActiveProject() {
        ...
        if (typeof Modules !== 'undefined' && Modules.writer && Modules.writer._resetProjectRuntime) {
            Modules.writer._resetProjectRuntime();
            if (typeof App !== 'undefined' && App._currentModule === 'writer' && Modules.writer._renderProjectGate) Modules.writer._renderProjectGate();
        }
        this._updateUIIndicators();
    },
    ...
    async deleteProject(id) {
        const wasActive = id && (this._activeProjectId === id || localStorage.getItem('genesis_active_project') === id);
        ...
        if (wasActive) {
            this.clearActiveProject();
            await this._clearActiveWorkspaceAfterProjectDelete();
        }
    },
    async getProjects() { const all = await DB.getAll('projects') || []; return all.sort((a, b) => (b.updatedAt || 0) - (a.updatedAt || 0)); },

    async _clearActiveWorkspaceAfterProjectDelete() {
        try {
            if (typeof Modules !== 'undefined' && Modules.writer && Modules.writer._resetProjectRuntime) {
                Modules.writer._resetProjectRuntime();
            }
            if (typeof LocalSync !== 'undefined') {
                if (LocalSync._writeQueue) {
                    Object.values(LocalSync._writeQueue).forEach(t => clearTimeout(t));
                    LocalSync._writeQueue = {};
                }
                if (LocalSync._clearAllStores) await LocalSync._clearAllStores();
                if (LocalSync._resetModuleStates) LocalSync._resetModuleStates();
            }
        } catch(e) {
            console.warn('[GenesisCore] 清理项目工作区失败:', e);
        }
    },
    ...
    async buildWriterContext(projectId, opts = {}) {
        ...
        if (proj.mode === 'import') {
            ...
            if (md.bookBrief) ctx += '\n\n【一句话开书】\n' + String(md.bookBrief).slice(0, 1000) + '\n';
    ...
    async refreshStats(projectId) {
        const proj = await DB.get('projects', projectId); if (!proj) return;
        const chaps = this.filterProjectItems(await DB.getAll('chapters') || [], projectId);
    ...
    async _renderActiveBar(el) {
        ...
        const proj = await this.getActiveProject();
        if (!proj || !el) { if (el) { el.innerHTML = ''; el.style.display = 'none'; } return; }
        const config = this.getModeConfig(proj.mode);
    ...
    async _renderMiniIndicator(el) {
        if (!this._activeProjectId) { el.innerHTML = ''; el.style.display = 'none'; return; }
        const proj = await this.getActiveProject();
        if (!proj) { el.innerHTML = ''; el.style.display = 'none'; return; }
        const config = this.getModeConfig(proj.mode);
