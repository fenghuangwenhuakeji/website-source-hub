const AI = {
    // Built-in placeholder API config (unconfigured)
    BUILTIN_CONFIG: {
        id: '_builtin_default',
        config_name: 'Built-in Placeholder (Unconfigured)',
        provider: 'custom',
        base_url: 'https://example.invalid/v1',
        api_key: '',
        model_name: 'configure-your-model',
        temperature: 0.85,
        max_tokens: 4096,
        is_active: 0,
        is_master: 0,
    ...
    isConfigUsable(config) {
        ...
        if (!provider || !apiKey || !modelName) return false;
    ...
    async getActiveConfig(type = 'text') {
        try {
            ...
            if (type === 'text') {
                const found = configs.find(c => c.is_master === 1) || configs.find(c => c.is_active === 1) || configs[0] || null;
                if (found) return found;
                // 兜底：返回内嵌默认配置
                return AI.BUILTIN_CONFIG;
            }
            ...
            if (type === 'parse' || type === 'fusion') {
                ...
                if (fallback) return { ...fallback, _fallbackType: 'text', _requestedType: type };
                // 兜底：返回内嵌默认配置
                return { ...AI.BUILTIN_CONFIG, _fallbackType: 'text', _requestedType: type };
            }
            return null;
        } catch(e) {
            console.error('[AI] Failed to get active config:', e);
            // 极端异常时也返回内嵌配置
            return type === 'text' ? AI.BUILTIN_CONFIG : null;
        }
    },
    ...
    async generate(prompt, config = {}, onChunk) {
        ...
        if (typeof Membership !== 'undefined') {
            const quotaCheck = await Membership.canConsume(1);
            if (!quotaCheck.allowed) {
                const errMsg = '⚠️ ' + quotaCheck.reason;
        ...
        if (!AI.isConfigUsable(apiConfig)) {
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            ...
            try {
                ...
                // 移动端 QQ 浏览器等 WebView 对 ReadableStream 支持不稳定，使用非流式请求
                const isMobile = /Mobile|Android|iPhone|iPad|iPod/i.test(navigator.userAgent) || window.innerWidth <= 768;
                const { url, headers, body } = AI.buildRequest(apiConfig, requestPrompt, !isMobile);
                ...
                if (isMobile) {
                    ...
                    if (trimmed.startsWith('data:')) {
                        // 某些移动端 WebView 可能仍返回 SSE 格式，自适应解析
                        const lines = trimmed.split('\n');
                        for (const line of lines) {
                            const t = line.trim();
                            if (!t || t === 'data: [DONE]' || t === '[DONE]') continue;
                            ...
                            try {
                                ...
                                txt += AI.parseStreamChunk(apiConfig.provider, json);
                            } catch (e) { /* skip */ }
                        }
                    } else {
                        try {
                            const data = JSON.parse(responseText);
                            txt = AI.parseNonStreamChunk(apiConfig.provider, data);
                        } catch (e) {
                            console.warn('[AI] 移动端返回格式异常:', responseText.slice(0, 300));
                        }
                    }
                    if (txt) { onChunk(txt); receivedAny = true; fullContent += txt; }
                    AI._abortControllers.delete(abortCtrl);
                    if (typeof Membership !== 'undefined') {
                        const inputTokens = AI.estimateTokens(requestPrompt);
                        const outputTokens = AI.estimateTokens(fullContent);
                        const totalTokens = Math.max(1, inputTokens + outputTokens);
                        await Membership.consume(totalTokens);
                        if (typeof App !== 'undefined' && App.updateMemberStatus) App.updateMemberStatus();
                    }
                    return;
                }
                ...
                if (isSSE || true) { // 大多数provider返回application/json但内容是SSE
                    ...
                    while(true) {
                        ...
                        for(const line of lines) {
                            ...
                            // 宽松匹配 data: 前缀（兼容 data: {...} / data:{...}）
                            const dataMatch = trimmed.match(/^data:\s*(.*)$/);
                            if (!dataMatch) continue;
                            const dataStr = dataMatch[1].trim();
                            if (!dataStr || dataStr === '[DONE]') continue;
                            rawLines.push(dataStr);
                            try {
                    ...
                    if (bufTrimmed) {
                        const bufMatch = bufTrimmed.match(/^data:\s*(.*)$/);
                        if (bufMatch) {
                            const dataStr = bufMatch[1].trim();
                ...
                if (!receivedAny) {
                    ...
                    try {
                        ...
                        if (res2.ok) {
                            ...
                            if (txt) { onChunk(txt); receivedAny = true; fullContent += txt; console.log('[AI] 非流式回退成功'); }
                        } else {
    ...
    buildRequest(config, prompt, stream) {
        ...
        // 修复：temperature 可能为 ''（空字符串），需要强转数字并兜底
        const tempRaw = config.temperature;
        const temperature = (tempRaw === '' || tempRaw === null || tempRaw === undefined || Number.isNaN(Number(tempRaw))) ? 0.85 : Number(tempRaw);
