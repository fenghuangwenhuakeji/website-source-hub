Object.assign(Modules.world_engine, {
                    ...
                    <div class="group cursor-pointer bg-[#0e0e12] rounded-xl border border-white/5 hover:border-green-500/30 p-5 transition-all col-span-2" onclick="Modules.world_engine._openNovelImportModal()">
                        <div class="flex items-center gap-3 mb-3">
                            <div class="w-10 h-10 rounded-lg bg-green-500/10 flex center"><i class="fa-solid fa-file-import text-green-400 text-lg"></i></div>
	                            <div><div class="font-bold text-white">导入新书/续写</div><div class="text-[9px] text-dim">快速导入 · 凤凰解析</div></div>
                        </div>
	                        <div class="text-[10px] text-gray-400 leading-relaxed">上传或粘贴已有小说，系统只把细纲和正文放进对应位置；人物、规则、伏笔、关系交给凤凰流后续精读。</div>
                    </div>
                </div>
            </div>
    ...
    async _refreshNarrativeConsistency() {
        ...
        const nonWorldEnts = pipelineEnts.filter(e => we._isGraphNodeEntity ? we._isGraphNodeEntity(e) : !we._isWorldEntity(e));
    ...
    async _syncConsistencyToFusion() {
        ...
        const nonWorld = allEnts.filter(e => this._isGraphNodeEntity ? this._isGraphNodeEntity(e) : !this._isWorldEntity(e));
        ...
        const worlds = allEnts.filter(e => this._isWorldEntity(e) && e.desc);
        if(worlds.length) {
            kg += '\n【世界观设定】\n';
    ...
    async _refreshDashboard() {
        ...
        let worldFilled = 0;
        const allEnts = we._cachedEntities || [];
        const projectId = await we._activeProjectIdForWorld?.();
        const isWorldviewEntity = (entity, cat = '') => we._isWorldEntity ? we._isWorldEntity(entity, cat) : false;
        for(const cat of worldCats) {
            const ent = allEnts.find(e => isWorldviewEntity(e, cat));
            if(ent && String(ent.desc || '').trim()) worldFilled++;
        }
        ...
        const nonWorldEnts = allEnts.filter(e => !isWorldviewEntity(e));
        ...
        we._updateEntityCountBadges?.(nonWorldEnts);
        const typeCounts = {};
        ...
        if(actEl) {
            ...
            if(recentEnts.length) {
                ...
                html = '<div class="text-[10px] text-dim">暂无活动 — 从「导入新书/续写」或「融合拆书」开始构建你的世界</div>';
            }
