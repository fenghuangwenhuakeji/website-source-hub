Object.assign(Modules.fusion_book, {
        ...
        for (let i = 0; i < steps.length; i++) {
            if (this._pipelinePaused) {
                await DB.put('settings', {
                    ...
                    projectId: this._pipelineProjectId || this._plConfig?.projectId || null
                });
    ...
    _getRangeSelectionState(side) {
        const sideKey = side === 'right' ? 'right' : 'left';
        const sideLabel = sideKey === 'right' ? '右书' : '左书';
        const book = (this._books || []).find(b => b.id === this[sideKey]?.bookId);
        ...
        return {
            ...
            endInput,
            boxes,
            maxChapterNo: chapterNos.length ? Math.max(...chapterNos) : 0
        };
    },
    ...
    _plConfigSelectRange(side) {
        const {
            ...
            boxes,
            maxChapterNo
        } = this._getRangeSelectionState(side);

        if (!book) {
            UI.toast(`请先选择${sideLabel}`);
            return;
        }
        if (!boxes.length || maxChapterNo <= 0) {
            UI.toast(`${sideLabel}没有可选章节`);
            return;
        }
        ...
        let start = Math.min(Math.max(parsedStart, 1), maxChapterNo);
        let end = Math.min(Math.max(parsedEnd, 1), maxChapterNo);
        if (start > end) {
            [start, end] = [end, start];
        }

        boxes.forEach(cb => {
            const chapterIdx = Number(cb.dataset.idx);
            const chapterNo = Number.isInteger(chapterIdx) ? chapterIdx + 1 : NaN;
            cb.checked = Number.isInteger(chapterNo) && chapterNo >= start && chapterNo <= end;
        });

        if (startInput) startInput.value = String(start);
        if (endInput) endInput.value = String(end);
        this._updateConfigSummary();
        UI.toast(`${sideLabel}已选择第${start}-${end}章`);
    },
    ...
    async startConfiguredPipeline() {
        ...
        this._plConfig.projectId = (typeof GenesisCore !== 'undefined' && GenesisCore._activeProjectId) ? GenesisCore._activeProjectId : null;
        this._pipelineProjectId = this._plConfig.projectId;
    ...
    async _runConfiguredPipeline(isResume = false) {
        ...
        if (isResume) {
            ...
            if (saved) {
                ...
                this._pipelineProjectId = saved.projectId || saved.config?.projectId || this._pipelineProjectId || null;
                if (this._plConfig) this._plConfig.projectId = this._pipelineProjectId;
            }
        }
