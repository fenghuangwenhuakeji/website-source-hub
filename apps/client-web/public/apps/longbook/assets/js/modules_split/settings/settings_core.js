...
Modules.settings = {
    ...
    _navItems: [
        ...
        { id: 'membership', icon: 'fa-crown', text: '会员与额度', desc: '激活与充值' },
    ...
    _renderContent: () => {
        ...
        if (t === 'membership') return S._renderMembershipTab();
    ...
    async _refreshSetupStatus() {
        ...
        // 内嵌默认配置也算已配置
        const hasBuiltin = !!AI.BUILTIN_CONFIG;
        set('text-api', masterText || hasBuiltin, masterText ? '主控已配置' : (hasBuiltin ? '默认主控就绪' : '还没配置'));
        set('writing', !!localStorage.getItem('global_rules'), localStorage.getItem('global_rules') ? '已设置' : '可用默认值');
        set('backup', true, '可随时导出');
    },

    _renderMembershipTab() {
        const status = typeof Membership !== 'undefined' ? Membership.getStatus() : null;
        const isFree = !status || status.isFree;
        const record = typeof Membership !== 'undefined' ? Membership._load() : null;
        const today = new Date().toISOString().split('T')[0];
        const daily = typeof Membership !== 'undefined' ? Membership._loadDaily() : null;
        const usedToday = (daily && daily.date === today) ? (daily.used || 0) + (daily.freeUsed || 0) : 0;
        const totalQuota = status ? (status.dailyQuota || status.freeTokens || 500000) : 500000;
        const pct = Math.min(100, (usedToday / totalQuota) * 100);
        const barColor = status && status.remainingToday < totalQuota * 0.1 ? 'bg-red-500' : 'bg-accent';

        const quotaBar = status ? `
            <div class="bg-[#111113] border border-white/5 rounded-lg p-4 space-y-4">
                <div class="flex items-center justify-between">
                    <div>
                        <div class="text-sm font-bold text-white">今日额度</div>
                        <div class="text-[11px] text-dim mt-1">${isFree ? '免费体验额度' : (status.typeName + ' &middot; ' + status.tierName)}</div>
                    </div>
                    <div class="text-right">
                        <div class="text-lg font-black ${status.remainingToday < 50000 ? 'text-red-400' : 'text-accent'}">${(status.remainingToday || 0).toLocaleString()}</div>
                        <div class="text-[10px] text-dim">剩余 Token</div>
                    </div>
                </div>
                ...
                <div class="flex justify-between text-[10px] text-dim">
                    <span>已用 ${usedToday.toLocaleString()}</span>
                    <span>总计 ${totalQuota.toLocaleString()}</span>
                </div>
                ${!isFree && status.daysLeft > 0 ? '<div class="text-[11px] text-amber-300"><i class="fa-solid fa-hourglass-half mr-1"></i>有效期剩余 ' + status.daysLeft + ' 天</div>' : ''}
                ${!isFree && status.expired ? '<div class="text-[11px] text-red-400"><i class="fa-solid fa-circle-exclamation mr-1"></i>已过期，请续费</div>' : ''}
            </div>
        ` : '<div class="bg-[#111113] border border-white/5 rounded-lg p-4 text-dim text-xs">额度系统加载中...</div>';

        const activateSection = `
            <div class="bg-[#111113] border border-white/5 rounded-lg p-4 space-y-4">
                <div class="text-sm font-bold text-white">${isFree ? '开通创作中心' : '续费与兑换'}</div>
                <div class="text-[11px] text-dim">推荐前往官网充值，支付成功后权益会直接进入当前官网账号；卡密仅用于兑换码和人工发放。</div>
                <div class="flex gap-2">
                    <button class="btn btn-sm bg-accent text-black hover:brightness-110 font-bold flex-1" onclick="App.openOfficialRecharge()">官网充值</button>
                    <button class="btn btn-sm bg-white/5 text-dim hover:text-white flex-1" onclick="App.syncOfficialAuth()">同步权益</button>
                </div>
                <input type="text" id="s-license-key" class="input bg-black/30 border-white/10 h-10 text-sm text-white font-mono text-center tracking-widest" placeholder="官网兑换码 / 旧版卡密" maxlength="39">
                <div class="flex gap-2">
                    <button class="btn btn-sm bg-white/5 text-dim hover:text-white flex-1" onclick="Modules.settings._handleSettingsActivate()">兑换卡密</button>
                    ${isFree ? '<button class="btn btn-sm bg-white/5 text-dim hover:text-white flex-1" onclick="App.showLicenseOverlay()">查看状态</button>' : ''}
                </div>
                <div id="s-license-msg" class="text-[11px] text-center hidden"></div>
            </div>
        `;

        let historySection = '';
        if (record && record.keys && record.keys.length > 0) {
            historySection = '<div class="bg-[#111113] border border-white/5 rounded-lg p-4 space-y-3"><div class="text-sm font-bold text-white">激活记录</div><div class="space-y-2">' +
                record.keys.map((k, i) => '<div class="flex items-center justify-between p-2 rounded bg-black/20 border border-white/5"><div class="text-[11px] font-mono text-dim">' + k + '</div><span class="text-[10px] px-1.5 py-0.5 rounded bg-accent/10 text-accent">' + (i === 0 ? '首次' : '叠加') + '</span></div>').join('') +
                '</div></div>';
        }

        return this._renderPageShell('会员与额度', '查看当前额度、官网充值、同步权益或兑换卡密。', quotaBar + activateSection + historySection);
    },

    _handleSettingsActivate() {
        const input = document.getElementById('s-license-key');
        ...
        if (!compactKey || compactKey.length < 12 || compactKey.length > 32) {
            if (msg) { msg.textContent = '请输入有效的官网兑换码或旧版卡密'; msg.className = 'text-[11px] text-center text-red-400'; msg.classList.remove('hidden'); }
            return;
        }
        if (msg) { msg.classList.add('hidden'); }
        Membership.activate(key).then(result => {
            if (msg) {
                msg.classList.remove('hidden');
                if (result.success) {
                    msg.textContent = result.isStacked ? '叠加成功！' : '激活成功！';
                    msg.className = 'text-[11px] text-center text-green-400';
                    input.value = '';
                    if (App.updateMemberStatus) App.updateMemberStatus();
                    setTimeout(() => Modules.settings.refresh(), 300);
                } else {
            ...
            if (msg) { msg.classList.remove('hidden'); msg.textContent = '验证失败: ' + e.message; msg.className = 'text-[11px] text-center text-red-400'; }
        });
    },
