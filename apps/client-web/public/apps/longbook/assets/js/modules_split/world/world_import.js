    ...
    async _confirmImport() {
        ...
        for(const wv of we._importParsed.worldViews) {
	        ...
	        for(const ent of we._importParsed.entities) {
	            ent.type = we._normalizeEntityType ? we._normalizeEntityType(ent.type) : (ent.type || '其他');
	            ent.desc = we._compactEntityDesc ? we._compactEntityDesc(ent.desc || ent.description || '', ent) : (ent.desc || ent.description || '');
	            const allNames = new Set([...(we._importParsed.entities || []).map(e => e.name).filter(Boolean), ...(we._cachedEntities || []).map(e => e.name).filter(Boolean)]);
	            ent.relations = we._compactEntityRelations ? we._compactEntityRelations(ent.relations || [], ent.name, allNames, 8) : (ent.relations || []);
	            if(we._isJunkEntity && we._isJunkEntity(ent)) continue;
	            if(shouldMerge) {
	                const existing = we._cachedEntities.find(e => e.name === ent.name && !e.id.startsWith('world_'));
	                if(existing) {
	                    existing.type = we._normalizeEntityType ? we._normalizeEntityType(existing.type || ent.type) : (existing.type || ent.type);
	                    existing.desc = we._mergeEntityDesc ? we._mergeEntityDesc(existing.desc || '', ent.desc || '', existing) : (ent.desc || existing.desc || '');
	                    const oldRelations = Array.isArray(existing.relations)
	                        ? existing.relations
	                        : (typeof existing.relations === 'string' ? existing.relations.split(',').map(s => s.trim()).filter(Boolean) : []);
	                    existing.relations = we._compactEntityRelations ? we._compactEntityRelations([...oldRelations, ...(ent.relations || [])], existing.name, allNames, 12) : Array.from(new Set([...oldRelations, ...(ent.relations || [])]));
	                    existing.updatedAt = Date.now();
                    if(assignChapter) {
