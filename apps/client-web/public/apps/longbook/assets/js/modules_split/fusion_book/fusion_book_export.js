Object.assign(Modules.fusion_book, {
    ...
    sendToPhoenix() {
        ...
        const direction = this._getDirectionLockText ? this._getDirectionLockText() : '';
        const ctx = '[融合拆书弹药] 以下内容为两书技法融合产出的通用技法模板。角色、世界观、主线方向以当前项目和世界引擎为准，严禁复用原书内容。';
        const guard = direction ? '\n\n[当前方向护栏]\n' + direction : '';
        const fullContent = ctx + guard + '\n\n' + content;
        // 存入凤凰创作的素材
        if (typeof Modules.phoenix !== 'undefined' && Modules.phoenix._fusionRef !== undefined) {
            Modules.phoenix._fusionRef = fullContent;
        }
        MemorySystem.addWorking('[融合技法→凤凰流] ' + ctx + '\n' + content.slice(0, 300), 'fusion_ref', 5, { nexusState: { chr: '原创', wld: '技法弹药' } });
        App.nav('phoenix');
    ...
    async sendToWriter() {
        ...
        const guard = direction ? '\n\n[当前方向护栏]\n' + direction : '';
        const fullContent = ctx + guard + '\n\n' + content;
        MemorySystem.addWorking('[融合技法→拆书弹药库] ' + ctx + '\n' + content.slice(0, 300), 'fusion_ref', 5, { nexusState: { chr: '原创', wld: '技法弹药' } });
        await DB.put('assets', {
            id: 'fusion_ammo_' + Date.now(),
            name: '融合拆书弹药（待用）',
            type: 'fusion_ammo',
            content: fullContent,
            source: 'fusion_workbench',
    ...
    async _cycleFusionSummary(cyclePairs, cycleSize, leftBook, rightBook) {
        ...
        const cycleNum = Math.ceil(startIdx / Math.max(1, cycleSize));
        ...
        this._plLog(`🔄 执行第${cycleNum}个循环融合 (第${startIdx}-${endIdx}章)`, 'info');

        const titleEl = document.getElementById('pl-current-title');
        if (titleEl) titleEl.innerHTML = `<i class="fa-solid fa-sync mr-1 text-cyan-400"></i>循环融合: 第${startIdx}-${endIdx}章`; 
        ...
        let cycleEntities = [];
        let cycleWritings = '';
        let hasLeftBatch = false;
        let hasRightBatch = false;

        try {
            const leftBatch = await DB.get('settings', `cycle_batch_${leftBook.id}_${startIdx}_${endIdx}`);
            const rightBatch = await DB.get('settings', `cycle_batch_${rightBook.id}_${startIdx}_${endIdx}`);
            if (leftBatch?.content) {
                leftCycleAnalyses += `## ${leftBook.name} 第${startIdx}-${endIdx}章批量拆解\n${leftBatch.content}\n\n`;
                hasLeftBatch = true;
            }
            if (rightBatch?.content) {
                rightCycleAnalyses += `## ${rightBook.name} 第${startIdx}-${endIdx}章批量拆解\n${rightBatch.content}\n\n`;
                hasRightBatch = true;
            }
        } catch(e) {}
        ...
        for (let i = 0; i < cyclePairs.length; i++) {
            ...
            try {
                ...
                if (!hasLeftBatch && leftStored?.content) {
                    leftCycleAnalyses += `## 第${leftIdx + 1}章: ${lCh.title}\n${leftStored.content}\n\n`;
                }
                if (!hasRightBatch && rightStored?.content) {
        ...
        const cycleKeyPrefix = isCreative ? 'creative_cycle_fusion' : 'cycle_fusion';
        const prevCycleKey = `${cycleKeyPrefix}_prev_${startIdx}`;
        ...
        let cycleFusionPrompt = isCreative
            ? `你是顶级网文技法融合大师，也是原创长篇细纲导演。\n【核心任务】以下内容来自两书的“去内容化技法拆解”。你必须结合用户一开始给的方向护栏/脑洞，把A/B技法融合成原创新小说的循环执行方案：先给循环技法方案，再给可直接进入执笔台的原创细纲方向。若启用正文，后续正文必须按该原创细纲写入执笔台，严禁借原书角色、剧情、设定换皮。\n【创作模式边界】\n1. 原书只提供技法、节奏、钩子、信息差、爽点密度和结构公式\n2. 新书人物、冲突、世界规则必须服从用户方向护栏\n3. 可以生成原创细纲方向，但必须标注“原创”，不能出现原书专名\n4. 创作产物只进入执笔台和世界引擎，不进入拆书弹药库\n\n【技法来源A（${leftBook.name || '左书'}）】\n${leftCycleAnalyses.slice(0, 5000)}\n\n【技法来源B（${rightBook.name || '右书'}）】\n${rightCycleAnalyses.slice(0, 5000)}\n\n【本循环执笔台细纲参考】\n${cycleOutlines.slice(0, 2000)}`
            : `你是顶级网文技法融合大师，同时是NEXUS OS v2.0叙事工程的技法拆解专家。\n【核心原则】以下分析来自两书的技法拆解，你要做的是融合这些技法，产出一份去内容化的循环弹药包。拆书只提供弹药，不替用户锁死人物、世界规则和主线方向。\n【绝对禁令】\n1. 禁止出现原书角色名、地名、势力名\n2. 禁止复述原书的具体情节\n3. 禁止把原书设定换皮成新书设定\n4. 没有明确方向护栏时，不生成原创人物和世界观实体\n\n【技法来源A（${leftBook.name || '左书'}）】\n${leftCycleAnalyses.slice(0, 5000)}\n\n【技法来源B（${rightBook.name || '右书'}）】\n${rightCycleAnalyses.slice(0, 5000)}\n\n【本循环弹药库/细纲参考】\n${cycleOutlines.slice(0, 2000)}`;
        cycleFusionPrompt += `
${entityContext}
${prevCycleSummary}
...
8. 【可选细纲方向】只给方向和执行方式，不擅自新增锁死人物设定
9. 【下一循环优化建议】针对下${cycleSize}章的技法提升建议
10. 【可复用套路清单(零件库)】3-5个可直接套用的写作模板，每个含:名称+适用场景+执行步骤

${isCreative
    ? '创作融合模式：必须额外输出【原创循环细纲执行方案】，每章写清本章目标、阻力与代价、情节动作、人物变化、世界规则、伏笔钩子、实体线索、章末钩子。严禁原书换皮。'
    : '弹药模式：只输出循环弹药包，严禁生成完整新书剧情，严禁涉及原书的具体角色和情节。'}`;
        cycleFusionPrompt = this._withDirectionGuard ? this._withDirectionGuard(cycleFusionPrompt, isCreative ? '创作循环融合' : '循环弹药') : cycleFusionPrompt;
        if (isCreative && this._strictWritingLawText) {
        ...
        const cycleKey = `${cycleKeyPrefix}_${startIdx}_${endIdx}`;
        await DB.put('settings', { id: cycleKey, content: result, createdAt: Date.now() });
        await this._savePipelineAmmo?.('cycle', {
            cycleStart: startIdx,
            cycleEnd: endIdx,
            content: result,
            title: `${isCreative ? '创作循环弹药' : '循环弹药'} · 第${startIdx}-${endIdx}章`,
            tags: [isCreative ? '原创细纲方案' : '循环总结']
        });

        const nextCycleKey = `${cycleKeyPrefix}_prev_${endIdx + 1}`;
        await DB.put('settings', { id: nextCycleKey, content: result.slice(0, 2000), createdAt: Date.now() });

        this._allPipelineResults.fusion += `\n\n---\n\n## 循环融合: 第${startIdx}-${endIdx}章\n\n${result}`;
        await DB.put('settings', {
            id: isCreative ? 'creative_pipeline_fusion_context' : 'pipeline_fusion_context',
            content: this._allPipelineResults.fusion,
            updatedAt: Date.now()
        });
        ...
        if (!isCreative) {
            await DB.put('settings', {
                id: `fwb_cycle_${startIdx}_${endIdx}`,
                content: result,
            ...
            this._plLog(`🔄 循环数据→拆书弹药库（待入世界）`, 'ok');
            try { await Modules.fusion_workbench?.refresh?.(); } catch(e) {}
        } else {
            this._plLog(`🔄 循环融合方案→执笔台/世界引擎上下文`, 'ok');
        }
    },
...
只输出JSON，不要其他内容。`,
                { apiType: 'fusion', module: 'fusion_cycle_patterns' }, c => { patterns += c; }
            );
    ...
    async getCycleFusionForChapter(chapterIdx) {
        ...
        const prefix = this._plConfig?.flowMode === 'creative' ? 'creative_cycle_fusion' : 'cycle_fusion';
        const cycleKey = `${prefix}_${start}_${end}`;
        try {
            ...
            if (prefix !== 'cycle_fusion') {
                const fallbackKey = `cycle_fusion_${start}_${end}`;
                const fallback = await DB.get('settings', fallbackKey);
                if (fallback && fallback.content) return { cycleId: fallbackKey, start, end, fusion: fallback.content };
            }
        } catch(e) {}
        return null;
    },
