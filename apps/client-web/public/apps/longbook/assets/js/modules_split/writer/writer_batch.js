Object.assign(Modules.writer, {
    ...
    async _resolveAutoPolishConfig() {
        const fallback = { autoPolish: false, applyToBatch: true };
        let settings = fallback;
        try {
            if (typeof this._getPolishSettings === 'function') {
                settings = Object.assign({}, fallback, await this._getPolishSettings());
            } else if (typeof this._getWriterPolishSettings === 'function') {
                settings = Object.assign({}, fallback, await this._getWriterPolishSettings());
            }
        } catch (e) {
            console.warn('[Writer] 读取自动润色设置失败，回退默认值:', e);
            settings = fallback;
        }

        let autoPolish = !!settings.autoPolish;
        try {
            if (typeof this._isAutoPolishEnabled === 'function') {
                autoPolish = !!(await this._isAutoPolishEnabled());
            }
        } catch (e) {
            console.warn('[Writer] 读取自动润色开关失败，回退设置值:', e);
        }

        return {
            autoPolish,
            applyToBatch: settings.applyToBatch !== false,
            settings
        };
    },

    async _maybeAutoPolishGeneratedContent(rawContent, opts = {}) {
        const content = typeof rawContent === 'string' ? rawContent : '';
        const polishState = opts.polishState || await this._resolveAutoPolishConfig();
        const shouldPolish = !!(polishState.autoPolish && polishState.applyToBatch);
        if (!shouldPolish) {
            return { content, attempted: false, polished: false, fallback: false };
        }

        if (typeof this._polishText !== 'function') {
            console.warn('[Writer] 自动润色已开启，但 _polishText 不可用，保留原稿');
            return { content, attempted: false, polished: false, fallback: false };
        }

        const st = opts.st;
        const statusText = opts.statusText || '正在自动润色...';
        if (st) st.textContent = statusText;
        if (typeof opts.onStatus === 'function') opts.onStatus(statusText);

        try {
            const polishedResult = await this._polishText(content, {
                mode: opts.mode || 'auto-write',
                chapterId: opts.chapter?.id,
                title: opts.chapter?.title || '',
                outline: opts.chapter?.outline || '',
                projectId: opts.project?.id,
                preview: false,
                forceDeepSeekRecipe: true
            });
            const polished = this._extractPolishText(polishedResult);
            if (!polished || !polished.trim()) {
                throw new Error('润色结果为空');
            }
            if (opts.editorEl) {
                opts.editorEl.value = polished;
                this.onInput();
            }
            return { content: polished, attempted: true, polished: true, fallback: false };
        } catch (e) {
            console.warn(`[Writer] 自动润色失败，回退原稿: ${opts.chapter?.title || ''}`, e);
            if (opts.editorEl) {
                opts.editorEl.value = content;
                this.onInput();
            }
            if (st && opts.fallbackStatusText) st.textContent = opts.fallbackStatusText;
            if (typeof opts.onFallback === 'function') opts.onFallback(e);
            return { content, attempted: true, polished: false, fallback: true, error: e };
        }
    },
...
正文：`;
        ...
        try {
            ...
            const polishResult = await W._maybeAutoPolishGeneratedContent(content, {
                ...
                st,
                editorEl,
                statusText: `生成完成 ${content.length}字 · 正在自动润色...`,
                fallbackStatusText: `生成完成 ${content.length}字 · 自动润色失败，回退原稿...`
            });
