...
Modules.writer = {
    ...
    _getFusionContext() {
        ...
        const FW = Modules.fusion_workbench;
        const d = FW?._data || {};
        const workbenchFusion = [
            ...(d.fusionAmmos || []),
            ...(d.cycles || [])
        ].map(x => x.content || '').filter(Boolean).slice(-3).join('\n\n---\n\n');
        const workbenchCompare = (d.compares || []).map(x => x.content || '').filter(Boolean).slice(-2).join('\n\n---\n\n');
        if (!fusion && workbenchFusion) ctx += '[拆书弹药库融合弹药]\n' + workbenchFusion.slice(0, 3000) + '\n\n';
        if (!compare && workbenchCompare) ctx += '[拆书弹药库对比弹药]\n' + workbenchCompare.slice(0, 1500) + '\n\n';
        return ctx;
    },
    ...
    async _buildNexusPrefix(compact = false) {
        ...
        if (compact) {
            ...
            prefix += 'L1铁律:①长篇只允许第三人称有限，禁第一人称视角/上帝视角/非观察位内心 ②禁解释癖 ③禁烂俗比喻 ④禁虚词模糊 ⑤禁情绪标签(动作呈现) ⑥长句≤25字但长短交替(3-5短句后接1长句描写) ⑦章末钩子 ⑧对话只能用中文双引号“”，严禁「」 ⑨对话功能化 ⑩开篇必动作/对话 ⑪结局禁梦 ⑫时间线向前 ⑬禁OOC ⑭禁逻辑连词 ⑮段落≤5行 ⑯跨模块一致\n';
            prefix += 'P协议:物理替代·拒绝升华·沟通失效·细节碎片化·认知反差·逻辑自毁·感官钝化·权力不对等·时间扭曲·自我意识抹除\n';
            prefix += 'L2建议:每章≥2种感官·≥1日常小动作·短句≥30%·每2-3章1偶然事件\n';
            prefix += '输出禁令:正文不得出现内心OS、作者注、技法标签、读者期待、读者恐惧、反应涟漪、本章分析、写作目的、AI痕迹、NEXUS、M06、读者协议、Segment字段、emotion_score、hook_type、tension_level、characters_in_segment、括号里的写作意图。\n';
        ...
        prefix += '篇幅:长篇 | 黄金螺旋:拉5%→扯75%→放15%→收5% | 创作融合固定5章/卷\n';
        prefix += '章字数:2500-3500 | 爽点间隔≤3章 | 反转密度:长篇≥3次/卷\n\n';

        // === M06 Segment 状态 ===
        prefix += `=== M06 正文管理状态 (第${chapterNum}章) ===\n`;
        prefix += 'Segment字段只供后台记录，不得输出到正文。内部追踪: emotion_score(1-10) | hook_type(悬念/爽点/转折/情感/信息差) | tension_level(1-10)\n';
        prefix += '内部追踪: characters_in_segment | world_rules_referenced | foreshadowing_created/recycled\n\n';

        // === L1 铁律 16条 ===
        prefix += '=== L1 行文铁律（16条·违反即重写）===\n';
        prefix += '1.视角锁死:长篇只允许第三人称有限，禁第一人称视角/上帝视角/非观察位内心 | 2.禁解释癖:"这不是…而是…"等 | 3.禁烂俗比喻:≤2/千字\n';
        prefix += '4.禁虚词模糊:"似乎/仿佛/好像" | 5.禁情绪标签:动作/环境/对话呈现 | 6.长句≤25字但长短交替:3-5短句后接1长句描写,禁止通篇电报体 | 6a.对话必须用中文双引号“”，严禁「」\n';
        prefix += '7.章末必有钩子:未完成动作+意外信息 | 8.对话格式:中文双引号“”独立成段，严禁「」 | 9.对话功能化:推剧情/塑性格/埋伏笔/造情绪\n';
        ...
        prefix += '=== 输出禁令 ===\n';
        prefix += '正文不得出现内心OS、作者注、技法标签、读者期待、读者恐惧、反应涟漪、本章分析、写作目的、AI痕迹、NEXUS、M06、读者协议、Segment字段、emotion_score、hook_type、tension_level、characters_in_segment、括号里的写作意图。拆书术语、读者协议、M06条款只能做隐性约束，不能露在正文里。\n\n';
    ...
    async _requireActiveProject(opts = {}) {
        const proj = typeof GenesisCore !== 'undefined'
            ? await GenesisCore.requireActiveProject(opts.message || '请先创建或选择一个项目，再进入执笔台')
            : null;
        if (!proj) {
            this._resetProjectRuntime();
            if (opts.renderGate !== false) this._renderProjectGate();
        }
        return proj;
    },

    _renderProjectGate() {
        const view = document.getElementById('module-view-writer');
        if (view) view.innerHTML = this._projectRequiredView();
    },
    ...
    _resetProjectRuntime() {
        ...
        if (this._batchSelected) this._batchSelected.clear();
        if (this._postProcessTimers) {
            Object.values(this._postProcessTimers).forEach(t => clearTimeout(t));
            this._postProcessTimers = {};
        }
        ['w-editor','w-title','w-outline'].forEach(id => {
    ...
    _scopeRecords(items, projectId = GenesisCore?._activeProjectId) {
        return typeof GenesisCore !== 'undefined' ? GenesisCore.filterProjectItems(items || [], projectId) : (items || []);
    },

    _stampProject(record, projectId = GenesisCore?._activeProjectId) {
        return typeof GenesisCore !== 'undefined' ? GenesisCore.stampProjectRecord(record, projectId) : record;
    },

    render: () => {
        const hasProject = typeof GenesisCore !== 'undefined'
            ? !!GenesisCore._activeProjectId
            : !!localStorage.getItem('genesis_active_project');
        if (!hasProject) return Modules.writer._projectRequiredView();
        return `
        <div class="flex h-full bg-[#08080a] overflow-hidden">
            ...
            <div class="w-64 shrink-0 flex flex-col bg-[#111113] border-r border-white/5">
                ...
                <div class="p-3 border-b border-white/5 bg-[#0d0d0f]">
                    ...
                    <!-- 新建按钮 -->
	                    <div class="flex gap-1">
	                        ...
	                        <button class="btn btn-xs bg-red-500/10 text-red-400 border-red-500/20" onclick="Modules.writer.clearAll()" title="一键清空当前项目所有卷和章节"><i class="fa-solid fa-trash-can mr-1"></i>清空</button>
	                    </div>
                </div>
                <!-- 筛选栏 -->
                ...
                <div class="p-2 border-t border-white/5 bg-[#0d0d0f] space-y-1">
                    ...
                    <button class="btn btn-xs bg-gradient-to-r from-purple-600/20 to-pink-600/20 text-pink-300 border border-pink-500/20 w-full font-bold" onclick="Modules.writer.autoWriteScopedChapters()"><i class="fa-solid fa-rocket mr-1"></i>一键写完指定卷章正文</button>
                </div>
            </div>
            ...
            <div class="flex-1 flex flex-col min-w-0 relative">
                ...
                <div class="h-12 flex items-center justify-between px-5 bg-[#0d0d0f] border-b border-white/5 shrink-0 z-10">
                    ...
                    <div class="flex gap-2 items-center">
                        ...
                        ${Modules.world_engine?.renderExportMenu ? Modules.world_engine.renderExportMenu('导出工程', false, 'down') : `<button class="btn btn-xs bg-amber-600/20 text-amber-400 border-amber-600/30" onclick="Modules.world_engine?.exportAll?.()" title="导出世界引擎、细纲和正文"><i class="fa-solid fa-download mr-1"></i>导出工程</button>`}
                ...
                <div class="flex-1 flex flex-col min-h-0 relative">
                    ...
                    <div class="flex items-center gap-2 px-5 py-2 bg-[#0a0a0c] border-b border-white/5 shrink-0 flex-wrap">
                        ...
                        <button class="btn btn-xs border transition-all w-auto-polish-btn" onclick="Modules.writer.toggleAutoPolish()" title="开启后：AI先后台生成原稿，再润色，再输出润色版"><i class="fa-solid fa-toggle-off mr-1"></i>自动润色</button>
                        <button class="btn btn-xs border transition-all w-batch-polish-btn" onclick="Modules.writer.toggleBatchPolish()" title="开启后：批量/当前章自动生成保存润色版"><i class="fa-solid fa-layer-group mr-1"></i>批量润色</button>
                        <button class="btn btn-xs border transition-all w-deepseek-polish-btn" onclick="Modules.writer.toggleDeepSeekPolishRecipe()" title="显式启用 DeepSeek-v4 正则+提示词人性化润色配方"><i class="fa-solid fa-wand-sparkles mr-1"></i>DeepSeek配方</button>
                        ...
                        <button class="btn btn-xs bg-emerald-600/20 text-emerald-400 border-emerald-600/30" onclick="Modules.writer.openFusionAmmo()" title="打开拆书弹药库；没有弹药就去融合拆书先拆"><i class="fa-solid fa-box-open mr-1"></i>拆书</button>
                        <button class="btn btn-xs bg-emerald-600/20 text-emerald-400 border-emerald-600/30" onclick="Modules.writer.saveAndSync()" id="w-save-sync-btn" title="保存当前正文，并同步章内细纲、实体和世界图谱"><i class="fa-solid fa-diagram-project mr-1"></i>保存同步</button>
            ...
            <div class="w-80 shrink-0 flex flex-col bg-[#111113] border-l border-white/5">
                ...
                <div id="w-tab-outline" class="flex-1 hidden flex flex-col p-3 gap-2 min-h-0">
                    ...
                    <div id="w-outline-confirm" class="hidden shrink-0 bg-blue-900/10 border border-blue-500/20 rounded-lg p-2 space-y-2">
                        <div class="flex items-center justify-between">
                            <span class="text-[10px] text-blue-400 font-bold"><i class="fa-solid fa-code-compare mr-1"></i>细化预览</span>
                ...
                <div id="w-tab-style" class="flex-1 hidden flex flex-col p-3 gap-2 min-h-0 overflow-y-auto scrollbar-thin">
                    ...
                    <div class="text-[9px] text-dim leading-relaxed p-2 bg-emerald-500/5 rounded border border-emerald-500/10"><i class="fa-solid fa-info-circle text-emerald-400 mr-1"></i>粘贴一段目标风格的原文，AI将提取文风特征，<b class="text-emerald-400">全局应用于续写和润色</b>；M06/M07 去AI味规则会强制默认注入。</div>
                    <div class="space-y-2">
    ...
    async init() {
        ...
        await this._refreshPolishControls();
    ...
    async onShow() {
        ...
        let current = this.currentChapterId ? await DB.get('chapters', this.currentChapterId) : null;
        if (current?.projectId && current.projectId !== project.id) current = null;
        const chaps = this._scopeRecords(await DB.getAll('chapters') || [], project.id)
            .sort((a,b) => (a.order||0) - (b.order||0));
        if (!current && chaps.length > 0) {
            const withContent = chaps.find(c => c.content && c.content.trim());
            await this.load(withContent ? withContent.id : chaps[0].id);
        } else if (current) {
            await this._refreshInfoTab?.();
        }
    },
    ...
    async _injectProjectContext() {
        try {
            const proj = await this._requireActiveProject({ renderGate: true });
            if (!proj) return;
    ...
    async _readPolishSettingsFallback() {
        const data = await DB.get('settings', 'writer_polish_settings');
        return Object.assign({
            autoPolish: false,
            applyToBatch: true,
            typewriterSpeed: 16,
            recipeVersion: 'default-v1',
            recipeId: 'default-v1',
            deepseekV4: { enabled: false }
        }, data || {});
    },
    async _writePolishSettingsFallback(settings) {
        await DB.put('settings', Object.assign({ id: 'writer_polish_settings' }, settings || {}));
    },
    async _getWriterPolishSettings() {
        if (this._getPolishSettings) return await this._getPolishSettings();
        return await this._readPolishSettingsFallback();
    },
    async _setWriterPolishSettings(settings) {
        if (this._savePolishSettings) return await this._savePolishSettings(settings);
        return await this._writePolishSettingsFallback(settings);
    },
    async _refreshPolishControls() {
        let settings = {};
        try {
            settings = await this._getWriterPolishSettings();
        } catch(e) {
            settings = { autoPolish: false, applyToBatch: true };
        }
        const autoOn = !!(settings.autoPolishEnabled ?? settings.autoPolish);
        const batchOn = settings.applyToBatch !== false;
        const deepseekOn = settings.recipeId === 'deepseek-v4-humanize-v1' && settings.deepseekV4?.enabled === true;
        document.querySelectorAll('.w-auto-polish-btn').forEach(b => {
            b.className = `btn btn-xs border transition-all w-auto-polish-btn ${autoOn ? 'bg-purple-500/20 text-purple-300 border-purple-500/40' : 'bg-black/30 text-dim border-white/10'}`;
            b.innerHTML = `<i class="fa-solid ${autoOn ? 'fa-toggle-on' : 'fa-toggle-off'} mr-1"></i>自动润色`;
            b.title = autoOn ? '自动润色已开启：生成后先润色再输出' : '自动润色已关闭：AI续写保持原流式输出';
        });
        document.querySelectorAll('.w-batch-polish-btn').forEach(b => {
            b.className = `btn btn-xs border transition-all w-batch-polish-btn ${batchOn ? 'bg-blue-500/15 text-blue-300 border-blue-500/30' : 'bg-black/30 text-dim border-white/10'}`;
            b.innerHTML = `<i class="fa-solid fa-layer-group mr-1"></i>批量润色`;
            b.title = batchOn ? '批量润色已开启：自动生成章节会尝试保存润色版' : '批量润色已关闭：批量生成保存原始生成稿';
        });
        document.querySelectorAll('.w-deepseek-polish-btn').forEach(b => {
            b.className = `btn btn-xs border transition-all w-deepseek-polish-btn ${deepseekOn ? 'bg-emerald-500/15 text-emerald-300 border-emerald-500/30' : 'bg-black/30 text-dim border-white/10'}`;
            b.innerHTML = `<i class="fa-solid fa-wand-sparkles mr-1"></i>DeepSeek配方`;
            b.title = deepseekOn ? 'DeepSeek-v4 人性化润色配方已启用' : 'DeepSeek-v4 配方未启用，润色使用默认配方';
        });
    },
    async toggleAutoPolish() {
        const settings = await this._getWriterPolishSettings();
        const next = !(settings.autoPolishEnabled ?? settings.autoPolish);
        settings.autoPolishEnabled = next;
        settings.autoPolish = next;
        if (next) {
            settings.recipeId = 'deepseek-v4-humanize-v1';
            settings.deepseekV4 = Object.assign({}, settings.deepseekV4 || {}, { enabled: true });
        }
        await this._setWriterPolishSettings(settings);
        await this._refreshPolishControls();
        UI.toast(next ? '自动润色已开启：DeepSeek 全量正则+提示词已绑定' : '自动润色已关闭');
    },
    async toggleBatchPolish() {
        const settings = await this._getWriterPolishSettings();
        settings.applyToBatch = settings.applyToBatch === false;
        await this._setWriterPolishSettings(settings);
        await this._refreshPolishControls();
        UI.toast(settings.applyToBatch ? '批量润色已开启' : '批量润色已关闭');
    },
    async toggleDeepSeekPolishRecipe() {
        const settings = await this._getWriterPolishSettings();
        const next = !(settings.recipeId === 'deepseek-v4-humanize-v1' && settings.deepseekV4?.enabled === true);
        settings.recipeId = next ? 'deepseek-v4-humanize-v1' : 'default-v1';
        settings.deepseekV4 = Object.assign({}, settings.deepseekV4 || {}, { enabled: next });
        await this._setWriterPolishSettings(settings);
        await this._refreshPolishControls();
        UI.toast(next ? 'DeepSeek-v4 润色配方已启用' : '已切回默认润色配方');
    },
    setFlowMode(mode) {
    ...
    async openFusionAmmo() {
        ...
        this.aiOpts.flowMode = 'fusion';
        UI.toast(`已打开拆书弹药库，可用 ${count} 条弹药`);
        App.nav('fusion_workbench');
    },
    async goFusionBook() {
        try {
            if (this.currentChapterId) await this.save({ silent: true, skipPostProcess: true });
    ...
    _setSyncBusy(on) {
        const btn = document.getElementById('w-save-sync-btn');
        if (btn) {
            btn.classList.toggle('opacity-50', !!on);
            btn.classList.toggle('pointer-events-none', !!on);
        }
    },
    _updateSyncProgress(label, current = 0, total = 0, opts = {}) {
        const st = document.getElementById('w-save-status');
        if (st) st.textContent = label;
        if (opts.showProgress && typeof App !== 'undefined' && App.showProgress) {
            App.showProgress(label, current, total, false);
            if (App.logIO) App.logIO(`[保存同步] ${label}`, opts.type || 'info');
        }
    },
    _syncArray(value) {
    ...
    async saveAndSync(opts = {}) {
        ...
        try {
            ...
            this._updateSyncProgress('保存同步 · 正在保存正文', 1, 7, { showProgress: true });
            await this.save({ silent: true, skipPostProcess: true });
            this._updateSyncProgress('保存同步 · 正在启动实体同步流水线', 2, 7, { showProgress: true });
            const result = await this._runPostWriteProcessing(chapterId, {
                forcePostProcess: true,
                showProgress: true,
                source: opts.source || 'save_sync'
            });
            const summary = result?.entities
                ? `保存同步完成 · 实体 +${result.entities.added || 0} / 更新${result.entities.updated || 0}`
                : '保存同步完成 · 细纲和图谱已刷新';
            this._updateSyncProgress(summary, 7, 7, { showProgress: true, type: 'success' });
            UI.toast(summary, 'success');
            setTimeout(() => App.hideProgress?.(), 1200);
            return result;
        } catch(e) {
            console.error('[Writer] 保存同步失败:', e);
            this._updateSyncProgress('保存同步失败：' + (e.message || '未知错误'), 0, 7, { showProgress: true, type: 'error' });
            UI.toast('保存同步失败: ' + (e.message || '未知错误'), 'error');
            setTimeout(() => App.hideProgress?.(), 1600);
        } finally {
    ...
    async _runPostWriteProcessing(chapterId, opts = {}) {
        ...
        this._updateSyncProgress(`${flowLabel} · 正在从正文+细纲反推章内细纲...`, 3, 7, { showProgress: !!opts.showProgress });
        if (typeof UI !== 'undefined') UI.toast(`${flowLabel}：正在从正文+细纲反推细纲`, 'info');
        const outline = this._autoExtractOutline ? await this._autoExtractOutline(content, chapterId, opts) : null;
        this._updateSyncProgress(`${flowLabel} · 正在从正文+细纲提取实体...`, 4, 7, { showProgress: !!opts.showProgress });
        if (typeof UI !== 'undefined') UI.toast(`${flowLabel}：正在从正文+细纲提取实体`, 'info');
        const entityResult = this._autoExtractEntities ? await this._autoExtractEntities(content, chapterId, opts) : null;
        if (entityResult?.error) throw new Error(entityResult.error);
        this._updateSyncProgress(`${flowLabel} · 正在写入RAG章节上下文...`, 6, 7, { showProgress: !!opts.showProgress });
        if (typeof RAGSystem !== 'undefined') {
            ...
            try { await RAGSystem.refreshEntityCache?.(); } catch(e) {}
        }
        if (typeof GenesisCore !== 'undefined' && GenesisCore.refreshStats) {
            const proj = await GenesisCore.getActiveProject();
            if (proj) await GenesisCore.refreshStats(proj.id);
        }
        this._updateSyncProgress(`${flowLabel} · 细纲&实体已同步`, 7, 7, { showProgress: !!opts.showProgress, type: 'success' });
        if (!opts.showProgress && typeof UI !== 'undefined') UI.toast(`${flowLabel}：细纲和实体已同步`, 'success');
        if (opts.forcePostProcess && outline && this.currentChapterId === chapterId) this.tab('outline');
        return { outline, entities: entityResult };
    },
    _flowLabel(mode) {
        return ({
            hybrid: '写作+拆书',
    ...
    async load(id) {
        ...
        if (!chap) return;
        if (chap.projectId && chap.projectId !== proj.id) return UI.toast('该章节不属于当前项目', 'warning');
    ...
    async saveContent() {
        ...
        this._stampProject(chap, proj.id);
    ...
    // 从正文自动提取细纲（反推大纲）
    async _autoExtractOutline(content, chapterId, opts = {}) {
        if (!content || content.length < 100) return;
        const chap = chapterId ? await DB.get('chapters', chapterId) : null;
        if (!chap) return;
        const existingOutline = (chap.outline || '').trim();
        
        const prompt = `请同时读取【已有章节细纲】和【章节正文】，反推并优化「章内分部分执行级细纲」。

正文是已发生事实；细纲是结构意图、伏笔线索和后续续写接口。以正文事实为准，不得凭空新增剧情；以细纲补足结构、规则、伏笔命名。
...
6. 只输出细纲，不要解释。`;
        ...
        try {
            ...
            await AI.generate(prompt, { apiType: 'parse', module: 'writer', max_tokens: 2048, temperature: 0.5 }, c => {
                ...
                if (opts.showProgress && outline.length % 300 < c.length) {
                    this._updateSyncProgress(`反推章内细纲 · 接收 ${outline.length} 字`, 3, 7, { showProgress: true });
                }
            });
            if (outline && outline.length > 50) {
                this._updateSyncProgress('反推章内细纲 · 写入章节细纲', 4, 7, { showProgress: !!opts.showProgress });
                ...
                console.log(`[Writer] 第${chap.order}章从正文+细纲反推章内细纲完成 (${outline.length}字)`);
                return outline;
            }
        } catch(e) {
    ...
    async _autoExtractEntities(content, chapterId, opts = {}) {
        if (!content || content.length < 100) return { added: 0, updated: 0, total: 0 };
        const chap = chapterId ? await DB.get('chapters', chapterId) : null;
        if (!chap || !Modules.world_engine) return { added: 0, updated: 0, total: 0 };
        
        // 获取已有实体名称，避免重复提取
        this._updateSyncProgress('实体提取 · 读取已有世界图谱', 4, 7, { showProgress: !!opts.showProgress });
        await Modules.world_engine._ensureCache();
        const existingEntities = (Modules.world_engine._cachedEntities || []).filter(e => !e.id.startsWith('world_'));
        const existingNames = existingEntities.map(e => e.name);
        const existingMap = new Map(existingEntities.map(e => [String(e.name || '').trim(), e]).filter(([name]) => !!name));
        ...
        const volume = volumeId ? await DB.get('volumes', volumeId).catch(() => null) : null;
        ...
        if (chap.order && Modules.world_engine.getCycleIdForChapter) {
            ...
            if (cycleInfo) cycleId = cycleInfo.cycleId || cycleInfo.id;
        }
        ...
        try {
            ...
            await AI.generate(prompt, { apiType: 'parse', module: 'writer', max_tokens: 3000, temperature: 0.25 }, c => {
                ...
                if (opts.showProgress && jsonStr.length % 240 < c.length) {
                    this._updateSyncProgress(`实体提取 · 接收 JSON ${jsonStr.length} 字`, 5, 7, { showProgress: true });
                }
            });
            
            // 健壮地清理JSON字符串
            this._updateSyncProgress('实体提取 · 解析 JSON 结构', 5, 7, { showProgress: !!opts.showProgress });
            let cleanRaw = jsonStr.trim().replace(/^```(?:json)?\s*\n?/i, '').replace(/\n?```\s*$/, '').trim();
            const start = cleanRaw.indexOf('[');
            const end = cleanRaw.lastIndexOf(']');
            if (start === -1 || end <= start) return { added: 0, updated: 0, total: 0 };
            
            const entities = JSON.parse(cleanRaw.slice(start, end + 1));
            if (!Array.isArray(entities) || entities.length === 0) return { added: 0, updated: 0, total: 0 };
            ...
            for (const ent of entities) {
                ...
                written++;
                this._updateSyncProgress(`实体写入 · ${written}/${entities.length} ${ent.name}`, written, entities.length, { showProgress: !!opts.showProgress });
                
                // 查找是否已存在同名实体
                const existing = existingMap.get(String(ent.name || '').trim());
                ...
                if (existing) {
                    // 更新已有实体
                    id = existing.id;
                    chapters = this._syncArray(existing.chapters);
                    cycles = this._syncArray(existing.cycles);
                    volumes = this._syncArray(existing.volumes);
                    ...
                    // 合并描述：追加本章新信息
                    const newDesc = (existing.desc ? `${existing.desc}\n\n[第${chap.order}章更新]\n${ent.desc}` : ent.desc || '').slice(0, 3000);
                    // 合并relations
                    const existingRels = Array.isArray(existing.relations)
                        ? existing.relations
                        : (typeof existing.relations === 'string' ? existing.relations.split(',').map(s => s.trim()).filter(Boolean) : []);
                    ...
                    const entityData = {
                        ...
                        cycles,
                        volumes,
                        chapterRef: [...new Set([...this._syncArray(existing.chapterRef), chap.order].filter(Boolean))],
                        chapterTitle: existing.chapterTitle || chap.title || '',
                        chapterTitles: [...new Set([...this._syncArray(existing.chapterTitles), chap.title].filter(Boolean))],
                        volumeTitle: existing.volumeTitle || volume?.title || chap.volumeTitle || '',
                        volumeTitles: [...new Set([...this._syncArray(existing.volumeTitles), volume?.title || chap.volumeTitle].filter(Boolean))],
                        updatedAt: Date.now()
                    };
                    await DB.put('entities', entityData);
                    existingMap.set(String(ent.name || '').trim(), entityData);
                    updated++;
                } else {
                    // 新建实体
                    id = Utils.uuid();
                    ...
                    const entityData = {
                        ...
                        relations, chapters, cycles, volumes,
                        chapterRef: chap.order ? [chap.order] : [],
                        chapterTitle: chap.title || '',
                        chapterTitles: chap.title ? [chap.title] : [],
                        volumeTitle: volume?.title || chap.volumeTitle || '',
                        volumeTitles: (volume?.title || chap.volumeTitle) ? [volume?.title || chap.volumeTitle] : [],
                        source, nexusState: null, updatedAt: Date.now()
                    };
                    await DB.put('entities', entityData);
                    existingMap.set(String(ent.name || '').trim(), entityData);
                    added++;
            ...
            // 刷新缓存
            this._updateSyncProgress('实体提取 · 重建分层图谱', 6, 7, { showProgress: !!opts.showProgress });
            ...
            console.log(`[Writer] 第${chap.order}章正文+细纲实体提取完成: 新增${added}个, 更新${updated}个`);
            if (!opts.showProgress) UI.toast(`正文+细纲实体提取: +${added} 更新${updated}`);
            return { added, updated, total: added + updated };
        } catch(e) {
            console.error('[Writer] 自动提取实体失败:', e);
            if (opts.showProgress) this._updateSyncProgress('实体提取失败：' + (e.message || '解析错误'), 0, 7, { showProgress: true, type: 'error' });
            return { added: 0, updated: 0, total: 0, error: e.message || String(e) };
        }
    },
