...
class TestPage {
        ...
        return [
            {
                id: 'npti',
                name: 'NPTI人格测试',
                description: '探索你的人格类型，了解内心真实的自己',
                icon: '🎭',
    ...
    bindEvents() {
        ...
        grid?.addEventListener('click', (e) => {
            ...
            if (card) {
                this.startTest(card.dataset.testId);
            }
        });
    ...
    filterTests() {
        ...
        cards.forEach(card => {
            ...
            if (test) {
                if (this.currentCategory === 'all' || test.category === this.currentCategory) {
    ...
    loadQuestion() {
        ...
        optionsList.querySelectorAll('.option-item').forEach(item => {
            item.addEventListener('click', () => {
                this.selectOption(parseInt(item.dataset.index));
            });
        });
        ...
        document.querySelector('#next-btn').textContent = 
            this.currentQuestionIndex === questions.length - 1 ? '提交' : '下一题';
    }
    ...
    getMockQuestions(testId) {
        ...
        return [
            {
                text: '在社交场合中，你通常感觉如何？',
                options: ['非常自在', '比较自在', '有些紧张', '非常紧张']
            },
            {
                text: '面对压力时，你倾向于？',
                options: ['主动解决', '寻求帮助', '独自思考', '逃避问题']
            },
            {
                text: '你认为自己是？',
                options: ['理想主义者', '现实主义者', '介于两者之间', '不确定']
            },
            {
                text: '在做决定时，你更依赖？',
                options: ['逻辑分析', '直觉感受', '他人建议', '随机决定']
            },
            {
                text: '你更喜欢哪种工作方式？',
                options: ['独立工作', '团队协作', '两者皆可', '视情况而定']
            }
        ];
    }
