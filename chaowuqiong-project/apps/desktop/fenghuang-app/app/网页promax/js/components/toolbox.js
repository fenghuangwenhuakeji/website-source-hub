...
function initializeToolbox() {
   ...
   initializeOneClickExport();
   initializePromptWorkflow();
...
function initializeBatchExport() {
   const exportBtn = document.getElementById('batch-export-txt-btn');
   if (!exportBtn) return;

   exportBtn.addEventListener('click', async () => {
       const selectedProjects = Array.from(document.querySelectorAll('.project-item.selected'));

       if (selectedProjects.length === 0) {
           showNotification('请先在左侧项目列表中选择至少一个项目！', 'error');
           return;
       }

       showNotification(`开始批量导出 ${selectedProjects.length} 个项目...`, 'info');

       for (const projectElement of selectedProjects) {
           ...
           try {
               ...
               if (project && project.messages) {
                   ...
                   if (lastMessage && lastMessage.sender === 'gemini') {
                       ...
                       await new Promise(resolve => setTimeout(resolve, 300)); // 防止浏览器阻止连续下载
                   }
               }
           } catch (error) {
               showNotification(`导出项目 ${projectElement.querySelector('.project-item-name')?.textContent || projectId} 失败: ${error.message}`, 'error');
           }
       }
...
* 初始化提示词工作流功能。
*/
function initializePromptWorkflow() {
   const modal = document.getElementById('prompt-workflow-modal');
   if (!modal) return;

   const openBtn = document.getElementById('prompt-workflow-btn');
   const closeBtn = modal.querySelector('.close-btn');
   const startBtn = document.getElementById('start-workflow-btn');
   const generatorSelect = document.getElementById('workflow-generator-prompt');
   const processorsContainer = document.getElementById('workflow-processors-container');
   const addProcessorBtn = document.getElementById('add-processor-btn');

   openBtn.addEventListener('click', () => {
       // 填充所有下拉框
       const allPrompts = customPrompts.map((p, index) => `<option value="${index}">${p.title}</option>`).join('');
       generatorSelect.innerHTML = allPrompts;
       processorsContainer.querySelectorAll('select').forEach(select => {
           select.innerHTML = `<option value="">-- 选择一个处理器 --</option>${allPrompts}`;
       });
       modal.classList.add('visible');
   });
   ...
   addProcessorBtn.addEventListener('click', () => {
       ...
       newSelect.className = 'workflow-processor-prompt';
       const allPrompts = customPrompts.map((p, index) => `<option value="${index}">${p.title}</option>`).join('');
       newSelect.innerHTML = `<option value="">-- 选择一个处理器 --</option>${allPrompts}`;
       processorsContainer.appendChild(newSelect);
   });
...
* 初始化一键导出TXT功能。
*/
function initializeOneClickExport() {
   const exportBtn = document.getElementById('one-click-export-txt-btn');
   if (!exportBtn) return;
...
async function startPromptWorkflow() {
    const generatorIndex = document.getElementById('workflow-generator-prompt').value;
    const itemCount = parseInt(document.getElementById('workflow-item-count').value, 10);
    const processorIndexes = Array.from(document.querySelectorAll('.workflow-processor-prompt'))
        .map(select => select.value)
        .filter(value => value !== "");

    if (!generatorIndex || processorIndexes.length === 0) {
        showNotification('请至少选择一个生成器和一个处理器！', 'error');
        return;
    }

    const generatorPrompt = customPrompts[generatorIndex];
    const processorPrompts = processorIndexes.map(index => customPrompts[index]);

    showNotification(`工作流启动：将生成 ${itemCount} 个项目，每个项目经过 ${processorPrompts.length} 个处理步骤。`, 'info');

    // 1. 调用生成器
    const generationCommand = `${generatorPrompt.prompt}\n\n---\n\n请生成 ${itemCount} 个项目。`;
    addMessageToHistory('user', generationCommand);

    // 创建生成器占位符
    const { messageContentElement: genMessageContent, footerElement: genFooter } = createStreamingMessage();
    const generatedText = await callApi(generationCommand).catch(err => {
        genFooter.textContent = '生成失败';
        genFooter.style.color = 'var(--error-color)';
        genFooter.classList.remove('generating');
        showNotification(`生成器调用失败: ${err.message}`, 'error');
        return null;
    });

    if (generatedText === null) return; // 如果生成失败则中止工作流

    genMessageContent.innerHTML = DOMPurify.sanitize(marked.parse(generatedText));
    genMessageContent.dataset.rawText = generatedText;
    genFooter.textContent = '生成完毕';
    genFooter.classList.remove('generating');
    ...
    if (items.length === 0) {
        showNotification('生成器未能产出任何项目，工作流中止。', 'error');
        return;
    }

    showNotification(`生成了 ${items.length} 个项目，开始逐一处理...`, 'info');

    // 2. 循环处理每个项目
    for (let i = 0; i < items.length; i++) {
        let currentItemContent = items[i];
        const projectName = currentItemContent.substring(0, 20).trim();
        
        showNotification(`正在处理项目 ${i + 1}/${items.length}: ${projectName}...`, 'info');

        let finalContent = `**原始项目:** ${currentItemContent}\n\n---\n\n`;

        for (const processor of processorPrompts) {
            const processCommand = `${processor.prompt}\n\n---\n\n请处理以下内容：\n\n${currentItemContent}`;
            addMessageToHistory('user', processCommand);

            // 创建处理器占位符
            const { messageContentElement: procMessageContent, footerElement: procFooter } = createStreamingMessage();
            const processedContent = await callApi(processCommand).catch(err => {
                procFooter.textContent = '处理失败';
                procFooter.style.color = 'var(--error-color)';
                procFooter.classList.remove('generating');
                showNotification(`处理器 ${processor.title} 调用失败: ${err.message}`, 'error');
                return `处理失败，保留原始项目内容。`; // 返回错误提示
            });

            procMessageContent.innerHTML = DOMPurify.sanitize(marked.parse(processedContent));
            procMessageContent.dataset.rawText = processedContent;
            procFooter.textContent = `处理完毕: ${processor.title}`;
            procFooter.classList.remove('generating');
            
            currentItemContent = processedContent; // 更新内容以供下一步处理
            finalContent += `**处理器: ${processor.title}**\n\n${currentItemContent}\n\n---\n\n`;
        }

        // 3. 保存为项目
        const messagesToSave = [
            { sender: 'user', content: `工作流: ${generatorPrompt.title} -> ${processorPrompts.map(p=>p.title).join(' -> ')}` },
            { sender: 'gemini', content: finalContent }
        ];
        await saveCurrentProject(projectName, `由工作流自动生成`, '工作流', messagesToSave);
    }

    showNotification('提示词工作流全部执行完毕！', 'success');
    await renderProjectList();
}