...
Modules.reader_center = {
    ...
    render: () => {
        ...
        return `
        <div class="flex h-full bg-white text-[#e4e4e7] overflow-hidden">
            <!-- 左侧导航 -->
            <div class="w-72 shrink-0 bg-white border-r border-gray-200 flex flex-col z-20" id="rc-sidebar">
                <div class="p-4 border-b border-gray-200 flex items-center gap-3 bg-gradient-to-r from-amber-900/20 to-transparent">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-gray-600 font-bold">书架 · 排版 · Markdown</p>
                    </div>
                </div>
                <div class="flex border-b border-gray-200">
                    ${[['library','书架','fa-book'],['typeset','排版','fa-text-height'],['markdown','MD','fa-brands fa-markdown']].map(([k,v,i]) => `
                        <button class="flex-1 py-3 text-xs font-bold transition-all ${RC.currentTab===k ? 'text-amber-600 border-b-2 border-amber-500' : 'text-gray-600 hover:text-gray-800'}" onclick="Modules.reader_center.switchTab('${k}')">
                            <i class="${i} mr-1"></i>
                            <span>${v}</span>
                        </button>
                    `).join('')}
            ...
            <!-- 中间主区域 -->
            <div class="flex-1 relative flex flex-col h-full overflow-hidden bg-white" id="rc-main">
        ...
        if (RC.currentTab === 'library') {
            return `
                <div class="p-3 space-y-2">
                    <button class="btn w-full h-11 rounded-lg bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-400 hover:to-orange-400 text-white border-none shadow-lg shadow-amber-500/30 hover:shadow-amber-500/50 transition-all hover:scale-105 font-bold text-sm flex center gap-2" onclick="document.getElementById('rc-upload').click()">
                        <i class="fa-solid fa-plus"></i>
                        <span>导入新书</span>
                    </button>
                    <input type="file" id="rc-upload" class="hidden" accept=".txt,.md,.html" onchange="Modules.reader_center.handleUpload(this)">
                    <button class="btn w-full h-11 rounded-lg bg-gradient-to-r from-gray-500 to-gray-600 hover:from-gray-400 hover:to-gray-500 text-white border-none shadow-lg shadow-gray-500/30 hover:shadow-gray-500/50 transition-all hover:scale-105 font-bold text-sm flex center gap-2" onclick="Modules.reader_center.importFromClipboard()">
                        <i class="fa-solid fa-paste"></i>
                        <span>从剪贴板导入</span>
                    </button>
                </div>
                <div class="px-3 py-1 space-y-0.5">
                    <div class="px-2 py-1 text-xs font-bold text-gray-700 flex items-center gap-2">
                    ...
                    ${[['all','全部','fa-layer-group'],['recent','最近阅读','fa-clock-rotate-left'],['favorite','收藏','fa-star'],['generated','AI生成','fa-wand-magic-sparkles']].map(([k,v,i]) => `
                        <button class="w-full flex items-center gap-2 px-3 py-2 rounded-lg text-xs transition-all border border-transparent ${RC.shelfFilter===k ? 'bg-gradient-to-r from-amber-50 to-orange-50 text-amber-700 border-amber-200 font-bold shadow-sm' : 'text-gray-700 hover:bg-gray-100 hover:border-gray-200'}" onclick="Modules.reader_center.setFilter('${k}')">
                            <i class="fa-solid ${i} w-5 text-center text-base"></i>
                ...
                <div class="px-3 py-1">
                    ...
                    <div class="flex gap-1 px-2">
                        ${[['date','时间'],['name','名称'],['size','大小']].map(([k,v]) => `
                            <button class="px-3 py-2 rounded-lg text-xs font-bold ${RC.shelfSort===k ? 'bg-gradient-to-r from-indigo-500 to-purple-500 text-white border-2 border-indigo-400 shadow-lg shadow-indigo-500/30' : 'bg-white text-gray-700 border-2 border-gray-200 hover:border-indigo-300 hover:bg-indigo-50'}" onclick="Modules.reader_center.setSort('${k}')">${v}</button>
                        `).join('')}
                    </div>
                </div>
                <div class="p-3 border-t border-gray-200 mt-auto">
                    <div class="grid grid-cols-3 gap-2">
                        <div class="bg-gradient-to-br from-amber-50 to-orange-50 rounded-lg p-3 text-center border-2 border-amber-200">
                            <div class="text-2xl font-bold text-amber-600" id="rc-stat-count">0</div>
                            <div class="text-xs font-bold text-gray-600 mt-1">藏书</div>
                        </div>
                        <div class="bg-gradient-to-br from-yellow-50 to-amber-50 rounded-lg p-3 text-center border-2 border-yellow-200">
                            <div class="text-2xl font-bold text-yellow-600" id="rc-stat-bookmarks">0</div>
                            <div class="text-xs font-bold text-gray-600 mt-1">书签</div>
                        </div>
                        <div class="bg-gradient-to-br from-green-50 to-emerald-50 rounded-lg p-3 text-center border-2 border-green-200">
                            <div class="text-2xl font-bold text-green-600" id="rc-stat-notes">0</div>
                            <div class="text-xs font-bold text-gray-600 mt-1">批注</div>
                        </div>
                    </div>
                </div>`;
        }
                ...
                <div class="p-3 flex flex-col gap-3 flex-1">
                    <div class="flex items-center justify-between">
                        <span class="text-sm font-bold text-gray-800 flex items-center gap-2">
                            <i class="fa-solid fa-text-height text-indigo-400"></i>
                            智能排版
                        </span>
                        <button class="btn px-3 py-2 bg-gradient-to-r from-yellow-500 to-orange-500 hover:from-yellow-400 hover:to-orange-400 text-white border-none shadow-lg shadow-yellow-500/30 hover:shadow-yellow-500/50 transition-all hover:scale-105 rounded-lg" onclick="Modules.reader_center.aiFormat()">
                            <i class="fa-solid fa-wand-magic-sparkles mr-2"></i>
                            <span class="text-xs font-bold">美化</span>
                        </button>
                    </div>
                    <div class="flex flex-col gap-2">
                        ...
                        <select class="epic-input h-10 rounded-lg text-xs text-gray-800 font-bold px-3 border-2 border-gray-300 focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-200" onchange="Modules.reader_center.updateTypeset('font',this.value)">
                    ...
                    <div class="grid grid-cols-2 gap-2">
                        <div class="flex flex-col gap-1">
                            <span class="text-xs font-bold text-gray-700 flex items-center gap-2">
                            <i class="fa-solid fa-text-size text-indigo-400"></i>
                            字号
                        </span>
                            <div class="flex items-center bg-gray-200 rounded border border-gray-300">
                                <button class="w-8 h-8 rounded-lg flex center hover:bg-gray-200 text-gray-600" onclick="Modules.reader_center.adjustTypeset('size',-1)"><i class="fa-solid fa-minus text-xs"></i></button>
                                <input type="number" class="w-full bg-transparent border-none text-center text-xs font-bold text-gray-800 h-6" value="${s.size}" id="rc-ty-size" onchange="Modules.reader_center.updateTypeset('size',this.value)">
                                <button class="w-8 h-8 rounded-lg flex center hover:bg-gray-200 text-gray-600" onclick="Modules.reader_center.adjustTypeset('size',1)"><i class="fa-solid fa-plus text-xs"></i></button>
                            </div>
                        </div>
                        <div class="flex flex-col gap-1">
                            <span class="text-xs font-bold text-gray-700 flex items-center gap-2">
                            <i class="fa-solid fa-arrows-up-down-line text-indigo-400"></i>
                            行距
                        </span>
                            <div class="flex items-center bg-gray-200 rounded border border-gray-300">
                                <button class="w-8 h-8 rounded-lg flex center hover:bg-gray-200 text-gray-600" onclick="Modules.reader_center.adjustTypeset('lineHeight',-0.1)"><i class="fa-solid fa-minus text-xs"></i></button>
                                <input type="number" class="w-full bg-transparent border-none text-center text-xs font-bold text-gray-800 h-6" value="${s.lineHeight}" step="0.1" id="rc-ty-lh" onchange="Modules.reader_center.updateTypeset('lineHeight',this.value)">
                                <button class="w-8 h-8 rounded-lg flex center hover:bg-gray-200 text-gray-600" onclick="Modules.reader_center.adjustTypeset('lineHeight',0.1)"><i class="fa-solid fa-plus text-xs"></i></button>
                            </div>
                        </div>
                    </div>
                    <div class="flex flex-col gap-1">
                        ...
                        <div class="flex bg-gray-200 rounded border border-gray-300 p-0.5">
                            ${['left','center','right','justify'].map(a => `<button class="flex-1 btn-xs hover:bg-gray-200 text-dim hover:text-gray-800" onclick="Modules.reader_center.updateTypeset('align','${a}')"><i class="fa-solid fa-align-${a}"></i></button>`).join('')}
                        </div>
                    </div>
                    <div class="flex flex-col gap-1">
                        <span class="text-xs font-bold text-gray-700 flex items-center gap-2">
                        ...
                        <div class="flex bg-gray-200 rounded border border-gray-300 p-0.5">
                            ${[1,2,3].map(n => `<button class="flex-1 btn-xs hover:bg-gray-200 text-dim hover:text-gray-800" onclick="Modules.reader_center.updateTypeset('columns',${n})">${n}</button>`).join('')}
                        </div>
                    </div>
                    <div class="flex-1 flex flex-col gap-1 min-h-[100px]">
                        ...
                        <textarea id="rc-ty-in" class="flex-1 bg-white border-2 border-gray-300 rounded-lg p-3 text-sm text-gray-700 resize-none focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-200 leading-relaxed" placeholder="粘贴需要排版的文本..." oninput="Modules.reader_center.renderTypesetPage()"></textarea>
                    </div>
                    <div class="flex gap-2">
                        <button class="btn flex-1 py-2.5 bg-gradient-to-r from-gray-500 to-gray-600 hover:from-gray-400 hover:to-gray-500 text-white border-none shadow-lg shadow-gray-500/30 hover:shadow-gray-500/50 transition-all hover:scale-105 rounded-lg" onclick="window.print()">
                            <i class="fa-solid fa-print mr-2"></i>
                            <span class="text-xs font-bold">打印</span>
                        </button>
                        <button class="btn flex-1 py-2.5 bg-gradient-to-r from-yellow-500 to-orange-500 hover:from-yellow-400 hover:to-orange-400 text-white border-none shadow-lg shadow-yellow-500/30 hover:shadow-yellow-500/50 transition-all hover:scale-105 rounded-lg" onclick="Modules.reader_center.exportToLibrary('typeset')">
                            <i class="fa-solid fa-book mr-2"></i>
                            <span class="text-xs font-bold">存书架</span>
                        </button>
                    </div>
                </div>`;
        }
                ...
                <div class="p-3 flex flex-col gap-2 h-full">
                    ...
                    <div class="p-3 bg-gradient-to-r from-purple-50 to-pink-50 rounded-lg border-2 border-purple-200">
                            <div class="flex items-start gap-2">
                                <i class="fa-brands fa-markdown text-purple-400 text-base mt-0.5"></i>
                                <span class="text-xs text-gray-700 font-bold leading-relaxed">支持标准 Markdown 语法，实时预览。</span>
                            </div>
                        </div>
                    <button class="btn w-full py-2.5 bg-gradient-to-r from-green-500 to-emerald-500 hover:from-green-400 hover:to-emerald-400 text-white border-none shadow-lg shadow-green-500/30 hover:shadow-green-500/50 transition-all hover:scale-105 rounded-lg" onclick="Modules.reader_center.exportToLibrary('markdown')">
                        <i class="fa-solid fa-book mr-2"></i>
                        <span class="text-xs font-bold">存书架</span>
                    </button>
                </div>`;
    ...
    _renderMainContent: () => {
        ...
        if (RC.currentTab === 'library') {
            return `
                <!-- 书架视图 -->
                <div id="rc-shelf-view" class="absolute inset-0 z-10 flex flex-col bg-[#F1F3F5]">
                    <div class="p-5 flex-1 flex flex-col">
                        <div class="flex justify-between items-center mb-5">
                            <div class="flex gap-3 text-xs font-bold">
                                <span class="text-gray-800 border-b-2 border-amber-500 pb-1">全部</span>
                            </div>
                            <div class="relative">
                                <i class="fa-solid fa-search absolute left-3 top-1/2 -translate-y-1/2 text-dim text-xs"></i>
                                <input class="bg-gray-100 border border-gray-300 rounded-full pl-8 pr-4 h-8 text-xs w-48 focus:border-amber-500 text-gray-800 outline-none" placeholder="搜索书名..." oninput="Modules.reader_center.searchBooks(this.value)">
                            </div>
                        </div>
                ...
                <!-- 阅读视图 -->
                <div id="rc-reader-view" class="absolute inset-0 z-20 flex flex-col bg-white hidden">
                    <div class="h-11 flex justify-between items-center px-4 bg-white/95 backdrop-blur border-b border-gray-200 z-30 shrink-0">
                        <div class="flex items-center gap-3">
                            <button class="w-8 h-8 rounded-full bg-gray-100 hover:bg-gray-200 flex center text-dim" onclick="Modules.reader_center.closeReader()"><i class="fa-solid fa-arrow-left"></i></button>
                            <span id="rc-reader-title" class="font-serif font-bold text-sm text-gray-600 truncate max-w-md"></span>
                            <span class="text-xs font-bold text-gray-600 bg-gray-100 px-3 py-1.5 rounded-lg border border-gray-200" id="rc-progress-badge">0%</span>
                        </div>
                        <div class="flex gap-1 items-center">
                            <button class="w-9 h-9 rounded-lg hover:bg-amber-50 flex center text-gray-600 hover:text-amber-500 transition-all" onclick="Modules.reader_center.addBookmark()" title="添加书签"><i class="fa-solid fa-bookmark text-sm"></i></button>
                            <button class="w-9 h-9 rounded-lg hover:bg-green-50 flex center text-gray-600 hover:text-green-500 transition-all" onclick="Modules.reader_center.addAnnotation()" title="添加批注"><i class="fa-solid fa-pen text-sm"></i></button>
                            <div class="w-px h-4 bg-gray-200 mx-1"></div>
                            <div class="flex items-center bg-gray-100 rounded-full border border-gray-200 p-0.5 gap-0.5">
                                <button class="w-8 h-8 rounded-lg hover:bg-gray-200 flex center text-gray-600" onclick="Modules.reader_center.toggleSize(-2)"><i class="fa-solid fa-minus text-xs"></i></button>
                                <span class="text-xs font-bold text-gray-700 font-mono w-10 text-center" id="rc-font-disp">${RC.currentSize}</span>
                                <button class="w-8 h-8 rounded-lg hover:bg-gray-200 flex center text-gray-600" onclick="Modules.reader_center.toggleSize(2)"><i class="fa-solid fa-plus text-xs"></i></button>
                            </div>
                            <div class="flex items-center bg-gray-100 rounded-full border border-gray-200 p-0.5 gap-0.5">
                                <button class="w-9 h-9 rounded-lg hover:bg-amber-50 flex center text-gray-600 hover:text-amber-500 transition-all" onclick="Modules.reader_center.setReaderTheme('light')"><i class="fa-solid fa-sun text-base"></i></button>
                                <button class="w-9 h-9 rounded-lg hover:bg-indigo-50 flex center text-gray-600 hover:text-indigo-500 transition-all" onclick="Modules.reader_center.setReaderTheme('dark')"><i class="fa-solid fa-moon text-base"></i></button>
                                <button class="w-9 h-9 rounded-lg hover:bg-amber-50 flex center text-gray-600 hover:text-amber-500 transition-all" onclick="Modules.reader_center.setReaderTheme('sepia')"><i class="fa-solid fa-mug-hot text-base"></i></button>
                            </div>
                        </div>
                    </div>
                    ...
                    <!-- 底部进度条 -->
                    <div class="h-1 bg-gray-200 shrink-0"><div class="h-full bg-amber-500 transition-all" id="rc-progress-bar" style="width:0%"></div></div>
                </div>`;
        }
        if (RC.currentTab === 'typeset') {
            return `
                <div class="bg-[#2a2a2a] p-6 flex justify-center overflow-auto flex-1 relative" style="background-image:radial-gradient(#333 1px,transparent 1px);background-size:20px 20px;">
                    ...
                    <div class="fixed bottom-6 left-1/2 -translate-x-1/2 flex gap-2 bg-black/80 p-1.5 rounded-full backdrop-blur border border-gray-300 shadow-xl z-20">
                        <button class="w-7 h-7 rounded-full flex center text-gray-800 hover:bg-gray-300" onclick="Modules.reader_center.typesetZoomFn(-0.1)"><i class="fa-solid fa-minus text-xs"></i></button>
                        <span class="flex center text-xs font-bold text-gray-700 font-mono w-12 bg-gray-100 px-2 py-1 rounded-lg border border-gray-200" id="rc-ty-zoom">${Math.round(RC.typesetZoom * 100)}%</span>
                        <button class="w-7 h-7 rounded-full flex center text-gray-800 hover:bg-gray-300" onclick="Modules.reader_center.typesetZoomFn(0.1)"><i class="fa-solid fa-plus text-xs"></i></button>
                        <div class="w-px h-4 bg-gray-300 my-auto"></div>
                        ${[['light','#fff'],['sepia','#f4ecd8'],['dark','#1a1a1a']].map(([t,c]) => `
                            <button class="w-7 h-7 rounded-full flex center text-gray-800 hover:bg-gray-300" onclick="Modules.reader_center.setTypesetTheme('${t}')"><div class="w-3 h-3 rounded-full border border-gray-400" style="background:${c}"></div></button>
                        `).join('')}
                    </div>
                </div>`;
        }
            ...
            return `
                <div class="flex h-full">
                    <div class="flex-1 flex flex-col border-r border-gray-300">
                        <textarea id="rc-md-input" class="w-full h-full bg-gray-50 text-gray-600 p-5 font-mono text-sm resize-none focus:outline-none" oninput="Modules.reader_center.parseMD()" placeholder="# Markdown 编辑器\n\n在此输入 Markdown..."></textarea>
                    </div>
    ...
    _renderAIPanel: () => {
        return `
            <div class="w-[340px] bg-white border-l border-gray-200 flex flex-col z-20 shrink-0" id="rc-ai-sidebar">
                <div class="flex border-b border-gray-200 bg-gray-100">
                    <button class="flex-1 py-3 text-xs font-bold text-amber-600 border-b-2 border-amber-500" onclick="Modules.reader_center.switchAITab('assistant')" id="rc-ai-tab-assistant">AI 助手</button>
                    <button class="flex-1 py-3 text-xs font-bold text-gray-600 hover:text-gray-800" onclick="Modules.reader_center.switchAITab('chat')" id="rc-ai-tab-chat">对话</button>
                    <button class="flex-1 py-3 text-xs font-bold text-gray-600 hover:text-gray-800" onclick="Modules.reader_center.switchAITab('notes')" id="rc-ai-tab-notes">笔记</button>
                </div>
                <!-- AI 助手 -->
                <div id="rc-ai-assistant" class="flex-1 flex flex-col overflow-hidden">
                    <div id="rc-ai-io" class="hidden absolute top-0 left-0 right-0 h-36 bg-white z-20 flex flex-col p-2 border-b border-gray-200">
                        <div class="flex justify-between items-center mb-1 pb-1 border-b border-gray-200">
                            ...
                            <i class="fa-solid fa-xmark text-dim hover:text-gray-800 cursor-pointer" onclick="document.getElementById('rc-ai-io').classList.add('hidden')"></i>
                        </div>
                        <div class="flex-1 flex gap-2 overflow-hidden">
                            <textarea id="rc-io-in" class="flex-1 bg-gray-100 border border-gray-200 rounded p-1 text-[9px] text-gray-400 resize-none font-mono" readonly></textarea>
                            <textarea id="rc-io-out" class="flex-1 bg-gray-100 border border-gray-200 rounded p-1 text-[9px] text-green-400 resize-none font-mono" readonly></textarea>
                        </div>
                    </div>
                    <div class="flex-1 overflow-y-auto p-3 space-y-2" id="rc-ai-log">
                        <div class="p-2 bg-gradient-to-br from-amber-900/20 to-black border border-amber-500/20 rounded-lg text-xs text-gray-600">
                    ...
                    <div class="p-2 border-t border-gray-200 bg-[#F1F3F5] flex flex-col gap-1.5 relative">
                        <button class="absolute top-[-16px] right-3 bg-[#F1F3F5] border border-gray-200 rounded-t px-2 text-[8px] text-dim hover:text-gray-800" onclick="document.getElementById('rc-ai-io').classList.toggle('hidden')">IO</button>
                        <div class="grid grid-cols-3 gap-1">
                            ${[['summary','摘要','fa-file-lines','blue'],['char','人物','fa-users','green'],['plot','情节','fa-timeline','purple'],['deep','解读','fa-brain','red'],['imitate','仿写','fa-feather-pointed','pink'],['logic','纠错','fa-check-double','cyan'],['extract','素材提取','fa-gem','amber'],['inspire','灵感联想','fa-lightbulb','yellow'],['hotspot','热点关联','fa-fire','rose']].map(([k,v,i,c]) => `
                                <button class="btn btn-xs bg-gray-100 hover:bg-gray-200 text-gray-600 border border-gray-200 justify-start text-[9px] relative group" onclick="Modules.reader_center.analyze('${k}')">
                                    <i class="fa-solid ${i} text-${c}-400 mr-1"></i>${v}
                                    <i class="fa-solid fa-gear absolute right-1 opacity-0 group-hover:opacity-100 text-dim hover:text-gray-800 text-[8px]" onclick="event.stopPropagation();Modules.short.openPromptModal('read_${k}')"></i>
                                </button>
                            `).join('')}
                        </div>
                    </div>
                ...
                <!-- 对话 -->
                <div id="rc-ai-chat" class="hidden flex-1 flex flex-col overflow-hidden bg-[#F1F3F5]">
                    ...
                    <div class="p-2 bg-white border-t border-gray-200">
                        <div class="relative">
                            <textarea id="rc-chat-in" class="w-full bg-gray-100 border border-gray-300 rounded-lg p-2 pr-9 text-xs resize-none h-16 focus:border-amber-500/50 placeholder-white/20 leading-relaxed" placeholder="输入问题..." onkeydown="if(event.key==='Enter'&&!event.shiftKey){event.preventDefault();Modules.reader_center.sendChat();}"></textarea>
                ...
                <!-- 笔记 -->
                <div id="rc-ai-notes" class="hidden flex-1 flex flex-col overflow-hidden bg-[#F1F3F5]">
                    <div class="flex-1 overflow-y-auto p-3 space-y-2" id="rc-notes-list">
                        <div class="text-dim text-xs text-center p-4">暂无书签和批注</div>
                    </div>
                    <div class="p-2 border-t border-gray-200 flex gap-1">
                        ...
                        <button class="btn btn-xs flex-1 bg-gray-100 text-dim" onclick="Modules.reader_center.clearNotes()"><i class="fa-solid fa-trash mr-1"></i>清空</button>
                    </div>
                </div>
            </div>`;
    },
    ...
    switchAITab: (tab) => {
        ['assistant', 'chat', 'notes'].forEach(t => {
            ...
            if (btn) btn.className = t === tab ? 'flex-1 py-2 text-[10px] font-bold text-amber-500 border-b-2 border-amber-500' : 'flex-1 py-2 text-[10px] font-bold text-dim hover:text-gray-800';
        });
    ...
    init: async () => {
        ...
        if (RC.currentTab === 'library') {
            ...
            shelf.innerHTML = books.length === 0
                ? `<div class="col-span-full flex flex-col items-center justify-center text-dim h-48 border-2 border-dashed border-gray-200 rounded-xl">
                    <i class="fa-solid fa-book text-3xl mb-3 opacity-20"></i><p class="text-sm">书架空空如也</p>
                    <button class="btn mt-3 bg-gray-100 hover:bg-gray-200 text-xs" onclick="document.getElementById('rc-upload').click()">导入第一本书</button>
                   </div>`
                : books.map((b, i) => {
                    ...
                    <div class="group cursor-pointer relative" onclick="Modules.reader_center.read('${b.id}')" data-name="${(b.name||'').replace(/"/g,'&quot;')}">
                        <div class="aspect-[2/3] rounded-r-lg rounded-l-sm bg-gradient-to-br ${colors[i % colors.length]} shadow-xl group-hover:-translate-y-2 group-hover:shadow-[0_12px_25px_rgba(0,0,0,0.5)] transition-all duration-300 flex flex-col relative overflow-hidden border-l-2 border-gray-300">
                            <div class="absolute left-0 top-0 bottom-0 w-0.5 bg-gray-300 z-20"></div>
                            <div class="flex-1 flex flex-col p-2.5 relative z-10">
                                <i class="fa-solid fa-book-open text-lg text-gray-800/20 mb-1"></i>
                                <div class="text-[9px] font-bold text-gray-800/90 leading-tight line-clamp-2 mb-1">${b.name}</div>
                                <div class="text-[8px] text-gray-800/50 font-serif line-clamp-2">${(b.content || '').slice(0, 25)}...</div>
                            </div>
                            <div class="p-1.5 bg-gray-100 flex justify-between items-center mt-auto">
                                <span class="text-[7px] text-gray-800/60">${((b.size || 0) / 1024).toFixed(1)}KB</span>
                                ${progress > 0 ? `<span class="text-[7px] text-amber-400">${progress}%</span>` : ''}
                            </div>
                            ${progress > 0 ? `<div class="absolute bottom-0 left-0 right-0 h-0.5 bg-gray-200"><div class="h-full bg-amber-500" style="width:${progress}%"></div></div>` : ''}
                            <button class="absolute top-1 right-1 w-5 h-5 bg-red-500/80 text-gray-800 rounded-full opacity-0 group-hover:opacity-100 flex center text-[9px] z-30 hover:bg-red-500" onclick="Modules.reader_center.delBook('${b.id}',event)"><i class="fa-solid fa-trash"></i></button>
                        </div>
                    </div>`;
                }).join('');
        }
    ...
    renderNotes: () => {
        ...
        if (bms.length > 0) {
            ...
            html += bms.map(b => `
                <div class="p-2 bg-gray-100 rounded border border-gray-200 text-[10px] flex justify-between items-center group cursor-pointer hover:bg-gray-200" onclick="Modules.reader_center._jumpTo('${b.bookId}',${b.position})">
        ...
        if (ans.length > 0) {
            ...
            html += ans.map(a => `
                <div class="p-2 bg-gray-100 rounded border border-gray-200 text-[10px] group hover:bg-gray-200">
    ...
    analyze: async (type) => {
        ...
        log.innerHTML += `<div class="p-2 bg-gray-100 rounded text-[10px] text-dim"><i class="fa-solid fa-spinner fa-spin mr-1 text-amber-500"></i>正在${typeNames[type] || '分析'}...</div>`;
        ...
        try {
            ...
            log.innerHTML += `
                <div class="p-2 bg-gradient-to-br from-amber-900/10 to-transparent border border-amber-500/10 rounded-lg text-[10px] text-gray-600 leading-relaxed">
                    <div class="flex justify-between items-center mb-1">
                        <span class="text-amber-500 font-bold text-[9px]"><i class="fa-solid fa-sparkles mr-1"></i>${typeNames[type] || '分析结果'}</span>
                        <div class="flex gap-1">
                            <button class="text-dim hover:text-gray-800" onclick="Utils.copy(this.closest('.p-2').querySelector('.ai-result-text').innerText);UI.toast('已复制')"><i class="fa-solid fa-copy"></i></button>
    ...
    _runAITool: async (toolId) => {
        ...
        const log = document.getElementById('rc-ai-log');
        log.innerHTML += `<div class="p-2 bg-gray-100 rounded text-[10px] text-dim"><i class="fa-solid fa-spinner fa-spin mr-1"></i>${tool.name}...</div>`;
        try {
            const result = await AI.generate(prompt);
            log.innerHTML += `<div class="p-2 bg-gray-100 rounded text-[10px] text-gray-600"><div class="text-amber-500 font-bold text-[9px] mb-1">${tool.name}</div><div class="whitespace-pre-wrap">${result}</div></div>`;
        } catch (e) {
    ...
    sendChat: async () => {
        ...
        log.innerHTML += `<div class="flex justify-end"><div class="bg-amber-600/20 border border-amber-500/20 rounded-lg px-3 py-1.5 text-xs text-gray-800 max-w-[80%]">${msg}</div></div>`;
        log.innerHTML += `<div class="flex" id="rc-chat-typing"><div class="bg-gray-100 rounded-lg px-3 py-1.5 text-xs text-dim"><i class="fa-solid fa-spinner fa-spin mr-1"></i>思考中...</div></div>`;
        log.scrollTop = log.scrollHeight;
        ...
        try {
            ...
            log.innerHTML += `<div class="flex"><div class="bg-gray-100 border border-gray-200 rounded-lg px-3 py-1.5 text-xs text-gray-600 max-w-[85%] leading-relaxed whitespace-pre-wrap">${(typeof marked !== 'undefined' ? marked.parse(result) : result)}</div></div>`;
            log.scrollTop = log.scrollHeight;
        } catch (e) {
            const typing = document.getElementById('rc-chat-typing');
            if (typing) typing.remove();
    ...
    renderChapterNav: () => {
        ...
        return `
            <div class="absolute left-0 top-0 bottom-0 w-48 bg-white border-r border-gray-200 overflow-y-auto z-10" id="rc-chapter-nav">
                ...
                <div class="p-1 space-y-0.5">
                    ${RC.chapters.map((ch, i) => `
                        <button class="w-full text-left px-2 py-1.5 rounded text-[9px] transition-all ${RC.currentChapter === i ? 'bg-amber-500/20 text-amber-400 font-bold' : 'text-dim hover:bg-gray-100 hover:text-gray-800'}" onclick="Modules.reader_center.jumpToChapter(${i})">
        ...
        return `
            <div class="p-3 bg-gray-100 rounded-lg border border-gray-200">
                ...
                <div class="grid grid-cols-2 gap-2 text-[9px]">
                    <div class="flex justify-between"><span class="text-dim">总字数</span><span class="text-gray-800 font-mono">${s.totalWords?.toLocaleString() || 0}</span></div>
                    <div class="flex justify-between"><span class="text-dim">章节数</span><span class="text-gray-800 font-mono">${s.totalChapters || 0}</span></div>
                    <div class="flex justify-between"><span class="text-dim">平均章节</span><span class="text-gray-800 font-mono">${s.avgChapterLen?.toLocaleString() || 0}字</span></div>
                    <div class="flex justify-between"><span class="text-dim">预计阅读</span><span class="text-gray-800 font-mono">${s.readingTime || 0}分钟</span></div>
                    <div class="flex justify-between"><span class="text-dim">段落数</span><span class="text-gray-800 font-mono">${s.paragraphs || 0}</span></div>
                    <div class="flex justify-between"><span class="text-dim">对话数</span><span class="text-gray-800 font-mono">${Math.round(s.dialogues || 0)}</span></div>
                    <div class="flex justify-between"><span class="text-dim">提取实体</span><span class="text-amber-400 font-mono">${s.entities || 0}</span></div>
                    <div class="flex justify-between"><span class="text-dim">句数</span><span class="text-gray-800 font-mono">${s.sentences || 0}</span></div>
                </div>
            </div>`;
    },
...
简洁输出，每项2-3条。`;
        ...
        const log = document.getElementById('rc-ai-log');
        if (log) {
            log.innerHTML += `<div class="p-2 bg-gray-100 rounded text-[10px] text-dim"><i class="fa-solid fa-spinner fa-spin mr-1 text-amber-500"></i>智能分析中...</div>`;
        }
        try {
            const result = await AI.generate(prompt);
            if (log) {
                log.innerHTML += `
                    <div class="p-2 bg-gradient-to-br from-purple-900/20 to-transparent border border-purple-500/20 rounded-lg text-[10px] text-gray-600 leading-relaxed">
                        <div class="flex justify-between items-center mb-1">
                            <span class="text-purple-400 font-bold text-[9px]"><i class="fa-solid fa-lightbulb mr-1"></i>创作建议</span>
                            <button class="text-dim hover:text-gray-800" onclick="Utils.copy(this.closest('.p-2').querySelector('.ai-suggest-text').innerText);UI.toast('已复制')"><i class="fa-solid fa-copy"></i></button>
                        </div>
    ...
    extractToCreation: async (type) => {
        ...
        const log = document.getElementById('rc-ai-log');
        if (log) {
            log.innerHTML += `<div class="p-2 bg-gray-100 rounded text-[10px] text-dim"><i class="fa-solid fa-spinner fa-spin mr-1 text-amber-500"></i>提取中...</div>`;
        }
        try {
            const result = await AI.generate(prompt);
            if (Modules.phoenix) {
            ...
            if (log) {
                log.innerHTML += `
                    <div class="p-2 bg-gradient-to-br from-green-900/20 to-transparent border border-green-500/20 rounded-lg text-[10px] text-gray-600 leading-relaxed">
                        <div class="flex justify-between items-center mb-1">
                            <span class="text-green-400 font-bold text-[9px]"><i class="fa-solid fa-file-export mr-1"></i>提取结果</span>
                            <div class="flex gap-1">
                                <button class="text-dim hover:text-gray-800" onclick="Utils.copy(this.closest('.p-2').querySelector('.ai-extract-text').innerText);UI.toast('已复制')"><i class="fa-solid fa-copy"></i></button>
    ...
    quickExplain: async () => {
        ...
        const log = document.getElementById('rc-ai-log');
        if (log) {
            log.innerHTML += `<div class="p-2 bg-gray-100 rounded text-[10px] text-dim"><i class="fa-solid fa-spinner fa-spin mr-1 text-amber-500"></i>解释中...</div>`;
        }
        try {
            const result = await AI.generate(prompt);
            if (log) {
                log.innerHTML += `
                    <div class="p-2 bg-gray-100 border border-gray-300 rounded-lg text-[10px] text-gray-600 leading-relaxed">
    ...
    quickContinue: async () => {
        ...
        const log = document.getElementById('rc-ai-log');
        if (log) {
            log.innerHTML += `<div class="p-2 bg-gray-100 rounded text-[10px] text-dim"><i class="fa-solid fa-spinner fa-spin mr-1 text-amber-500"></i>续写中...</div>`;
        }
        try {
            const result = await AI.generate(prompt);
            if (log) {
                log.innerHTML += `
                    <div class="p-2 bg-gradient-to-br from-cyan-900/20 to-transparent border border-cyan-500/20 rounded-lg text-[10px] text-gray-600 leading-relaxed">
                        <div class="flex justify-between items-center mb-1">
                            <span class="text-cyan-400 font-bold text-[9px]"><i class="fa-solid fa-pen-fancy mr-1"></i>续写结果</span>
                            <div class="flex gap-1">
                                <button class="text-dim hover:text-gray-800" onclick="Utils.copy(this.closest('.p-2').querySelector('.ai-continue-text').innerText);UI.toast('已复制')"><i class="fa-solid fa-copy"></i></button>
...
Modules.web_chat = {
    ...
    render: () => {
        ...
        return `
        <div class="flex h-full bg-white text-[#e4e4e7] overflow-hidden" id="wc-root">
            <!-- 左侧会话列表 -->
            <div class="w-64 shrink-0 bg-white border-r border-gray-200 flex flex-col">
                <div class="p-3 border-b border-gray-200 bg-gradient-to-r from-blue-900/20 to-transparent">
                    <div class="flex items-center gap-2 mb-2">
                        ...
                        <div>
                            <h2 class="text-sm font-bold text-gray-800">网页对话</h2>
                            <p class="text-[8px] text-dim">多角色 · 多会话 · RAG增强</p>
                        </div>
                    </div>
                    <div class="flex gap-1">
                        <button class="btn flex-1 h-10 rounded-lg bg-gradient-to-r from-blue-500 to-indigo-500 hover:from-blue-400 hover:to-indigo-400 text-white border-none shadow-lg shadow-blue-500/30 hover:shadow-blue-500/50 transition-all hover:scale-105 font-bold text-sm flex center gap-2" onclick="Modules.web_chat.newSession()">
                            <i class="fa-solid fa-plus"></i>
                            <span>新对话</span>
                        </button>
                        <button class="btn h-8 w-8 rounded bg-red-600/20 text-red-400 border border-red-600/40 hover:bg-red-600 hover:text-gray-800 text-xs" onclick="Modules.web_chat.delCurrentSession()" title="删除当前对话 (Ctrl+D)">
                            <i class="fa-solid fa-trash"></i>
                        </button>
                        <button class="btn h-8 w-8 rounded bg-yellow-600/20 text-yellow-400 border border-yellow-600/40 hover:bg-yellow-600 hover:text-gray-800 text-xs" onclick="Modules.web_chat.clearSession()" title="清空当前对话内容 (Ctrl+L)">
                ...
                <div class="p-2 border-b border-gray-200">
                    <div class="text-xs font-bold text-gray-700 flex items-center gap-2 px-1 mb-2">
                            <i class="fa-solid fa-masks-theater text-indigo-400"></i>
                            AI 角色
                        </div>
                    <div class="flex flex-wrap gap-1">
                        ${Object.entries(WC.personas).map(([k, v]) => `
                            <button class="px-3 py-2 rounded-lg text-xs flex items-center gap-2 transition-all border-2 border-transparent ${WC.currentPersona === k ? `bg-gradient-to-r from-${v.color}-500 to-${v.color}-600 text-white border-${v.color}-400 shadow-lg shadow-${v.color}-500/30` : 'bg-white text-gray-700 hover:bg-gray-100 hover:border-gray-300'}" onclick="Modules.web_chat.setPersona('${k}')" title="${v.desc}">
                ...
                <div class="p-2 border-t border-gray-200 space-y-1">
                    <div class="flex items-center justify-between px-1">
                        <label class="flex items-center gap-1.5 cursor-pointer">
                            ...
                            <span class="text-xs font-bold text-gray-700">RAG 上下文增强</span>
                        </label>
                    </div>
                    <button class="btn w-full py-2.5 bg-gradient-to-r from-gray-500 to-gray-600 hover:from-gray-400 hover:to-gray-500 text-white border-none shadow-lg shadow-gray-500/30 hover:shadow-gray-500/50 transition-all hover:scale-105 rounded-lg" onclick="Modules.web_chat.exportAll()">
                            <i class="fa-solid fa-download mr-2"></i>
                            <span class="text-xs font-bold">导出全部对话</span>
                        </button>
                </div>
            </div>
            <!-- 右侧对话区 -->
            <div class="flex-1 flex flex-col relative">
                <!-- 顶栏 -->
                <div class="h-10 flex items-center justify-between px-4 bg-gray-200 backdrop-blur border-b border-gray-200 shrink-0">
                    <div class="flex items-center gap-2">
                        <i class="fa-solid ${p.icon} text-${p.color}-400"></i>
                        <span class="text-sm font-bold text-gray-800" id="wc-session-title">${WC.currentSession?.title || '新对话'}</span>
                        <span class="text-xs font-bold bg-gradient-to-r from-indigo-100 to-purple-100 text-indigo-700 px-2 py-1 rounded-lg border-2 border-indigo-200">${p.name}</span>
                    </div>
                    <div class="flex gap-1">
                        <button class="w-7 h-7 rounded hover:bg-gray-200 flex center text-dim text-xs" onclick="Modules.web_chat.toggleIO()" title="IO调试"><i class="fa-solid fa-terminal"></i></button>
                        <button class="w-7 h-7 rounded hover:bg-gray-200 flex center text-dim text-xs" onclick="Modules.web_chat.exportSession()" title="导出"><i class="fa-solid fa-file-export"></i></button>
                        <button class="w-7 h-7 rounded hover:bg-gray-200 flex center text-dim text-xs" onclick="Modules.web_chat.clearSession()" title="清空"><i class="fa-solid fa-broom"></i></button>
                    </div>
                </div>
                <!-- IO 调试面板 -->
                <div id="wc-io-panel" class="hidden bg-white border-b border-gray-200 h-32 flex gap-2 p-2 shrink-0">
                    <div class="flex-1 flex flex-col">
                        ...
                        <textarea id="wc-io-in" class="flex-1 bg-white border-2 border-gray-300 rounded-lg p-2 text-xs text-gray-700 font-mono resize-none focus:border-blue-400 focus:outline-none focus:ring-2 focus:ring-blue-200" readonly></textarea>
                    </div>
                    <div class="flex-1 flex flex-col">
                        ...
                        <textarea id="wc-io-out" class="flex-1 bg-white border-2 border-gray-300 rounded-lg p-2 text-xs text-green-700 font-mono resize-none focus:border-green-400 focus:outline-none focus:ring-2 focus:ring-green-200" readonly></textarea>
                    </div>
                </div>
                <!-- 消息区 -->
                ...
                <!-- 快捷指令 -->
                <div id="wc-shortcuts" class="hidden absolute bottom-[88px] left-4 right-4 bg-white border border-gray-300 rounded-lg shadow-xl p-2 z-30">
                    ${WC.shortcuts.map((s, i) => `
                        <button class="w-full text-left px-3 py-1.5 rounded text-xs hover:bg-gray-200 text-gray-600" onclick="Modules.web_chat.useShortcut(${i})">${s.cmd} <span class="text-dim ml-2">${s.prompt.slice(0, 20)}...</span></button>
                    `).join('')}
                </div>
                <!-- 输入区 -->
                <div class="p-3 bg-white border-t border-gray-200 shrink-0">
                    <div class="space-y-1.5 mb-2" id="wc-quick-guide">
                        <div class="flex gap-1">
                            <button class="flex-1 px-3 py-2 rounded-lg text-xs font-bold bg-gradient-to-r from-indigo-50 to-purple-50 text-indigo-700 border-2 border-indigo-200 hover:border-indigo-400 hover:shadow-lg transition-all text-left" onclick="document.getElementById('wc-input').value='/workflow ';document.getElementById('wc-input').focus()" title="调用已保存的工作流"><i class="fa-solid fa-diagram-project mr-1.5"></i>/workflow 工作流</button>
                            <button class="flex-1 px-3 py-2 rounded-lg text-xs font-bold bg-gradient-to-r from-blue-50 to-cyan-50 text-blue-700 border-2 border-blue-200 hover:border-blue-400 hover:shadow-lg transition-all text-left" onclick="document.getElementById('wc-input').value='/agent ';document.getElementById('wc-input').focus()" title="调用已部署的智能体"><i class="fa-solid fa-robot mr-1.5"></i>/agent 智能体</button>
                            <button class="flex-1 px-3 py-2 rounded-lg text-xs font-bold bg-gradient-to-r from-emerald-50 to-green-50 text-emerald-700 border-2 border-emerald-200 hover:border-emerald-400 hover:shadow-lg transition-all text-left" onclick="document.getElementById('wc-input').value='/rag ';document.getElementById('wc-input').focus()" title="RAG 上下文检索"><i class="fa-solid fa-magnifying-glass mr-1.5"></i>/rag 检索</button>
                        </div>
                        <div class="flex gap-1">
                            <button class="flex-1 px-3 py-2 rounded-lg text-xs font-bold bg-white text-gray-700 border-2 border-gray-200 hover:border-amber-400 hover:bg-amber-50 hover:text-amber-600 transition-all" onclick="document.getElementById('wc-input').value='/续写 ';document.getElementById('wc-input').focus()"><i class="fa-solid fa-pen mr-1.5"></i>续写</button>
                            <button class="flex-1 px-3 py-2 rounded-lg text-xs font-bold bg-white text-gray-700 border-2 border-gray-200 hover:border-pink-400 hover:bg-pink-50 hover:text-pink-600 transition-all" onclick="document.getElementById('wc-input').value='/润色 ';document.getElementById('wc-input').focus()"><i class="fa-solid fa-wand-magic-sparkles mr-1.5"></i>润色</button>
                            <button class="flex-1 px-3 py-2 rounded-lg text-xs font-bold bg-white text-gray-700 border-2 border-gray-200 hover:border-purple-400 hover:bg-purple-50 hover:text-purple-600 transition-all" onclick="document.getElementById('wc-input').value='/扩写 ';document.getElementById('wc-input').focus()"><i class="fa-solid fa-expand mr-1.5"></i>扩写</button>
                            <button class="flex-1 px-3 py-2 rounded-lg text-xs font-bold bg-white text-gray-700 border-2 border-gray-200 hover:border-cyan-400 hover:bg-cyan-50 hover:text-cyan-600 transition-all" onclick="document.getElementById('wc-input').value='/翻译 ';document.getElementById('wc-input').focus()"><i class="fa-solid fa-language mr-1.5"></i>翻译</button>
                        </div>
                        <div id="wc-custom-workflows" class="flex gap-1 overflow-x-auto"></div>
                    </div>
                    <div class="relative">
                        <textarea id="wc-input" class="w-full bg-gray-100 border border-gray-300 rounded-xl p-3 pr-12 text-sm resize-none h-20 focus:border-blue-500/50 placeholder-white/20 leading-relaxed text-gray-800" placeholder="输入消息... (输入 / 查看快捷指令)" oninput="Modules.web_chat.onInput(this)" onkeydown="if(event.key==='Enter'&&!event.shiftKey){event.preventDefault();Modules.web_chat.send();}"></textarea>
                        <button class="absolute bottom-3 right-3 w-8 h-8 bg-blue-600 hover:bg-blue-500 text-gray-800 rounded-lg flex center shadow-lg transition-all" id="wc-send-btn" onclick="Modules.web_chat.send()"><i class="fa-solid fa-paper-plane text-xs"></i></button>
                        <div id="wc-gen-status" class="hidden absolute bottom-3 left-3 flex items-center gap-2 bg-blue-600/15 border border-blue-500/25 rounded-lg px-3 py-1.5">
                            ...
                            <span class="text-xs font-bold text-blue-300" id="wc-gen-label">思考中...</span>
                            <span class="text-xs font-bold text-dim font-mono" id="wc-gen-chars">0 字</span>
                        </div>
                    </div>
                </div>
            </div>
    ...
    init: async () => {
        ...
        if (WC.sessions.length === 0) {
            ...
            list.innerHTML = WC.sessions.map(s => `
                <div class="flex items-center gap-2 px-2 py-1.5 rounded cursor-pointer group transition-all ${WC.currentSession?.id === s.id ? 'bg-blue-500/10 border border-blue-500/20' : 'hover:bg-gray-100 border border-transparent'}" onclick="Modules.web_chat.loadSession('${s.id}')">
                    <i class="fa-solid fa-message text-[9px] ${WC.currentSession?.id === s.id ? 'text-blue-400' : 'text-dim'}"></i>
                    <span class="flex-1 text-[10px] truncate ${WC.currentSession?.id === s.id ? 'text-gray-800 font-bold' : 'text-dim'}">${s.title || '未命名'}</span>
                    <span class="text-[8px] text-dim">${(s.messages || []).length}</span>
                    <button class="opacity-0 group-hover:opacity-100 text-red-400 hover:text-red-300 text-xs" onclick="event.stopPropagation();Modules.web_chat.delSession('${s.id}')"><i class="fa-solid fa-xmark"></i></button>
                </div>
            `).join('');
        }
        // 恢复当前会话消息
        ...
        if (cwEl) {
            ...
            try {
                ...
                if (TC) {
                    ...
                    if (wfs.length > 0) {
                        btns += wfs.map(w => `<button class="shrink-0 px-3 py-2 rounded-lg text-xs font-bold bg-indigo-50 text-indigo-700 border-2 border-indigo-200 hover:border-indigo-400 hover:shadow-lg transition-all whitespace-nowrap" onclick="document.getElementById('wc-input').value='/workflow ${w.name} ';document.getElementById('wc-input').focus()" title="工作流：${w.name}"><i class="fa-solid fa-diagram-project mr-1.5 text-indigo-400"></i>${w.name}</button>`).join('');
                    }
                    if (agents.length > 0) {
                        btns += agents.map(a => `<button class="shrink-0 px-3 py-2 rounded-lg text-xs font-bold bg-blue-50 text-blue-700 border-2 border-blue-200 hover:border-blue-400 hover:shadow-lg transition-all whitespace-nowrap" onclick="document.getElementById('wc-input').value='/agent ${a.name} ';document.getElementById('wc-input').focus()" title="智能体：${a.name}"><i class="fa-solid fa-robot mr-1.5 text-blue-400"></i>${a.name}</button>`).join('');
                    }
                }
            } catch(e) {}
    ...
    send: async () => {
            ...
            try {
                ...
                if (msg.startsWith('/workflow ')) {
                    ...
                    if (!wf) {
                        ...
                        msgsEl.innerHTML += `<div class="flex gap-3" id="${streamId}"><div class="w-7 h-7 rounded-full bg-${persona.color}-500/20 flex center shrink-0 border border-${persona.color}-500/30"><i class="fa-solid ${persona.icon} text-${persona.color}-400 text-[10px]"></i></div><div class="flex-1 max-w-[85%]"><div class="bg-gray-100 border border-gray-200 rounded-xl px-3 py-2 text-xs text-gray-600 leading-relaxed" id="${streamId}-body"><i class="fa-solid fa-spinner fa-spin"></i> 工作流执行中: ${wf.name}...</div></div></div>`;
        ...
        try {
            ...
            // 替换打字动画为流式容器
            if (bodyEl) bodyEl.innerHTML = `<div class="w-7 h-7 rounded-full bg-${persona.color}-500/20 flex center shrink-0 border border-${persona.color}-500/30"><i class="fa-solid ${persona.icon} text-${persona.color}-400 text-[10px]"></i></div><div class="flex-1 max-w-[85%]"><div class="bg-gray-100 border border-gray-200 rounded-xl px-3 py-2 text-xs text-gray-600 leading-relaxed" id="${streamBodyId}"><i class="fa-solid fa-ellipsis fa-beat-fade"></i> 思考中...</div></div>`;
    ...
    _renderMessages: () => {
        ...
        msgsEl.innerHTML = msgs.map((m, i) => {
            if (m.role === 'user') {
                return `<div class="flex gap-3 justify-end"><div class="bg-blue-600/20 border border-blue-500/20 rounded-xl px-3 py-2 text-xs text-gray-800 max-w-[75%] leading-relaxed whitespace-pre-wrap">${m.content}</div><div class="w-7 h-7 rounded-full bg-blue-600/30 flex center shrink-0"><i class="fa-solid fa-user text-blue-400 text-[10px]"></i></div></div>`;
            }
            ...
            return `
                <div class="flex gap-3">
                    ...
                    <div class="flex-1 max-w-[85%]">
                        <div class="bg-gray-100 border border-gray-200 rounded-xl px-3 py-2 text-xs text-gray-600 leading-relaxed">${rendered}</div>
                        <div class="flex gap-2 mt-1 ml-1">
                            <button class="text-xs text-dim hover:text-gray-800" onclick="Utils.copy(Modules.web_chat.currentSession.messages[${i}].content);UI.toast('已复制')"><i class="fa-solid fa-copy mr-0.5"></i>复制</button>
                            <button class="text-xs text-dim hover:text-amber-400" onclick="ContextHelper.exportToLibrary('对话_'+new Date().toLocaleString(),Modules.web_chat.currentSession.messages[${i}].content);UI.toast('已存入书架')"><i class="fa-solid fa-book mr-0.5"></i>存书架</button>
                        </div>
                    </div>
                </div>`;
        }).join('');
