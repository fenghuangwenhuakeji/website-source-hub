...
const APP_NAME = '凤煌管理后台';
const APP_ID = 'com.fenghuang.admin';
const DEFAULT_CLOUD_ORIGIN = 'https://fhwhkj.top';
const ROUTE_BASE = '/admin';
const DEFAULT_ENTRY_PATH = `${ROUTE_BASE}/login`;
...
function resolveCloudOrigin(...candidates) {
    for (const candidate of candidates) {
        ...
        try {
            ...
            // Ignore malformed overrides and keep searching.
        }
    }
...
const CLOUD_ORIGIN = resolveCloudOrigin(
    process.env.ADMIN_SERVER_ORIGIN,
...
function ensureFrontendBuilt() {
    ...
    if (!fs.existsSync(indexFile)) {
        throw new Error(`未找到后台本地前端产物：${indexFile}`);
    }
}

function normalizeAdminPath(requestPath) {
    const decodedPath = decodeURIComponent(requestPath || '/');

    if (decodedPath === ROUTE_BASE || decodedPath === `${ROUTE_BASE}/`) {
        return '/index.html';
    }

    if (decodedPath.startsWith(`${ROUTE_BASE}/`)) {
        const strippedPath = decodedPath.slice(ROUTE_BASE.length);
        return strippedPath || '/index.html';
    }
...
function resolveStaticFile(requestPath) {
    const normalizedPath = normalizeAdminPath(requestPath);
...
function createWindow() {
    mainWindow = new BrowserWindow({
        ...
        webPreferences: {
            ...
            partition: 'persist:fenghuang-admin',
        },
        frame: true,
        backgroundColor: '#050814',
        show: false,
    ...
    app.on('activate', () => {
        if (BrowserWindow.getAllWindows().length === 0) {
            if (localAppOrigin) {
                ...
                createFallbackWindow('后台本地资源未成功启动');
            }
        }
    });
});
