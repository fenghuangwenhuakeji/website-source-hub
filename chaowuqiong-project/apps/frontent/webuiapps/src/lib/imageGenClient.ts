...
export type ImageGenProvider = 'openai' | 'gemini' | 'custom';
...
export async function loadImageGenConfig(): Promise<ImageGenConfig | null> {
  ...
  try {
    ...
    if (persisted?.imageGen) {
      writeScopedStorageValue(CONFIG_KEY, JSON.stringify(persisted.imageGen));
...
export function saveImageGenConfig(config: ImageGenConfig): void {
  ...
  void persistImageGenConfig(config);
}

async function persistImageGenConfig(config: ImageGenConfig): Promise<void> {
  try {
    const persisted = await loadPersistedConfig();
