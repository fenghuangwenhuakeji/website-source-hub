...
let currentScopedStoreKey: string | null = null;

function loadSessionCache(): Map<string, StoredSession> {
  const scopedStoreKey = getUserScopedStorageKey(SESSION_STORE_KEY);
  if (sessionCache && currentScopedStoreKey === scopedStoreKey) return sessionCache;
  
  sessionCache = new Map();
  currentScopedStoreKey = scopedStoreKey;
  
  try {
    const stored = localStorage.getItem(scopedStoreKey);
    if (stored) {
...
function persistSessions(): void {
  ...
  try {
    ...
    localStorage.setItem(getUserScopedStorageKey(SESSION_STORE_KEY), JSON.stringify(sessions));
  } catch (e) {
...
export function clearAllSessions(): void {
  sessionCache = new Map();
  currentScopedStoreKey = getUserScopedStorageKey(SESSION_STORE_KEY);
  localStorage.removeItem(getUserScopedStorageKey(SESSION_STORE_KEY));
}
