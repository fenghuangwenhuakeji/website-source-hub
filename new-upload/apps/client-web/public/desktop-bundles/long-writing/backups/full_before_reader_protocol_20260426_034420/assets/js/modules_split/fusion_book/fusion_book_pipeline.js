Object.assign(Modules.fusion_book, {
    ...
    _updateConfigSummary() {
        ...
        if (el) el.textContent = cycleMode
            ...
            : `左书 ${leftCount} 章 × 右书 ${rightCount} 章 = 共 ${total} 轮流水线`;
    },
    ...
    async _runConfiguredPipeline(isResume = false) {
        ...
        if (infoEl) {
            ...
            infoEl.innerHTML = `左书: 《${leftBook.name}》 | 右书: 《${rightBook.name}》<br>` +
                ...
                `策略: ${cfg.cycleMode ? '循环融合 ' + (cfg.cycleSize || 5) + '章/组' : '逐章精修'} | Agent并发: ${maxConcurrency} | 启用: ${opts.join(' | ')}`;
        }

        this._plLog(`🚀 流水线${isResume ? '恢复' : '启动'}: ${pairs.length} 对章节 | ${cfg.cycleMode ? '循环融合' : '逐章精修'} | Agent并发:${maxConcurrency}`, 'ok');
