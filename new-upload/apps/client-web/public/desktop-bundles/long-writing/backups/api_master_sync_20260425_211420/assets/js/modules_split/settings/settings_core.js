...
Modules.settings = {
    ...
    _renderQuickStartTab() {
        const steps = [
            { id: 'text-api', icon: 'fa-plug-circle-bolt', title: '接入文本模型', desc: '写作、拆解、总结都靠它。', action: '去配置', tab: 'api_pool' },
    ...
    _renderModelTab() {
        return this._renderPageShell('模型/API', '普通用户只需要做一件事：添加一个文本模型并激活。图像、视频、音频是可选能力。', `
            <div class="bg-blue-500/10 border border-blue-500/20 rounded-lg p-4 flex flex-col md:flex-row md:items-center gap-3">
                ...
                <div class="flex-1">
                    ...
                    <div class="text-[11px] text-blue-100/70 mt-1">选择“文本模型” → 添加 API → 填名称、地址、密钥、模型名 → 激活 → 开始写作。</div>
                </div>
                <button class="btn btn-sm bg-blue-600 text-white rounded-lg" onclick="Modules.settings._apiPoolType='text';Modules.settings.refresh();setTimeout(()=>Modules.settings._openApiPoolModal(),80)">添加文本模型</button>
            </div>
    ...
    async _refreshSetupStatus() {
        ...
        const activeText = textApis.some(a => a.is_active === 1);
        set('text-api', activeText, activeText ? '已配置' : '还没配置');
