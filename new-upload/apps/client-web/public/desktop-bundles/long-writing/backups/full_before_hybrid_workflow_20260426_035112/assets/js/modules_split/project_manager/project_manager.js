...
Modules.project_manager = {
            ...
            <div class="shrink-0 border-b border-white/5 bg-[#0a0a0c]">
                <div class="h-14 px-5 flex items-center justify-between gap-4">
                    <div class="min-w-0">
                        ...
                        <div class="text-[10px] text-dim mt-1">先选中项目，再推进世界观、大纲、正文和记忆。</div>
                    </div>
    ...
    _renderQuickStart() {
        ...
        const sub = isEmpty ? '不用先懂内部模块，按你手里的材料选。' : '需要开新书时，从这里直接建。';
        el.innerHTML = `
            ...
            <div class="rounded-lg border border-amber-500/20 bg-amber-500/5 p-4 flex items-center justify-between">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <div class="text-[11px] text-dim mt-1">选中后，世界观、章节、RAG、记忆都会归到这本书下面。</div>
                    </div>
                </div>
    ...
    _showCreateModal(preselectedMode = 'phoenix') {
        ...
        const modes = [
            ['phoenix','fa-fire-flame-curved','我要从零写一本','题材 → 执行级细纲 → 图谱 → 正文','orange'],
            ['import','fa-file-import','我要接着已有作品写','原文保留 → 章内细纲 → 图谱 → 续写','cyan'],
            ['fusion','fa-code-compare','我要拆书融合出新书','双书 → 循环拆技法 → 新细纲 → 正文','purple']
        ];
        overlay.innerHTML = `
        <div class="w-full max-w-[560px] rounded-lg border border-white/10 bg-[#101014] shadow-2xl overflow-hidden">
            <div class="px-5 py-4 border-b border-white/5 flex items-center justify-between">
                <div>
                    ...
                    <div class="text-[10px] text-dim mt-1">先选目标，系统会自动把你送到对应入口。</div>
                </div>
    ...
    _pathProfile(mode) {
        const profiles = {
            phoenix: {
                headline: '从零写一本',
                plain: '没有原稿，从题材和执行级细纲开始。',
                icon: 'fa-fire-flame-curved',
                color: 'orange'
            },
            import: {
                headline: '导入续写',
                plain: '正文保留，拆章内细纲，提实体后续写。',
                icon: 'fa-file-import',
                color: 'cyan'
            },
            fusion: {
                headline: '拆书融合',
                plain: '两本参考书按循环拆技法，融合成新细纲。',
    ...
    _nextAction(p) {
        ...
        const map = {
            phoenix: [
                ...
                ['做执行级细纲', '把每章冲突、转折、钩子写到能直接开写。'],
                ['实体入图谱', '把人物、规则、伏笔、地点注入世界引擎。'],
                ['进入执笔台', '带着知识图谱和记忆上下文写正文。']
            ],
            import: [
                ['导入正文', '把已有小说放进系统，正文保留到执笔台。'],
                ['拆成细纲', '按卷章拆出已写内容和后续缺口。'],
                ['实体入图谱', '人物、世界规则、伏笔、关系进入世界引擎。'],
                ['补缺章/续写', '已有正文不动，只从缺口或末尾继续写。']
            ],
            fusion: [
                ['导入双书', '选择两本参考书，只作为技法来源。'],
                ['循环拆技法', '按5章/一卷提炼节奏、钩子、爽点和对话模板。'],
