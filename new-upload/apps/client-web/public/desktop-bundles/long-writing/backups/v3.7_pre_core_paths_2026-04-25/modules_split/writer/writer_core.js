...
Modules.writer = {
    ...
    async _loadProjectContext() {
        try {
            ...
            if (!proj) return;
            const modeLabels = { phoenix: '凤凰创作流', import: '导入作品', fusion: '拆书融合' };
    ...
    async _autoExtractOutline(content, chapterId) {
        ...
        const prompt = `请从以下小说正文中提取「写作执行级细纲」，每个段落提炼为一句话情节要点。
...
【提取要求】
1. 按段落顺序提取，每个段落对应一个情节点
2. 每行格式：[情绪X|钩子类型|功能定位] 情节要点（一句话）
3. 情绪分值1-10，钩子类型：悬念/爽点/转折/情感/信息差
4. 功能定位：铺垫/爆发/转折/收束/过渡
5. 最后一行标注「章末钩子」的具体内容
6. 只输出提取结果，不要分析
...
请直接输出提取的细纲。`;
    ...
    async _autoExtractEntities(content, chapterId) {
        ...
        const prompt = `你是一个专业的小说实体提取引擎。请从以下章节正文中提取所有有价值的实体。
...
只输出JSON数组，不要任何其他文字。`;
        ...
        try {
            ...
            for (const ent of entities) {
                ...
                let id, chapters = [], cycles = [], source = 'auto_extract';
                ...
                if (existing) {
                    ...
                    await DB.put('entities', {
                        ...
                        relations, chapters, cycles, source, nexusState: null, updatedAt: Date.now()
                    });
                    added++;
                }
