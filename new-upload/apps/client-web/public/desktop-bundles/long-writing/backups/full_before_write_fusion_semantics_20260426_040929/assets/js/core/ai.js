const AI = {
    ...
    async generate(prompt, config = {}, onChunk) {
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            ...
            try {
                const requestPrompt = AI.augmentCreativePrompt(prompt, config);
                const { url, headers, body } = AI.buildRequest(apiConfig, requestPrompt, true);
                console.log(`[AI] Request → ${apiConfig.provider} | model:${body.model||body.model_name||'-'} | max_tokens:${body.max_tokens||body.maxOutputTokens||'-'} | stream:${body.stream||body.streamGenerateContent||false} | prompt:${requestPrompt.length}字`);
    ...
    augmentCreativePrompt(prompt, config = {}) {
        ...
        const mod = (typeof App !== 'undefined' && App._currentModule) ? App._currentModule : '';
        const moduleMode = {
            phoenix: 'phoenix',
            world_engine: 'import',
            writer: null,
        ...
        if (!Object.prototype.hasOwnProperty.call(moduleMode, mod)) return prompt;
        ...
        return `${prompt}\n\n【内部读者引擎】\n以下内容只作为写作/拆解/续写/融合的隐性约束，不要在用户可见结果中解释、复述或单独列出。\n${ctx}\n\n${workflowCtx}\n\n【隐性执行要求】\n- 细纲里把读者期待、恐惧、信息缺口、回收点藏进章节动作和伏笔钩子。\n- 正文里只呈现身体动作、环境反馈、对话错位和未完成动作，不解释读者机制。\n- 拆书时只提炼触发读者停留/复购/痴迷的技法模板，不搬原文内容。\n- 续写时优先保护原读者已经投入的旧缺口、旧情绪、旧承诺。\n- 无论用户先写、先拆、导入续写还是拆书融合，都按当前材料补齐下一步，不强迫一次性拆完。`;
    },
