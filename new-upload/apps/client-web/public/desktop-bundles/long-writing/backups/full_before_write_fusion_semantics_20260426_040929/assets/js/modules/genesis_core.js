...
const GenesisCore = {
    ...
    MODE_CONFIG: {
        ...
        fusion: {
            ...
            stages: [
                ...
                { id: 'fusion_outline', label: '融合细纲', icon: 'fa-list-check' },
                { id: 'graph', label: '入图谱', icon: 'fa-diagram-project' },
            ...
            features: ['dual_deconstruct', 'cycle_pattern_extract', 'fusion_outline', 'graph_injection']
        }
    },
    ...
    WORKFLOW_PROTOCOL: {
        modes: {
            hybrid: '混合循环：用户可随时先写、先拆、续写、补拆；AI必须根据当前材料补齐缺口，不强迫一次性完成全流程。',
            write_then_split: '先写后拆：先保护作者手感，写完后反推章内细纲、实体、规则、伏笔和一致性风险。',
            outline_first: '先拆后写：先把章节拆到可执行细纲，再按细纲写正文；细纲不能停留在概念。',
            continue_existing: '导入续写：已有正文默认不重写，只从缺章、断点或末尾继续；旧人物状态和旧伏笔优先。',
            manual: '自由手写：AI只在被要求时补拆、续写、检查，不主动覆盖作者正文。',
            fusion: '融合写作：只借技法，不借原书内容；融合结果必须落到原创细纲和原创正文。'
        },
        invariants: [
            '正文、细纲、实体、图谱可以任意先后进入系统，但最终都要互相同步',
            '用户手写内容永远是源事实，AI只能拆解、补齐、续写或建议修改',
            '每次生成后都要能反哺章内细纲和世界引擎',
    ...
    buildWorkflowContext(mode, flowMode = 'hybrid') {
        ...
        return [
            ...
            '执行原则：能先写就先写，能先拆就先拆；只要材料进入系统，就自动补齐细纲、实体、图谱、上下文和一致性检查。'
        ].join('\n');
    },
