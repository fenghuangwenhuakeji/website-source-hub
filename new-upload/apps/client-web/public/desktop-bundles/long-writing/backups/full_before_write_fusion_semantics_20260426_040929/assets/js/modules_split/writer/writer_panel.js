Object.assign(Modules.writer, {
    ...
    async _aiRefineOutline() {
        ...
        UI.toast('AI 正在细化大纲...');
        const prompt = `你是一位资深网文策划师。请将以下章节大纲进一步细化为「写作执行级细纲」，每个情节点具体到可执行程度。
...
【细化要求】
1. 每个段落标注：情节要点 + 情绪分值(1-10) + 钩子类型(悬念/爽点/转折/情感/信息差)
2. 补充具体细节：场景描写要点、对话关键句、动作细节、感官描写方向
3. 明确每段的核心功能和节奏定位（铺垫/爆发/转折/收束）
4. 标注章末钩子的具体形式
5. 输出格式：每段以 [情绪X|钩子类型|功能定位] 开头，接情节要点和写作细节
...
请直接输出细化后的大纲。`;
        ...
        try {
            ...
            this._showOutlineConfirm(result, outlineEl ? outlineEl.value : '');
    ...
    async _aiExpandOutline() {
        ...
        UI.toast('AI 正在修改大纲...');
        const prompt = `你是一位资深网文策划师。用户提出了以下修改要求：
...
请严格按照用户要求修改大纲，保留未被要求改动的部分。直接输出修改后的完整大纲。`;
