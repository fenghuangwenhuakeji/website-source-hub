...
Modules.writer = {
            ...
            <div class="flex-1 flex flex-col min-w-0 relative">
                ...
                <div class="flex-1 flex flex-col min-h-0 relative">
                    ...
                    <div class="flex items-center gap-2 px-5 py-2 bg-[#0a0a0c] border-b border-white/5 shrink-0 flex-wrap">
                        ...
                        <select id="w-flow-mode" class="h-7 rounded-lg bg-black/30 border border-white/10 px-2 text-[10px] text-white outline-none" onchange="Modules.writer.setFlowMode(this.value)" title="写拆循环方式">
                            <option value="hybrid">边写边拆</option>
                            <option value="write_then_split">先写后拆</option>
                            <option value="outline_first">先拆后写</option>
                            <option value="continue_existing">导入续写</option>
                            <option value="fusion">融合写作</option>
                            <option value="manual">自由手写</option>
                        </select>
                        <button class="btn btn-xs bg-emerald-600/20 text-emerald-400 border-emerald-600/30" onclick="Modules.writer.save({forcePostProcess:true})" title="保存当前正文，并反推细纲、实体和世界图谱"><i class="fa-solid fa-diagram-project mr-1"></i>保存并拆</button>
    ...
    setFlowMode(mode) {
        ...
        const labels = {
            hybrid: '边写边拆',
            write_then_split: '先写后拆',
            outline_first: '先拆后写',
            continue_existing: '导入续写',
            fusion: '融合写作',
            manual: '自由手写'
        };
        UI.toast(`写拆模式：${labels[this.aiOpts.flowMode] || '边写边拆'}`);
    },
    ...
    _schedulePostWriteProcessing(chapterId, opts = {}) {
        ...
        this._postProcessTimers[chapterId] = setTimeout(() => {
            this._runPostWriteProcessing(chapterId, opts).catch(e => {
                console.warn('[Writer] 写拆后处理失败:', e);
                const st = document.getElementById('w-save-status');
                if (st) st.textContent = '已保存 · 后台拆解失败';
                if (typeof UI !== 'undefined') UI.toast('已保存，但后台拆解失败：' + (e.message || '未知错误'), 'error');
            });
        }, opts.delay || 300);
    },
    ...
    async save(opts = {}) {
        ...
        if (!opts.skipPostProcess && this._shouldAutoPostProcess(chap, opts)) {
            if (st) st.textContent = '已保存 · 后台拆解中';
...
6. 只输出细纲，不要解释。`;
        ...
        try {
            let outline = '';
            await AI.generate(prompt, { max_tokens: 2048, temperature: 0.5 }, c => { outline += c; });
...
只输出JSON数组，不要任何其他文字。`;
        ...
        try {
            let jsonStr = '';
            await AI.generate(prompt, { max_tokens: 2048, temperature: 0.3 }, c => { jsonStr += c; });
