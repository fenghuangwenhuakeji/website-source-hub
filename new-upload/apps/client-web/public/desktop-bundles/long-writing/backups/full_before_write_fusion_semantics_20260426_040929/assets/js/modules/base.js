...
Modules.home = {
    ...
    _renderWorkflowDeck() {
        const cards = [
            ['write_then_split','writer','fa-pen-nib','先写后拆','先把一段写出来，保存后系统反推细纲和实体。','amber'],
            ['outline_first','phoenix','fa-list-check','先拆后写','先做章节骨架，再把每章送进执笔台落正文。','cyan'],
            ['continue_existing','world_engine','fa-file-import','导入续写','把没写完的原稿丢进来，已写正文保留，缺的继续。','emerald'],
            ['hybrid','writer','fa-arrows-rotate','边写边拆','你写一部分，AI拆一部分；你拆一部分，AI写一部分。','purple']
        ];
        return `
        <div class="mt-3 rounded-lg border border-white/5 bg-black/18 p-3">
            <div class="flex items-center justify-between gap-3 mb-2">
                <div>
                    <div class="text-xs font-bold text-white">写拆循环</div>
                    <div class="text-[10px] text-dim mt-0.5">不用一次性拆完。你手里有什么，就从哪里开始。</div>
                </div>
    ...
    _actionsForProject(p) {
        ...
        return [['fusion_book','fa-code-compare','循环拆书','按循环/卷提取两本书技法'], ['fusion_workbench','fa-table-columns','查看成果','管理拆书输出'], ['writer','fa-feather-pointed','融合写作','把新细纲落到正文']];
    },
    ...
    _routeForProject(p) {
        ...
        return (p.stageIndex || 0) <= 3 ? { id: 'fusion_book', label: '循环拆书' } : { id: 'writer', label: '融合写作' };
    },
