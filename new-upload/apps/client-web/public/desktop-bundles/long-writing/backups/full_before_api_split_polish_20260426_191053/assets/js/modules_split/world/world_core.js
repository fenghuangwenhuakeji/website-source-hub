...
Modules.world_engine = {
    ...
    async _loadGraphSources() {
        ...
        const chapters = (chaptersRaw || []).sort((a,b) => (a.order || a.number || 0) - (b.order || b.number || 0));
        const volumes = (volumesRaw || []).sort((a,b) => (a.order || 0) - (b.order || 0));
        const cycles = (cyclesRaw || []).sort((a,b) => (a.startChapter || 0) - (b.startChapter || 0));
        const chapterMap = new Map(chapters.map(c => [c.id, c]));
        const cycleMap = new Map(cycles.map(c => [c.id, c]));
        return { volumes, chapters, cycles, chapterMap, cycleMap };
    },
    ...
    _inferEntityVolumeIds(entity, chapterMap, cycleMap) {
        const ids = new Set(entity.volumes || []);
        (entity.chapters || []).forEach(chId => {
            const chapter = chapterMap.get(chId);
            const volumeId = this._getChapterVolumeId(chapter);
            if(volumeId) ids.add(volumeId);
        });
        (entity.cycles || []).forEach(cycleId => {
            const cycle = cycleMap.get(cycleId);
            if(cycle?.volumeId) ids.add(cycle.volumeId);
    ...
    async _buildLayeredGraphIndexes() {
        ...
        const entities = (this._cachedEntities || []).filter(e => e && e.id && !e.id.startsWith('world_'));
        ...
        entities.forEach(entity => {
            const volumeIds = this._inferEntityVolumeIds(entity, sources.chapterMap, sources.cycleMap);
        ...
        const cycleGraphs = sources.cycles.map(cycle => {
            ...
            const entityList = entities.filter(e =>
                ...
                (e.chapters || []).some(chId => chapterIds.includes(chId)) ||
        ...
        <div class="flex h-full bg-[#08080a] overflow-hidden">
            <div class="w-64 shrink-0 flex flex-col bg-[#0e0e10] border-r border-white/5">
                ...
                <div class="p-3 border-t border-white/5 bg-[#0a0a0c] space-y-2">
                    <button class="btn btn-xs bg-amber-600/20 text-amber-400 border-amber-600/30 w-full font-bold" onclick="Modules.world_engine.exportAll()"><i class="fa-solid fa-download mr-1"></i>一键导出工程</button>
    ...
    _renderSubPanel: () => {
        ...
        if(t === 'dashboard') {
            return `<div class="p-2 space-y-2">
                ...
                <button class="btn btn-xs bg-gradient-to-r from-amber-600/20 to-orange-600/20 text-amber-400 border border-amber-500/30 w-full font-bold" onclick="Modules.world_engine._openNovelImportModal()"><i class="fa-solid fa-book mr-1"></i>导入续写</button>
        ...
        if(t === 'graph') {
            ...
            return `<div class="p-2 space-y-2">
                <button class="btn btn-xs bg-purple-600/20 text-purple-400 border-purple-600/30 w-full" onclick="Modules.world_engine.rebuildLayeredGraphs('manual_refresh')"><i class="fa-solid fa-layer-group mr-1"></i>重建分层图谱</button>
                ...
                <div class="border-t border-white/5 pt-2">
                    <div class="text-[9px] text-dim font-bold uppercase tracking-wider mb-1">图谱层级</div>
                    ...
                    <div class="text-[9px] text-gray-500 leading-relaxed mt-2">正文/细纲抽取实体后会进所属卷；拆书循环数据进循环图谱。避免整本书挤成一团。</div>
                </div>
                <div class="border-t border-white/5 pt-2">
        ...
        if(t === 'graph') return `
            <div class="h-11 flex items-center justify-between px-5 bg-[#0d0d0f] border-b border-white/5 shrink-0">
                <div class="flex items-center gap-3">
                    <span class="text-xs font-bold text-purple-400"><i class="fa-solid fa-circle-nodes mr-1"></i>分层知识图谱 · 卷/循环监控</span>
