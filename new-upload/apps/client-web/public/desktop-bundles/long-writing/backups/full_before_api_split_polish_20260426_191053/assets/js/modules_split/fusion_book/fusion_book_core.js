...
Modules.fusion_book = {
    ...
    _plConfig: {
        ...
        doOutline: false,   // 是否写细纲（默认只拿弹药）
        doWrite: false,     // 是否写正文（默认不越过执笔台）
        doRAG: true,        // 是否存RAG
        saveFolder: '',     // 本地保存文件夹
        lastSync: null,     // 最后同步时间
        directionLock: '',   // 当前新书方向/人物护栏
        modePreset: 'ammo',  // ammo / outline / full
        cycleMode: true,    // 默认按循环拆解，避免整本书一章章硬跑
        cycleSize: 5,       // 循环大小（默认5章为一个循环）
        maxConcurrency: 1   // 默认并发数（保守模式，大模型友好）
    },
    ...
    render() {
        ...
        <div class="flex flex-col h-full bg-[#0a0a0c] overflow-hidden relative">
                ...
                <div class="fb-main-panel flex-1 flex flex-col min-w-0 overflow-hidden bg-[#0a0a0c]">
                    <div class="p-3 border-b border-white/5 bg-[#0e0e10]">
                        <div class="flex items-center justify-center mb-2">
                            <button class="btn btn-md bg-gradient-to-r from-red-600 to-orange-500 text-white font-bold shadow-lg hover:scale-105 transition-transform px-6" onclick="Modules.fusion_book.showPipelineConfig()"><i class="fa-solid fa-box-open mr-2"></i>拆一轮拿弹药</button>
                        </div>
                        <div class="flex items-center justify-center gap-2 mb-2">
                            ...
                            <button class="btn btn-xs bg-indigo-600/20 text-indigo-400 border-indigo-600/30" onclick="Modules.fusion_book.sendToWriter()"><i class="fa-solid fa-pen-nib mr-1"></i>送弹药到执笔台</button>
                        ...
                        <div class="rounded-lg border border-white/5 bg-black/20 p-2 mb-2 space-y-2">
                            ...
                            <textarea id="fb-direction-lock" class="w-full h-16 bg-black/30 border border-white/10 rounded-lg px-2 py-1.5 text-[10px] text-gray-200 resize-none outline-none focus:border-amber-500/40" placeholder="写清当前新书方向、主角欲望、不能崩的人设、世界规则、下一步要推进什么。拆书只提供技法弹药，不能替你改方向。">${FB._escapeAttr ? FB._escapeAttr(FB._plConfig.directionLock || '') : (FB._plConfig.directionLock || '')}</textarea>
                            <div class="text-[9px] text-dim leading-relaxed">拆书只拿技法、节奏、钩子。人物、关系、世界规则和主线方向以这里与世界引擎为准。</div>
                        </div>
            ...
            <div id="fb-pipeline-overlay" class="fb-pipeline-overlay absolute inset-0 z-50 flex flex-col bg-[#0a0a0c] border border-white/5" style="display:none;">
                <div class="h-11 flex items-center justify-between px-4 bg-[#0e0e10] border-b border-white/5 shrink-0">
                    <div class="flex items-center gap-3">
                        <span class="text-base font-bold text-green-400"><i class="fa-solid fa-box-open mr-2"></i>拆书弹药 · 实时监控</span>
                ...
                <div class="px-4 py-1 bg-[#0e0e10] border-b border-white/5 shrink-0 flex items-center gap-2 text-[10px]">
                    ...
                    <span class="text-dim">→</span>
                    <span class="px-1.5 py-0.5 rounded bg-white/5 text-dim" id="pl-phase-4">④入库/落稿</span>
                </div>
            ...
            <div id="fb-pipeline-mini" class="absolute bottom-12 right-4 z-50 bg-gradient-to-r from-red-600 to-orange-500 rounded-full shadow-lg shadow-red-500/30 px-5 py-2.5 flex items-center gap-3 cursor-pointer hover:scale-105 transition-transform" onclick="Modules.fusion_book._pipelineRunning ? Modules.fusion_book.plRestore() : (Modules.fusion_book._savedPipelineState ? Modules.fusion_book._resumeFromSaved() : Modules.fusion_book.showPipelineConfig())">
                <span class="text-white text-sm font-bold"><i class="fa-solid fa-box-open mr-1.5"></i><span id="pl-mini-status">${FB._savedPipelineState ? '继续上次拆书 (' + (FB._savedPipelineState.completedPairs||[]).length + '章已完成)' : '拆书弹药库'}</span></span>
            ...
            <div id="fb-pipeline-config" class="absolute inset-0 z-[60] flex items-center justify-center bg-black/60 backdrop-blur-sm" style="display:none;" onclick="if(event.target===this)this.style.display='none'">
                <div class="fb-config-panel w-[720px] max-h-[85vh] bg-[#111113] border border-white/10 rounded-2xl shadow-2xl overflow-hidden flex flex-col" onclick="event.stopPropagation()">
                    <div class="px-5 py-3 bg-[#0e0e10] border-b border-white/5 flex items-center justify-between">
                        <span class="text-base font-bold text-green-400"><i class="fa-solid fa-box-open mr-2"></i>拆书弹药 · 配置</span>
                    ...
                    <div class="flex-1 overflow-y-auto p-5 space-y-4">
                        ...
                        <div class="bg-black/20 rounded-lg p-4 border border-white/5 space-y-3">
                            <div class="flex items-center justify-between gap-2 mb-2">
                                <div>
                                    <div class="text-xs font-bold text-white">执行档位</div>
                                    <div class="text-[9px] text-dim mt-0.5">默认只拿弹药，需要时再生成细纲或正文。</div>
                                </div>
                                <div class="flex gap-1">
                                    <button class="btn btn-xs bg-emerald-600/20 text-emerald-400 border-emerald-600/30 text-[9px]" onclick="Modules.fusion_book._setPipelinePreset('ammo')">只拿弹药</button>
                                    <button class="btn btn-xs bg-orange-600/20 text-orange-400 border-orange-600/30 text-[9px]" onclick="Modules.fusion_book._setPipelinePreset('outline')">生成细纲</button>
                                    <button class="btn btn-xs bg-purple-600/20 text-purple-400 border-purple-600/30 text-[9px]" onclick="Modules.fusion_book._setPipelinePreset('full')">拆完写书</button>
                                </div>
                            </div>
                            <label class="flex items-center gap-2 cursor-pointer"><input type="checkbox" id="plc-do-extract" ${FB._plConfig.doExtract ? 'checked' : ''} class="accent-green-500" onchange="Modules.fusion_book._updateConfigSummary()"><span class="text-xs text-gray-300">🌍 提取实体/规则/伏笔 → 世界引擎</span></label>
                            <label class="flex items-center gap-2 cursor-pointer"><input type="checkbox" id="plc-do-outline" ${FB._plConfig.doOutline ? 'checked' : ''} class="accent-green-500" onchange="Modules.fusion_book._updateConfigSummary()"><span class="text-xs text-gray-300">📋 生成细纲 → 执笔台大纲</span></label>
                            <label class="flex items-center gap-2 cursor-pointer"><input type="checkbox" id="plc-do-write" ${FB._plConfig.doWrite ? 'checked' : ''} class="accent-green-500" onchange="Modules.fusion_book._updateConfigSummary()"><span class="text-xs text-gray-300">✍️ 生成正文 → 执笔台正文</span></label>
                            <label class="flex items-center gap-2 cursor-pointer"><input type="checkbox" id="plc-do-rag" ${FB._plConfig.doRAG ? 'checked' : ''} class="accent-green-500" onchange="Modules.fusion_book._updateConfigSummary()"><span class="text-xs text-gray-300">🔍 拆书弹药存入RAG</span></label>
                        ...
                        <div class="bg-cyan-900/10 rounded-lg p-4 border border-cyan-500/20 space-y-3">
                            <div class="text-xs font-bold text-cyan-400 mb-2"><i class="fa-solid fa-sync mr-1"></i>智能循环融合（推荐）</div>
                            <label class="flex items-center gap-2 cursor-pointer"><input type="checkbox" id="plc-cycle-mode" ${FB._plConfig.cycleMode ? 'checked' : ''} class="accent-cyan-500" onchange="Modules.fusion_book._updateConfigSummary()"><span class="text-xs text-gray-300">按N章打包拆技法，再做循环总结；比逐章硬拆更稳。</span></label>
                            <div class="flex items-center gap-3">
                                <div class="flex items-center gap-2">
                        ...
                        <div class="bg-purple-900/10 rounded-lg p-4 border border-purple-500/20 space-y-3">
                            <div class="text-xs font-bold text-purple-400 mb-2"><i class="fa-solid fa-bolt mr-1"></i>稳定并发设置</div>
                            <div class="flex items-center gap-3">
                                <span class="text-[10px] text-dim">最大并发数:</span>
                                <input type="range" id="plc-concurrency" min="1" max="3" value="${Math.min(FB._plConfig.maxConcurrency || 1, 3)}" class="accent-purple-500 w-32" oninput="document.getElementById('plc-concurrency-val').textContent=this.value">
                                <span class="text-xs text-purple-400 font-bold font-mono" id="plc-concurrency-val">${Math.min(FB._plConfig.maxConcurrency || 1, 3)}</span>
                                <span class="text-[10px] text-dim">个批次同时运行，推荐1</span>
                            </div>
                        </div>
                    </div>
                    <div class="px-5 py-3 bg-[#0e0e10] border-t border-white/5 flex items-center justify-between">
                        ...
                        <div class="flex gap-2">
                            ...
                            <button class="btn btn-sm bg-gradient-to-r from-red-600 to-orange-500 text-white font-bold shadow-lg" onclick="Modules.fusion_book.startConfiguredPipeline()"><i class="fa-solid fa-box-open mr-1"></i>开始拿弹药</button>
                        </div>
                    </div>
                </div>
            </div>
