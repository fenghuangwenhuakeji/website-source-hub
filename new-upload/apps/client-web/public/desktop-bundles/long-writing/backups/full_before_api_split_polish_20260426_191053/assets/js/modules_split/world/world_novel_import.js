Object.assign(Modules.world_engine, {
    ...
    _restoreNovelImportJob() {
        try {
            ...
            this._novelImportJob = raw ? JSON.parse(raw) : null;
        } catch(e) {
    ...
    _setNovelImportJob(patch = {}) {
        ...
        this._novelImportJob = { ...prev, ...patch, logs, updatedAt: Date.now() };
    ...
    _finishNovelImportJob(status = '完成') {
        this._setNovelImportJob({ running: false, phase: 'done', current: 4, total: 4, status, log: status });
    },

    _renderNovelImportJobStatus() {
        const job = this._novelImportJob;
        if (!job) return '';
        const total = Math.max(1, job.total || 4);
        ...
        return `
        <div id="we-novel-import-job-status" class="px-6 py-3 border-b border-white/5 bg-black/20">
            <div class="flex items-center justify-between mb-2">
                <div class="text-[10px] font-bold ${job.running ? 'text-amber-300' : 'text-green-400'}">
                    <i class="fa-solid ${job.running ? 'fa-spinner fa-spin' : 'fa-check'} mr-1"></i>${job.status || '解析准备中'}
                </div>
            ...
            <div class="space-y-0.5">${logs || '<div class="text-[9px] text-dim">等待开始</div>'}</div>
            <div class="text-[9px] text-dim mt-2">解析任务进度已写入本地状态；关闭此窗口后重新打开仍可看到最近进度。</div>
        </div>`;
    },

    _renderNovelImportJobStatusIntoDom() {
        const old = document.getElementById('we-novel-import-job-status');
    ...
    _renderNovelImportModal() {
        ...
        modal.innerHTML = `
            <div class="bg-[#1a1a2e] rounded-2xl border border-white/10 w-[980px] max-w-[96vw] max-h-[90vh] flex flex-col shadow-2xl">
                ...
                ${we._renderNovelImportJobStatus()}
    ...
    async _startNovelImport() {
        ...
        we._setNovelImportJob({
            running: true,
            phase: 'parse',
            status: `正在解析原稿 · ${chunks.length} 个文本块`,
            current: 0,
            total: 4,
            logs: [],
            log: `开始解析：${text.length.toLocaleString()}字`
        });
        ...
        try {
            ...
            we._setNovelImportJob({ phase: 'structure', status: '正在识别卷章结构', current: 1, total: 4, log: '识别卷/章/正文边界' });
            ...
            we._setNovelImportJob({ phase: 'outline', status: `正在生成章内细纲 · ${structure.chapters?.length || 0}章`, current: 2, total: 4, log: '正文保留，反推分部分细纲' });
            ...
            if(extractEntities) {
                we._setNovelImportJob({ phase: 'entities', status: '正在提取实体入图谱', current: 3, total: 4, log: '结合正文和细纲提取人物、规则、伏笔、关系' });
            ...
            we._renderNovelImportModal();
            we._finishNovelImportJob(`体检完成：${structure.volumes?.length || 0}卷 / ${structure.chapters?.length || 0}章 / ${entities.length}实体`);
...
4. 一致性风险要指出续写时最容易写崩的地方。`;
    ...
    async _confirmNovelImport() {
        ...
        we._setNovelImportJob({
            ...
            status: '正在写入执笔台和世界引擎',
            current: 0,
            total: data.chapters.length + data.entities.length + 5,
            log: '开始写入章节正文、细纲和图谱实体'
        });
        ...
        try {
            ...
            for(const v of data.volumes) {
                ...
                we._setNovelImportJob({ current: progress, log: `写入卷：${v.title || '未命名卷'}` });
            }
            ...
            for(const c of data.chapters) {
                ...
                App.showProgress('写入数据库', ++progress, data.chapters.length + data.entities.length + 5);
                we._setNovelImportJob({ current: progress, log: `写入章节：${c.title || '未命名章'}` });
            }
            ...
            for(const [key, desc] of Object.entries(data.worldview || {})) {
                ...
                App.showProgress('写入数据库', ++progress, data.chapters.length + data.entities.length + 5);
                we._setNovelImportJob({ current: progress, log: `写入世界规则：${ent.name}` });
            }
            ...
            for(const e of data.entities || []) {
                ...
                App.showProgress('写入数据库', ++progress, data.chapters.length + data.entities.length + 5);
                we._setNovelImportJob({ current: progress, log: `写入实体：${e.name || '未命名实体'}` });
            }
            ...
            App.hideProgress();
            we._finishNovelImportJob(`导入成功：${data.chapters.length}章正文 / ${data.entities?.length || 0}实体`);
