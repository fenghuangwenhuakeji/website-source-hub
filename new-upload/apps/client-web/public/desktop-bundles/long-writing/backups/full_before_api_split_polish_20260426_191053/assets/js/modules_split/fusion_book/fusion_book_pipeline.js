Object.assign(Modules.fusion_book, {
        ...
        const renderSide = (side, book, containerId) => {
            ...
            el.innerHTML = book.chapters.map((ch, i) => `
                <label class="flex items-center gap-2 px-2 py-1 rounded hover:bg-white/5 cursor-pointer">
                    <input type="checkbox" class="plc-ch-${side} accent-green-500" data-idx="${i}" checked onchange="Modules.fusion_book._updateConfigSummary()">
    ...
    _updateConfigSummary() {
        ...
        const opts = [];
        if (document.getElementById('plc-do-extract')?.checked) opts.push('入世界');
        if (document.getElementById('plc-do-outline')?.checked) opts.push('写细纲');
        if (document.getElementById('plc-do-write')?.checked) opts.push('写正文');
        if (document.getElementById('plc-do-rag')?.checked) opts.push('存RAG');
        const el = document.getElementById('plc-summary');
        if (el) el.textContent = (cycleMode
            ? `左书 ${leftCount} 章，右书 ${rightCount} 章；按 ${cycleSize} 章/循环，共 ${cycles} 个循环`
    ...
    async _runConfiguredPipeline(isResume = false) {
        ...
        this._plLog(`📌 技法来源: 《${primaryName}》+ 《${secondaryName}》→ 写作弹药（角色/方向由世界引擎和执笔台控制）`, 'ok');
        ...
        if (infoEl) {
            ...
            infoEl.innerHTML = `左书: 《${leftBook.name}》 | 右书: 《${rightBook.name}》<br>` +
                ...
                `策略: ${cfg.cycleMode ? '循环拿弹药 ' + (cfg.cycleSize || 5) + '章/组' : '逐章拿弹药'} | Agent并发: ${maxConcurrency} | 启用: ${opts.join(' | ') || '只分析'}`;
        }

        this._plLog(`🚀 拆书${isResume ? '恢复' : '启动'}: ${pairs.length} 对章节 | ${cfg.cycleMode ? '循环拿弹药' : '逐章拿弹药'} | Agent并发:${maxConcurrency}`, 'ok');
        ...
        if (!this._pipelinePaused) {
            ...
            this._plLog(`\n🎉 拆书完成！共处理 ${pairs.length} 对章节`, 'ok');
            if (cfg.doOutline) this._plLog('📋 细纲已同步到: 长篇执笔(旗舰)', 'ok');
