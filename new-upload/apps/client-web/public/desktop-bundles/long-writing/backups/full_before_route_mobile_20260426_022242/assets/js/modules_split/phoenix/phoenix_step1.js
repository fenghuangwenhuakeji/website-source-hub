                    ...
                    <div class="w-64 shrink-0 flex flex-col bg-[#0a0a0c] border-l border-white/5">
                        ...
                        <div class="p-3 border-t border-white/5 space-y-2 shrink-0">
                            <button class="btn btn-xs bg-green-600/20 text-green-400 border-green-600/30 w-full ${extracted.length===0?'opacity-50 pointer-events-none':''}" onclick="Modules.phoenix._injectExtractedEntities()" id="ph-inject-entities-btn">
                                <i class="fa-solid fa-atom mr-1"></i>注入知识图谱+记忆
                            </button>
...
4. 情绪锚点 - 情绪分值、钩子类型、张力来源
5. 地点 - 场景、城市、秘境、地标
6. 势力 - 门派、组织、阵营、国家
7. 物品/能力 - 武器、法宝、道具、功法、技能

【输出格式】严格JSON数组：
[
  {
    "name":"名称",
    "type":"人物|世界规则|伏笔|情绪锚点|地点|势力|物品|能力",
    "desc":"一句话描述",
...
- 不要遗漏主角、核心反派、核心世界规则、第一卷主伏笔`;
        ...
        for (const ent of parsed) {
            ...
            unique.push({
                name: ent.name,
                type: ['人物','世界规则','伏笔','情绪锚点','地点','势力','物品','能力','功法','种族'].includes(ent.type) ? ent.type : '其他',
                desc: [
                    ent.desc || ent.description || '',
                    ent.state ? `状态: ${ent.state}` : '',
                    ent.risk ? `风险: ${ent.risk}` : ''
                ].filter(Boolean).join(' | '),
                state: ent.state || '',
        ...
        UI.toast(`提取完成！发现 ${unique.length} 个图谱实体`);
        this._setGenerating(false);
        this.refresh();
    },
    ...
    async _injectExtractedEntities() {
        ...
        for (const ent of entities) {
            ...
            try {
                if (typeof MemorySystem !== 'undefined') {
                    MemorySystem.addWorking(`[从零写一本/知识图谱] ${ent.type} ${ent.name}: ${ent.desc}`, 'phoenix_graph', 4, {
                        module: 'phoenix',
                        tags: ['从零写一本', '知识图谱', ent.type],
                        source: 'phoenix_extract'
                    });
                }
            } catch(e) {}
        ...
        UI.toast(`已注入 ${count} 个图谱实体到世界引擎和记忆`);
    },
