...
Modules.home = {
    ...
    _pathProfile(mode) {
        ...
        const map = {
            phoenix: {
                headline: '我要从零写一本',
                plain: '没有原稿，可以先写一段找手感，也可以先做细纲。',
                input: '输入：题材/一句话脑洞',
                process: '系统：写和拆交替补齐细纲、实体、规则、伏笔',
                result: '产出：可执行细纲 + 图谱上下文 + 正文',
                target: 'phoenix',
                targetLabel: '打开从零写',
                steps: ['想法', '试写/细纲', '入图谱', '正文']
            },
	            import: {
	                headline: '我要导入新书材料',
		                plain: '细纲先入工作台，已有正文直接进执笔台；精读解析交给凤凰流。',
	                input: '输入：一句话开书/细纲/已有正文',
	                process: '系统：按细纲提实体、规则、伏笔、关系',
	                result: '产出：章节树 + 图谱上下文 + 执笔台正文',
	                target: 'world_engine',
	                targetLabel: '打开导入新书',
	                steps: ['开书', '细纲工作台', '进入执笔']
	            },
            fusion: {
                headline: '我要拆书融合出新书',
                plain: '拿两本参考书提技法；可先拆循环，也可先写样章再反拆。',
                input: '输入：两本参考书',
                process: '系统：按循环/卷拆技法，融合成原创细纲',
                result: '产出：技法模板 + 新细纲 + 图谱 + 正文',
                target: 'fusion_book',
                targetLabel: '打开拆书融合',
                steps: ['导入双书', '循环拆技法', '融合细纲', '入图谱/正文']
            }
        };
    ...
    _renderModeCard(mode) {
        ...
        <div class="genesis-path-card rounded-lg border border-${config.color}-500/20 bg-${config.color}-500/[0.045] p-4 flex flex-col min-h-[210px]">
            ...
            <div class="route-meta-grid grid gap-1.5 mt-3">
                <div class="rounded-md bg-black/18 border border-white/5 px-2.5 py-2 text-[10px] text-gray-300 leading-relaxed">${profile.input}</div>
                <div class="rounded-md bg-black/18 border border-white/5 px-2.5 py-2 text-[10px] text-gray-300 leading-relaxed">${profile.process}</div>
                <div class="rounded-md bg-${config.color}-500/10 border border-${config.color}-500/15 px-2.5 py-2 text-[10px] text-${config.color}-200 leading-relaxed">${profile.result}</div>
            </div>
            <div class="route-step-row grid grid-cols-4 gap-1.5 mt-4">
    ...
    _renderWorkflowDeck() {
        const cards = [
            ['write_then_split','fusion_book','fa-book-open-reader','写到一半拆书','写不动时去拆几章参考书，技法回来继续写。','amber'],
            ['outline_first','fusion_book','fa-code-compare','先拆书再写','先拆参考书节奏和钩子，再回执笔台开正文。','cyan'],
		            ['continue_existing','world_engine','fa-file-import','导入新书/续写','快速放入工作台/执笔台，再交给凤凰流解析。','emerald'],
            ['hybrid','writer','fa-arrows-rotate','写作拆书来回切','写一阵、拆一阵、再回来写，不用一次跑完整链。','purple']
        ];
        return `
        <div class="mt-3 rounded-lg border border-white/5 bg-black/18 p-3">
            <div class="flex items-center justify-between gap-3 mb-2">
                <div>
                    <div class="text-xs font-bold text-white">写作 × 融合拆书</div>
                    <div class="text-[10px] text-dim mt-0.5">拆书不是一次性全自动跑完；你可以随时停、随时回写。</div>
                </div>
                <button class="text-[10px] text-dim hover:text-white" onclick="Modules.home._setFlowModeAndGo('hybrid','writer')">直接进执笔台 <i class="fa-solid fa-arrow-right ml-1"></i></button>
            </div>
            <div class="workflow-card-grid grid grid-cols-4 gap-2">
                ${cards.map(([mode, target, icon, title, sub, color]) => `
                    <button class="text-left rounded-lg border border-${color}-500/15 bg-${color}-500/[0.04] hover:bg-${color}-500/[0.075] p-3 transition-all" onclick="Modules.home._setFlowModeAndGo('${mode}','${target}')">
                        <div class="flex items-center gap-2">
                            <i class="fa-solid ${icon} text-${color}-400 text-xs"></i>
                            <span class="text-[11px] font-bold text-white">${title}</span>
                        </div>
    ...
    _actionsForProject(p) {
        ...
        if (p.mode === 'import') return [['world_engine','fa-seedling','一句话开书','题材、主角欲望、阻力和代价'], ['world_engine','fa-diagram-project','细纲入工作台','细纲提实体、规则、伏笔和关系'], ['writer','fa-feather-pointed','进入执笔','章节树、图谱和正文一起进入执笔台']];
        return [['fusion_book','fa-code-compare','拆书拿弹药','按循环/卷提取两本书技法'], ['fusion_workbench','fa-table-columns','查看成果','管理拆书输出'], ['writer','fa-feather-pointed','用技法写','把细纲和正文落在执笔台']];
    },

    _routeForProject(p) {
        if (p.mode === 'phoenix') return (p.stageIndex || 0) <= 1 ? { id: 'phoenix', label: '推进凤凰流' } : { id: 'writer', label: '继续写作' };
        if (p.mode === 'import') return (p.stageIndex || 0) <= 1 ? { id: 'world_engine', label: '细纲入工作台' } : { id: 'writer', label: '进入执笔' };
        return (p.stageIndex || 0) <= 3 ? { id: 'fusion_book', label: '拆书拿弹药' } : { id: 'writer', label: '用技法写' };
    },
    ...
    _setFlowModeAndGo(mode, target) {
        localStorage.setItem('writer_flow_mode', mode || 'hybrid');
        App.nav(target || 'writer');
    },
    ...
    async _activateProject(id) {
        if (Modules.project_manager && Modules.project_manager._activateProject) {
            ...
            if (proj) localStorage.setItem('genesis_active_project_mode', proj.mode || 'phoenix');
        }
        UI.toast('项目已激活');
        await this._refreshDashboard();
    },
    ...
    _nextAction(p) {
        ...
        const map = {
            ...
            import: ['一句话开书', '细纲入工作台', '进入执笔'],
            fusion: ['导入双书', '循环拆技法', '融合细纲', '入图谱/写正文', '进入执笔台']
        };
