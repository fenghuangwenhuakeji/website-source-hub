Object.assign(Modules.phoenix, {
    ...
    _parseOutlineStructureForSync(outline) {
        ...
        let volOrder = 1;
        let chapOrder = 1;
        const cleanHeading = rawLine => this._cleanOutlineHeading ? this._cleanOutlineHeading(rawLine) : String(rawLine || '').replace(/^\s{0,3}#{1,6}\s*/, '').replace(/\*\*/g, '').trim();
        const isVol = title => this._isVolumeHeadingTitle ? this._isVolumeHeadingTitle(title) : /^第\s*[一二三四五六七八九十百千万零〇两0-9]+\s*卷(?:[：:\s]|$)/.test(String(title || '').trim());
        const isChap = title => this._isChapterHeadingTitle ? this._isChapterHeadingTitle(title) : /^第\s*[一二三四五六七八九十百千万零〇两0-9]+\s*章(?:[：:\s]|$)/.test(String(title || '').trim());
        const isStructureHeading = rawLine => {
            const title = cleanHeading(rawLine);
            return isVol(title) || isChap(title);
        };
        const collectOutline = index => {
            const parts = [];
            for(let j = index + 1; j < lines.length; j++) {
                if(isStructureHeading(lines[j])) break;
                parts.push(lines[j]);
            }
            return parts.join('\n').trim();
        };
        ...
        if(!volumes.length && !chapters.length && this._parseOutlineStructureLegacy) return this._parseOutlineStructureLegacy(outline);
        return { volumes, chapters };
    },

    _normalizeSyncTitle(title) {
        return String(title || '')
            .replace(/\*\*/g, '')
            .replace(/[《》「」『』"'“”‘’\s]/g, '')
            .replace(/[：:，、；;。.!！?？-]/g, '')
