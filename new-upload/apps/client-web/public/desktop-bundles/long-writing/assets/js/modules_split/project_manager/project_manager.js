...
Modules.project_manager = {
            ...
            <div class="shrink-0 border-b border-white/5 bg-[#0a0a0c]">
                ...
                <div class="pm-filter-wrap px-5 pb-4 space-y-3">
                    ...
                    <select class="col-span-2 bg-black/30 border border-white/10 rounded-lg px-3 py-2 text-xs text-white outline-none" onchange="Modules.project_manager._modeFilter=this.value;Modules.project_manager._renderDynamic()">
                        ...
                        ${this._option('import','导入新书/续写',this._modeFilter)}
    ...
    async _refresh() {
        ...
        if (this._currentProjectId && !this._projects.some(p => p.id === this._currentProjectId)) {
            this._currentProjectId = null;
            GenesisCore.clearActiveProject();
            if (Modules.writer?._resetProjectRuntime) Modules.writer._resetProjectRuntime();
        }
        this._renderDynamic();
    },
    ...
    _showCreateModal(preselectedMode = 'phoenix') {
        ...
        const modes = [
            ...
            ['import','fa-file-import','我要导入新书材料','一句话开书 → 细纲入工作台 → 进入执笔','cyan'],
        ...
        <div class="w-full max-w-[560px] rounded-lg border border-white/10 bg-[#101014] shadow-2xl overflow-hidden">
            <div class="px-5 py-4 border-b border-white/5 flex items-center justify-between">
                <div>
                    ...
                    <div class="text-[10px] text-dim mt-1">先选目标。进入后可以随时先写、先拆、导入新书或混着来。</div>
                </div>
    ...
    async _deleteProject(id, name) {
        ...
        if (id === this._currentProjectId) this._currentProjectId = null;
        await this._refresh();
        UI.toast('项目已删除');
    },
    ...
    _pathProfile(mode) {
        const profiles = {
            ...
            import: {
                headline: '导入新书/续写',
	                plain: '细纲先进工作台，正文直接进执笔台；精读解析交给凤凰流。',
                icon: 'fa-file-import',
                color: 'cyan'
            },
            fusion: {
    ...
    _nextAction(p) {
        ...
        const map = {
            ...
            import: [
                ['一句话开书', '先放题材、主角欲望、阻力和代价。'],
                ['细纲入工作台', '细纲同步章节树，并提人物、规则、伏笔和关系入世界引擎。'],
                ['进入执笔', '已有正文直接落到执笔台，缺正文章节保留细纲待写。']
            ],
            fusion: [
