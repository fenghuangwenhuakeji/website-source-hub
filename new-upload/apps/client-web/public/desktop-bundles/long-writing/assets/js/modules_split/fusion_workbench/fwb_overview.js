Object.assign(Modules.fusion_workbench, {
    ...
    _renderOverview() {
        ...
        const stats = [
            ...
            { label: '融合精华', count: (d.cycles || []).length + (d.ammo || []).length + (d.fusionContext?.content ? 1 : 0), icon: 'fa-wand-magic-sparkles', color: 'green' },
            ...
            { label: '循环融合', count: (d.worldCycles || []).length, icon: 'fa-arrows-spin', color: 'indigo' }
        ];
        const totalChars = [
            d.fusionContext?.content?.length || 0,
            ...(d.ammo || []).map(x => x.content?.length || 0),
    ...
    _getRecentActivity() {
        ...
        (d.ammo || []).forEach(a => items.push({ title: a.name || '融合弹药', time: fmt(a.createdAt), _ts: a.createdAt, icon: 'fa-wand-magic-sparkles', color: 'green' }));
