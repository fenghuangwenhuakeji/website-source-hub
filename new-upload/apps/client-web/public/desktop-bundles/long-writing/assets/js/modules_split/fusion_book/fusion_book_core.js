...
Modules.fusion_book = {
    ...
    _plConfig: {
        ...
        doExtract: true,    // 创作模式入世界引擎；弹药模式入拆书弹药库
        doOutline: false,   // 创作模式写入执笔台细纲
        doWrite: false,     // 创作模式在执笔台自动写正文
        doRAG: true,        // 是否存RAG
        saveFolder: '',     // 本地保存文件夹
        lastSync: null,     // 最后同步时间
        directionLock: '',   // 当前新书方向/人物护栏
        flowMode: 'tool',    // tool=弹药模式 / creative=按脑洞融合出新书
    ...
    render() {
        ...
        <div class="flex flex-col h-full bg-[#0a0a0c] overflow-hidden relative">
            ...
            <div id="fb-pipeline-mini" class="absolute bottom-12 right-4 z-50 bg-gradient-to-r from-red-600 to-orange-500 rounded-full shadow-lg shadow-red-500/30 px-5 py-2.5 flex items-center gap-3 cursor-pointer hover:scale-105 transition-transform" onclick="Modules.fusion_book._pipelineRunning ? Modules.fusion_book.plRestore() : (Modules.fusion_book._savedPipelineState ? Modules.fusion_book._resumeFromSaved() : Modules.fusion_book.showPipelineConfig())">
                    <span class="text-white text-sm font-bold"><i class="fa-solid fa-box-open mr-1.5"></i><span id="pl-mini-status">${FB._savedPipelineState ? '继续上次拆书 (' + (FB._savedPipelineState.completedPairs||[]).length + '章已完成)' : '拆书模式'}</span></span>
                ...
                <div class="fb-config-panel w-[720px] max-h-[85vh] bg-[#111113] border border-white/10 rounded-2xl shadow-2xl overflow-hidden flex flex-col" onclick="event.stopPropagation()">
                    ...
                    <div class="flex-1 overflow-y-auto p-5 space-y-4">
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <button id="plc-flow-creative" class="text-left rounded-xl border p-3 transition-all ${FB._plConfig.flowMode === 'creative' ? 'border-orange-400 bg-orange-500/15 ring-1 ring-orange-400/50' : 'border-white/10 bg-black/20 hover:border-orange-400/50'}" onclick="Modules.fusion_book._setPipelineFlowMode('creative')">
                            ...
                            <button id="plc-flow-tool" class="text-left rounded-xl border p-3 transition-all ${FB._plConfig.flowMode !== 'creative' ? 'border-emerald-400 bg-emerald-500/15 ring-1 ring-emerald-400/50' : 'border-white/10 bg-black/20 hover:border-emerald-400/50'}" onclick="Modules.fusion_book._setPipelineFlowMode('tool')">
                        ...
                        <div class="bg-black/20 rounded-lg p-4 border border-white/5 space-y-3">
                            ...
                            <label class="flex items-center gap-2 cursor-pointer"><input type="checkbox" id="plc-do-extract" ${FB._plConfig.doExtract ? 'checked' : ''} class="accent-green-500" onchange="Modules.fusion_book._updateConfigSummary()"><span class="text-xs text-gray-300">🌍 创作入世界引擎；弹药入弹药库/RAG</span></label>
                            <label class="flex items-center gap-2 cursor-pointer"><input type="checkbox" id="plc-do-outline" ${FB._plConfig.doOutline ? 'checked' : ''} class="accent-green-500" onchange="Modules.fusion_book._updateConfigSummary()"><span class="text-xs text-gray-300">📋 生成原创循环细纲 → 执笔台</span></label>
                            <label class="flex items-center gap-2 cursor-pointer"><input type="checkbox" id="plc-do-write" ${FB._plConfig.doWrite ? 'checked' : ''} class="accent-green-500" onchange="Modules.fusion_book._updateConfigSummary()"><span class="text-xs text-gray-300">✍️ 执笔台自动写正文 → M06默认文风</span></label>
                            <label class="flex items-center gap-2 cursor-pointer"><input type="checkbox" id="plc-do-rag" ${FB._plConfig.doRAG ? 'checked' : ''} class="accent-green-500" onchange="Modules.fusion_book._updateConfigSummary()"><span class="text-xs text-gray-300">🔍 RAG上下文同步</span></label>
                        ...
                        <div class="bg-cyan-900/10 rounded-lg p-4 border border-cyan-500/20 space-y-3">
                            <div class="text-xs font-bold text-cyan-400 mb-2"><i class="fa-solid fa-sync mr-1"></i>逐循环处理</div>
	                            <label class="flex items-center gap-2 cursor-pointer"><input type="checkbox" id="plc-cycle-mode" ${FB._plConfig.cycleMode ? 'checked' : ''} class="accent-cyan-500" onchange="Modules.fusion_book._updateConfigSummary()"><span class="text-xs text-gray-300">左书N章 + 右书N章 → 批量拆左N章 → 批量拆右N章 → 循环对比 → 按脑洞融合 → 产出新书N章。</span></label>
                            <div class="flex items-center gap-3">
                                <div class="flex items-center gap-2">
...
- 输出正文时只给正文；输出细纲时只给可执行细纲，不要自检报告。`;
    ...
    async fusionMerge() {
        ...
        if (result) {
            const contextId = this._isCreativeFlow?.() ? 'creative_pipeline_fusion_context' : 'pipeline_fusion_context';
            await DB.put('settings', { id: contextId, content: this._allPipelineResults.fusion, updatedAt: Date.now() });
            const leftBook = (this._books || []).find(b => b.id === this.left.bookId);
            const rightBook = (this._books || []).find(b => b.id === this.right.bookId);
            const leftIdx = this.left.chapterIdx;
            const rightIdx = this.right.chapterIdx;
            await this._savePipelineAmmo?.('fusion', {
                ...
                rightTitle: rightBook?.chapters?.[rightIdx]?.title || '',
                content: result,
                title: `融合弹药 · 第${(leftIdx || 0) + 1}章 vs 第${(rightIdx || 0) + 1}章`,
                tags: ['融合精华']
            });
        }
        // ★ 同步融合数据到项目
