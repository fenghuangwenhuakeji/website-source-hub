        ...
        return `
            <div class="flex-1 flex items-center justify-center animate-fade-in">
                <div class="text-center max-w-lg">
                    ...
                    <h3 class="text-2xl font-bold text-white mb-3">准备直接同步</h3>
                    <p class="text-gray-400 mb-6">执行级细纲进入执笔台章节树，图谱实体进入世界引擎；只有拆书/融合拆书产物才走暂存。</p>
                    <div class="grid grid-cols-2 gap-3 mb-4">
                        <div class="p-4 bg-white/5 rounded-lg border border-white/10">
                            <div class="text-xl font-bold text-accent" id="ph-chap-count">${outlineStats.chapCount}</div>
                            <div class="text-[10px] text-dim uppercase mt-1">总章数</div>
                        </div>
                        <div class="p-4 bg-white/5 rounded-lg border border-white/10">
                            <div class="text-xl font-bold text-blue-400" id="ph-vol-count">${outlineStats.volCount}</div>
                    ...
                    <div class="flex gap-2 justify-center mb-4">
                        <button class="btn btn-xs bg-green-600/20 text-green-400 border-green-600/30 ${extracted.length===0?'opacity-50':''}" onclick="Modules.phoenix.syncExtractedEntitiesToWorld()" ${extracted.length===0?'disabled':''}><i class="fa-solid fa-atom mr-1"></i>实体同步世界</button>
                        <button class="btn btn-xs bg-amber-600/20 text-amber-400 border-amber-600/30" onclick="Modules.phoenix.syncOutlineToWriterAndWorld()"><i class="fa-solid fa-layer-group mr-1"></i>结构同步执笔</button>
                    ...
                    <div class="text-[10px] text-dim">点击「确认同步」后，细纲会直接写入执笔台，实体会直接写入世界引擎。</div>
                </div>
            </div>`;
    },
...
请客观分析。`;
    ...
    next() {
        ...
        if (this.step < 2) {
            ...
            if (this.step === 2) {
                const outlineStats = this._getOutlineStructureStats(this.data.outlineRaw || '');
                const el = document.getElementById('ph-chap-count'); if(el) el.innerText = outlineStats.chapCount;
                const vl = document.getElementById('ph-vol-count'); if(vl) vl.innerText = outlineStats.volCount;
            }
        } else {
            this.finish();
        }
    ...
    _updateStats() {
        ...
        const outlineStats = this._getOutlineStructureStats(raw);
        if(el) el.textContent = raw.length + ' 字 · ' + outlineStats.chapCount + ' 章';
    },
    _setGenerating(on) {
        this._generating = on;
    ...
    _updateGenProgress(text) {
        ...
        const outlineStats = this._getOutlineStructureStats(text);
        const chaps = outlineStats.chapCount;
        const vols = outlineStats.volCount;
