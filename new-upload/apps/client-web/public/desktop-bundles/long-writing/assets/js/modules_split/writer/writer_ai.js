...
${targetWords ? `11. 目标长度：${targetWords}。` : ''}${title ? `\n12. 当前章节：${title}。` : ''}`;
    ...
    _sanitizeEditableProse(text) {
        return this._sanitizeGeneratedProse(text || '').trim();
    },
...
7. 直接输出正文，不要解释`;
    ...
    async polish() {
        ...
        let result = '';
        await AI.generate(prompt, { apiType: 'text', module: 'writer_polish' }, c => { result += c; this.updateIO(prompt, result); });
        this._setGenerating(false);
        result = this._sanitizeEditableProse(result);
