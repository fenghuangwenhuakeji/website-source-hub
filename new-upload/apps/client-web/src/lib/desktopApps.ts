...
const isDesktopAppsDebugEnabled = (): boolean => {
  if (typeof window === 'undefined') return false;
  try {
    return window.localStorage?.getItem('desktopAppsDebug') === '1';
  } catch {
    return false;
  }
};
const logDesktopApps = (...args: unknown[]) => {
  ...
  if (!isDesktopAppsDebugEnabled()) return;
  console.debug('[desktopApps]', ...args);
};
...
const BUILTIN_DESKTOP_APPS: DesktopAppDefinition[] = [
  ...
  },
  {
    ...
    kind: 'legacy',
    runtime: 'static-web',
    route: buildStaticBundleRoute('long-writing'),
    tags: ['imported', 'static-web', 'writing', 'long-form', 'novel'],
    ...createExperienceMetadata('writing', ['writing', 'long', 'novel', 'chapter'], 'primary'),
  },
  {
    id: 'kui-chat',
    title: '聊天',
  ...
  },
  {
    ...
    kind: 'special',
    runtime: 'static-web',
    route: `${buildStaticBundleRoute('agent-novel')}?v=20260429-cn`,
    tags: ['imported', 'static-web', 'agent', 'novel', 'writing'],
    ...createExperienceMetadata('agent', ['agent', 'novel', 'writing', 'workflow'], 'primary'),
  },
  {
  ...
  },
  {
    ...
    ...createExperienceMetadata('writing', ['writing', 'medium', 'short', 'novella'], 'primary'),
  },
  {
    id: 'codeEditor',
    title: 'IDE',
    description: '打开全新版 Hyper Code IDE。',
    icon: 'Code2',
    color: '#007acc',
    width: 1280,
    height: 860,
  ...
  },
  {
    ...
    ...createExperienceMetadata('writing', ['writing', 'reading', 'analysis', 'chapter'], 'secondary'),
  },
  {
    id: 'fenghuang',
    title: '凤煌创作',
...
async function readInstalledManifest(appId: string): Promise<DesktopAppDefinition | null> {
  ...
  logDesktopApps('readInstalledManifest skipped stale registry entry', { appId });
  return null;
}
...
export async function getInstalledDesktopApps(): Promise<DesktopAppDefinition[]> {
  ...
  let hasStaleRegistryEntries = false;
  ...
  for (const appId of registry.appIds) {
    ...
    if (manifest) {
      ...
      hasStaleRegistryEntries = true;
    }
  }

  if (hasStaleRegistryEntries) {
    await writeRegistry({
      appIds: validAppIds,
      updatedAt: Date.now(),
    }).catch((error) => {
      warnDesktopApps('prune stale registry entries failed', { error });
    });
  }
