    ...
    <div className={styles.overlay} onClick={onClose}>
      <div className={`${styles.panel} ${styles.panelWide}`} onClick={(e) => e.stopPropagation()}>
        <div className={styles.panelHeader}>
          <span className={styles.panelTitle}>故事模组</span>
          ...
          <div className={styles.listView}>
            {mods.map((entry) => {
              ...
              const progress = st.is_finished
                ? '已完成'
                : `阶段 ${st.current_stage_index + 1}/${cfg.stage_count}`;
                ...
                >
                  <div className={styles.listItemInfo} style={{ flex: 1 }}>
                    ...
                    <div className={styles.listItemDesc}>
                      {progress} &middot; {cfg.mod_description?.slice(0, 60) || '无描述'}
                    </div>
                    {/* Mini progress bar */}
...
const ModEditor: React.FC<{
  ...
  return (
    <div className={styles.overlay} onClick={onClose}>
      <div className={`${styles.panel} ${styles.panelWide}`} onClick={(e) => e.stopPropagation()}>
        <div className={styles.panelHeader}>
          <span className={styles.panelTitle}>编辑模组</span>
...
const ProgressView: React.FC<{ config: ModConfig; state: ModState }> = ({ config, state }) => {
  ...
  return (
    <div className={styles.progressView}>
          ...
          return (
            ...
            >
              <div className={styles.stageCardHeader}>
                ...
                {isCurrent && <span className={styles.stageCardBadge}>当前</span>}
              </div>
      ...
      {state.is_finished && <div className={styles.finishedBanner}>所有阶段已完成！</div>}
    </div>
  );
};
  ...
  return (
    <div className={styles.editView}>
      <div className={styles.field}>
        <label className={styles.label}>模组名称</label>
        <input
      ...
      {stages.map((stage, si) => (
        <div key={si} className={styles.stageEditCard}>
          <div className={styles.stageEditHeader}>
            <span className={styles.stageEditIndex}>阶段 {si + 1}</span>
          ...
          <div className={styles.targetsSection}>
            <span className={styles.targetsLabel}>目标</span>
