...
type MobileTabId = 'home' | 'medium' | 'ide' | 'agent' | 'long';
...
const MOBILE_CORE_PRODUCTIVITY_APP_IDS = [
  'long-writing',
  'webChat',
  'ai-agent',
  'codeEditor',
...
const MOBILE_FULLSCREEN_WORKBENCH_APP_IDS = new Set(['webChat', 'codeEditor', 'ai-agent', 'long-writing']);
...
const APP_SUMMARY_MAP: Record<string, string> = {
  'long-writing': '正式版长篇创作工作区。',
  'kui-chat': '打开聊天、角色和历史会话。',
  'ai-agent': '打开正式版智能体写小说工作台。',
  settings: '调整外观、窗口和运行偏好。',
  'frontend-converter': '接入网页、本地程序和展示页。',
  codeEditor: '打开全新版 Hyper Code IDE。',
  webChat: '打开正式版中短篇创作页面。',
  'short-book-lab': '短篇、短短篇拆书与内容分析。',
  'html-vault': '浏览内置 HTML 应用内容库。',
  fenghuang: '打开凤煌创作套件。',
...
const MOBILE_TABS: Array<{
  ...
  { id: 'long', label: '长篇', icon: FileText },
  { id: 'medium', label: '中短篇', icon: NotebookPen },
  { id: 'agent', label: 'Agent', icon: Bot },
  { id: 'ide', label: 'IDE', icon: Code2 },
];
const MOBILE_DOCK_TABS = MOBILE_TABS.filter((tab) => tab.id !== 'home');
...
const getLauncherTypeLabel = (app: DesktopAppDefinition) => {
  ...
  if (app.runtime === 'static-web') return '本地应用';
...
const MacOSDesktop: React.FC<MacOSDesktopProps> = ({ onOpenRecharge }) => {
  ...
  useEffect(() => {
    const syncViewport = () => {
      const isElectronDesktop = !!(window as any).electronAPI?.isElectron;
      setIsMobile(!isElectronDesktop && (window.innerWidth <= 900 || /iPhone|Android|iPad/i.test(navigator.userAgent)));
    };
    syncViewport();
  ...
  const desktopApps = useMemo(() => visibleCatalog.filter((app) => !['finder', '终端', 'settings'].includes(app.id)), [visibleCatalog]);
  ...
  const pinnedLauncherItems = useMemo(() => {
    const preferredIds = ['long-writing', 'webChat', 'ai-agent', 'codeEditor', 'html-vault'];
  ...
  const mobilePrimaryApps = useMemo(() => {
    ...
    const preferred = MOBILE_CORE_PRODUCTIVITY_APP_IDS
      .map((appId) => appsById.get(appId))
      .filter(Boolean) as DesktopAppDefinition[];
    const ordered = [...preferred, ...importedDesktopApps, ...(diaryApp ? [diaryApp] : [])];
    const seen = new Set<string>();
  ...
  }, [appSearch, appsById, importedDesktopApps, visibleCatalog]);
  ...
  const mobileFullscreenWorkbenchMode = Boolean(
    mobileCurrentAppId && MOBILE_FULLSCREEN_WORKBENCH_APP_IDS.has(mobileCurrentAppId),
  );
  ...
  const openApp = useCallback((app: DesktopAppDefinition) => {
    if (isMobile) {
      setMobileMenuOpen(false);
      if (app.id === MOBILE_TAB_APP_MAP.medium) {
        setMobileTab('medium');
        setMobileAppId(null);
        return;
      }
      if (app.id === MOBILE_TAB_APP_MAP.ide) {
        setMobileTab('ide');
        setMobileAppId(null);
        return;
      }
      if (app.id === MOBILE_TAB_APP_MAP.agent) {
        setMobileTab('agent');
        setMobileAppId(null);
        return;
      }
      if (app.id === MOBILE_TAB_APP_MAP.long) {
        setMobileTab('long');
        setMobileAppId(null);
        return;
      }
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      if (event.ctrlKey && event.key.toLowerCase() === 'd') {
        ...
        setDockVisible((prev) => {
          const next = !prev;
          if (!next && dockHideTimerRef.current) {
            window.clearTimeout(dockHideTimerRef.current);
            dockHideTimerRef.current = null;
          }
          return next;
      ...
      prev.map((item) => {
        ...
        if (item.maximized) {
          return {
            ...item,
            maximized: false,
            x: item.prevSize?.x ?? item.x,
            y: item.prevSize?.y ?? item.y,
            width: item.prevSize?.width ?? item.width,
            height: item.prevSize?.height ?? item.height,
            zIndex: item.prevSize?.zIndex ?? item.zIndex,
            prevSize: undefined,
          };
        }
        return {
          ...item,
          maximized: true,
          prevSize: { x: item.x, y: item.y, width: item.width, height: item.height, zIndex: item.zIndex },
          x: 0,
          y: 0,
    ...
    return (
      ...
      >
        ...
        <div className={`${styles.mobileContent} ${mobileAppMode ? styles.mobileContentAppMode : ''}`}>
          {mobileCurrentAppId ? (
            <div
              className={`${styles.mobileAppShell} ${mobileFullscreenWorkbenchMode ? styles.mobileAppShellProductivity : ''}`}
              data-app-id={mobileCurrentAppId}
            >
              <div className={styles.mobileAppOverlay}>
                ...
                {mobileFullscreenWorkbenchMode && mobileCurrentApp ? (
                  <div className={styles.mobileAppOverlayTitle}>
                    <strong>{mobileCurrentApp.title}</strong>
                    <span>{mobileHeaderSubtitle}</span>
                  </div>
                ) : null}
              </div>
            ...
            <>
              <section className={styles.mobilePhoneHero}>
                <div className={styles.mobilePhoneHeroCopy}>
                  ...
                  <h1>核心生产力</h1>
                  <span>长篇、中短篇、智能体写小说和 IDE 固定到底部，HTML 内容库放在核心入口。</span>
                </div>
                ...
                <div className={styles.mobileTopPills}>
                  <span className={styles.mobileTopPill}>核心 {Math.min(mobilePrimaryApps.length, MOBILE_CORE_PRODUCTIVITY_APP_IDS.length)}</span>
  ...
  return (
    ...
    >
      ...
      {windows.filter((item) => !item.minimized).sort((a, b) => a.zIndex - b.zIndex).map((item) => (
        <div key={item.id} className={`${styles.windowShell} ${item.active ? styles.windowActive : ''} ${item.maximized ? styles.windowMaximized : ''}`} style={item.maximized ? { left: 0, top: 0, width: '100vw', height: '100vh', zIndex: item.zIndex } : { left: item.x, top: item.y, width: item.width, height: item.height, zIndex: item.zIndex }} onMouseDown={() => focusWindow(item.id)}>
          <div className={styles.windowHeader} onMouseDown={(event) => { if (!item.maximized) { dragRef.current = { id: item.id, startX: event.clientX, startY: event.clientY, x: item.x, y: item.y }; } }}>
            <div className={styles.windowControls}>
              ...
              <button className={styles.controlMaximize} onClick={() => maximizeWindow(item.id)}>{item.maximized ? <Minimize2 size={10} /> : <Maximize2 size={10} />}</button>
            </div>
...
const FinderPanel: React.FC<{
  ...
  const quickOpenApps = apps.filter((app) =>
    ['long-writing', 'ai-agent', 'codeEditor', 'webChat', 'frontend-converter', 'kui-chat', 'html-vault', 'fenghuang', 'settings'].includes(app.id),
  );
    ...
    <div className={`${styles.settingsShell} ${isMobile ? styles.settingsShellMobile : ''}`}>
      ...
      <section className={styles.settingsHero}>
        ...
        <div className={styles.settingsActionList}>
          <button type="button" className={styles.settingsLaunchButton} onClick={() => onOpenApp('long-writing')}>
            打开长篇
          </button>
          ...
          <button type="button" className={styles.settingsLaunchButton} onClick={() => onOpenApp('webChat')}>
            打开中短篇
          </button>
