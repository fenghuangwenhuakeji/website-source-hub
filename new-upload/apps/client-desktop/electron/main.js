...
async function handleLocalFsApi(req, res, requestUrl) {
    const pathname = requestUrl.pathname;

    try {
        if (pathname === '/api/fs/drives') {
            const drives = getAvailableDrives();
            sendJson(res, 200, { success: true, data: drives });
            return;
        }
        ...
        if (pathname.startsWith('/api/agent/')) {
            // Agent APIs: return empty success for now to prevent 404 errors
            sendJson(res, 200, { success: true, data: null });
            return;
        }

        // Unknown local API path
        sendJson(res, 404, { success: false, error: 'Unknown local API endpoint' });
    } catch (error) {
        console.error('[LocalFS API] Error:', error);
        sendJson(res, 500, {
            success: false,
    ...
    return new Promise((resolve, reject) => {
        const server = http.createServer(async (req, res) => {
            try {
                ...
                if (requestUrl.pathname.startsWith('/api/fs/') || requestUrl.pathname.startsWith('/api/agent/')) {
                    await handleLocalFsApi(req, res, requestUrl);
                    return;
                }
