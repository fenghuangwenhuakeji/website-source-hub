const { Client } = require('ssh2');

const conn = new Client();
conn.on('ready', () => {
  console.log('SSH连接成功');
  
  const envContent = `NODE_ENV=production
PORT=3001
DB_HOST=localhost
DB_USER=root
DB_PASSWORD=gong134135
DB_NAME=license_db
JWT_SECRET=your-super-secret-jwt-key-change-this-in-production
JWT_EXPIRES_IN=7d
ALIPAY_APP_ID=2021006140607225
ALIPAY_GATEWAY_URL=https://openapi.alipay.com/gateway.do
ALIPAY_NOTIFY_URL=http://115.190.158.182/api/payment/callback/alipay
ALIPAY_RETURN_URL=http://115.190.158.182/payment/return
ALIPAY_PRIVATE_KEY=
ALIPAY_PUBLIC_KEY=
`;
  
  conn.sftp((err, sftp) => {
    if (err) throw err;
    
    sftp.writeFile('/root/backup_20260324/apps/license-backend/.env', envContent, (err) => {
      if (err) {
        console.error('创建.env文件失败:', err);
        conn.end();
        return;
      }
      console.log('.env 文件创建成功');
      
      // 安装dotenv包并重启服务
      conn.exec('cd /root/backup_20260324/apps/license-backend && npm install dotenv --save 2>/dev/null; pm2 restart license-backend', (err, stream) => {
        if (err) throw err;
        stream.on('close', (code, signal) => {
          console.log('服务重启完成，退出码: ' + code);
          conn.end();
        }).on('data', (data) => {
          console.log('' + data);
        }).stderr.on('data', (data) => {
          console.log('STDERR: ' + data);
        });
      });
    });
  });
}).connect({
  host: '115.190.158.182',
  port: 22,
  username: 'root',
  password: 'Brfj0114'
});
