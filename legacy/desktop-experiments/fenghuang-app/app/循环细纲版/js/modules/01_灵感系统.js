...
function renderInspirationPanel() {
        ...
        <div class="inspiration-panel-grid">
            <div class="inspiration-section">
                ...
                <div id="writing-config-section">
                    ...
                    <div class="form-group">
                        <label>篇幅选择</label>
                        <div class="button-group" id="length-options">
                            <button class="settings-btn" data-volumes="3" data-chapters="30">短篇 (15-30章)</button>
                            <button class="settings-btn" data-volumes="5" data-chapters="90">短中篇 (约90章)</button>
                            <button class="settings-btn active" data-volumes="10" data-chapters="200">中篇 (30-200章)</button>
                            <button class="settings-btn" data-volumes="20" data-chapters="500">长篇 (400-600章)</button>
                        </div>
                    </div>
                    ...
                    <div class="scribe-controls" style="grid-template-columns: 1fr 1fr 1fr; gap: 15px;">
                        ...
                        <div class="form-group"><label for="total-volumes">总卷数</label><input type="number" id="total-volumes" value="10" step="1" min="1"></div>
                        <div class="form-group"><label for="total-chapters">总章节数</label><input type="number" id="total-chapters" value="300" step="1" min="1"></div>
                    </div>
                    <div id="manual-mode-actions">
                        <div class="button-group" style="grid-template-columns: 1fr 1fr; gap: 10px; margin-top: 10px;">
                           <button id="start-manual-btn" class="action-btn"><i class="fas fa-play-circle"></i> AI生成蓝图</button>
                           <button id="import-blueprint-btn" class="settings-btn"><i class="fas fa-upload"></i> 导入外部大纲</button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="inspiration-section">
                <div class="card-header" style="display: flex; justify-content: space-between; align-items: center; padding: 15px 25px;">
                    <h3><i class="fas fa-file-alt"></i> AI生成的故事蓝图 (可修改)</h3>
                    <button id="toggle-blueprint-view-btn" class="settings-btn" data-view-mode="rendered"><i class="fas fa-code"></i> 查看源码</button>
                </div>
                <div id="inspiration-output-container" class="document-panel" style="min-height: 75vh; display: flex; flex-direction: column; padding: 0;">
                     <div id="blueprint-editor" contenteditable="true" class="cockpit-scroll-area document-panel" style="flex-grow: 1; width: 100%; background: transparent; border: none; resize: none; color: var(--text-color); padding: 25px; line-height: 1.8;" placeholder="请先输入灵感，然后点击左侧的“生成故事蓝图”按钮。">${creationState.inspirationConcept || ''}</div>
                </div>
                <div class="button-group" style="margin-top: 20px;">
                    <button id="continue-blueprint-btn" class="settings-btn hidden" style="flex-grow: 1;"><i class="fas fa-angle-double-down"></i> 继续生成蓝图</button>
                    <button id="confirm-inspiration-btn" class="action-btn" style="flex-grow: 2; background-color: var(--success-color);" ${!creationState.inspirationConcept ? 'disabled' : ''}><i class="fas fa-check-double"></i> 确认蓝图，进入写作</button>
                </div>
            </div>
        </div>
    ...
    document.getElementById('import-blueprint-btn').addEventListener('click', () => {
        document.getElementById('import-blueprint-modal').classList.remove('hidden');
    });
    document.getElementById('confirm-inspiration-btn').addEventListener('click', confirmInspiration);

    // 为新的导入模态框添加事件监听
    const importModal = document.getElementById('import-blueprint-modal');
    importModal.querySelector('.close-btn').addEventListener('click', () => importModal.classList.add('hidden'));
    importModal.querySelector('#cancel-import-btn').addEventListener('click', () => importModal.classList.add('hidden'));
    importModal.querySelector('#confirm-import-btn').addEventListener('click', processImportedBlueprint);
    document.getElementById('continue-blueprint-btn').addEventListener('click', continueBlueprintGeneration);
    document.getElementById('toggle-blueprint-view-btn').addEventListener('click', toggleBlueprintView);

    const blueprintEditor = document.getElementById('blueprint-editor');
    blueprintEditor.addEventListener('input', () => {
        creationState.inspirationConcept = blueprintEditor.innerHTML;
        checkBlueprintForContinuation();
    });

    document.getElementById('length-options').addEventListener('click', (e) => {
        if (e.target.tagName === 'BUTTON') {
            document.querySelectorAll('#length-options button').forEach(btn => btn.classList.remove('active'));
            e.target.classList.add('active');
            document.getElementById('total-volumes').value = e.target.dataset.volumes;
            document.getElementById('total-chapters').value = e.target.dataset.chapters;
        }
    });

    // document.getElementById('scribe-persona').addEventListener('change', () => { // 已迁移
