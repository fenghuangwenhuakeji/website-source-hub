...
Modules.web_chat = {
    ...
    render() {
        ...
        <div class="flex h-full bg-[#F8F9FA] overflow-hidden">
            <!-- 左侧：会话列表 -->
            <div class="w-72 shrink-0 flex flex-col bg-white border-r border-gray-200">
                <div class="p-4 border-b border-gray-200 bg-[#F1F3F5]">
                ...
                <div class="p-3 border-t border-gray-200">
                    <button class="btn w-full h-8 bg-gray-100 text-dim hover:text-gray-800 text-xs" onclick="Modules.web_chat.exportAllSessions()">
            ...
            <div class="flex-1 flex flex-col min-w-0">
                <div class="h-12 flex items-center justify-between px-5 bg-[#F1F3F5] border-b border-gray-200 shrink-0">
                    <div class="flex items-center gap-3">
                        <div class="flex items-center gap-2">
                            ${this.roles.map(r => `
                                <button class="px-2.5 py-1.5 rounded-lg text-[10px] font-bold transition-all ${this.currentRoleId === r.id ? 'bg-gray-200 border border-gray-300 text-gray-800' : 'text-dim hover:bg-gray-100 border border-transparent'}" onclick="Modules.web_chat.switchRole('${r.id}')" title="${r.desc}">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <button class="btn btn-xs bg-gray-100 text-dim" onclick="Modules.short.openPromptModal('web_chat')">
                ...
                <div class="p-3 border-t border-gray-200 shrink-0 bg-[#F1F3F5]">
                    <div class="flex gap-2 mb-2 flex-wrap">
                        ${['续写', '润色', '扩写', '翻译'].map((t, i) => `
                            <button class="btn btn-xs bg-gray-100 text-dim hover:text-gray-800" onclick="Modules.web_chat.quickAction('${t}')">${t}</button>
                        `).join('')}
                    ...
                    <div class="flex gap-2">
                        <textarea id="wc-input" class="flex-1 bg-gray-100 border border-gray-300 rounded-lg p-3 text-sm text-gray-700 resize-none h-20 focus:border-accent/30" placeholder="输入消息... (Shift+Enter换行)" onkeydown="if(event.key==='Enter'&&!event.shiftKey){event.preventDefault();Modules.web_chat.sendMessage()}"></textarea>
            ...
            <!-- 右侧：资源管理库 -->
            <div class="w-80 shrink-0 flex flex-col bg-white border-l border-gray-200" id="wc-resources-panel">
    ...
    _renderSessionsList() {
        ...
        return this.sessions.map(s => `
            <div class="p-3 rounded-lg mb-1 cursor-pointer transition-all ${this.currentSessionId === s.id ? 'bg-gray-200 border border-gray-300' : 'hover:bg-gray-100 border border-transparent'}" onclick="Modules.web_chat.selectSession('${s.id}')">
                <div class="flex items-center justify-between mb-1">
                    <span class="text-xs font-bold text-gray-800 truncate">${s.title || '新对话'}</span>
    ...
    _renderMessages() {
            ...
            return `
                <div class="flex items-center justify-center h-full">
                    <div class="text-center max-w-md">
                        <div class="w-16 h-16 rounded-full bg-gradient-to-br from-blue-500/20 to-purple-500/20 border border-gray-300 flex center mx-auto mb-4">
                            <i class="fa-solid ${role?.icon || 'fa-robot'} text-2xl ${role?.color || 'text-blue-400'}"></i>
                        </div>
                        <div class="text-sm font-bold text-gray-800 mb-2">${role?.name || '智能助手'}</div>
                        <div class="text-[10px] text-dim mb-4">${role?.desc || '准备好帮助您了'}</div>
                        <div class="text-[9px] text-dim">
                            <p class="mb-2">试试这些快捷操作：</p>
                            <div class="flex flex-wrap gap-1 justify-center">
                                <button class="px-2 py-1 rounded bg-gray-100 hover:bg-gray-200 text-[9px]" onclick="Modules.web_chat.quickAction('续写')">续写</button>
                                <button class="px-2 py-1 rounded bg-gray-100 hover:bg-gray-200 text-[9px]" onclick="Modules.web_chat.quickAction('润色')">润色</button>
                                <button class="px-2 py-1 rounded bg-gray-100 hover:bg-gray-200 text-[9px]" onclick="Modules.web_chat.quickAction('扩写')">扩写</button>
                            </div>
                        </div>
                    </div>
                </div>`;
        ...
        return this.messages.map(m => `
            <div class="flex gap-3 ${m.role === 'user' ? 'flex-row-reverse' : ''}">
                <div class="w-8 h-8 rounded-full flex shrink-0 center text-sm ${m.role === 'user' ? 'bg-gradient-to-br from-green-500 to-emerald-600 text-gray-800' : 'bg-gradient-to-br from-blue-500/20 to-purple-500/20 border border-gray-300 text-blue-400'}">
                ...
                <div class="flex-1 ${m.role === 'user' ? 'text-right' : ''}">
                    <div class="inline-block max-w-[85%] text-left">
                        ...
                        <div class="p-3 rounded-lg ${m.role === 'user' ? 'bg-gradient-to-br from-green-600/30 to-emerald-600/30 border border-green-500/20' : 'bg-gray-100 border border-gray-300'} text-sm text-gray-700 leading-relaxed markdown-body">
                            ${typeof marked !== 'undefined' ? marked.parse(m.content) : m.content}
                        </div>
                        <div class="flex gap-2 mt-1 ${m.role === 'user' ? 'flex-row-reverse' : ''}">
                            <button class="text-[8px] text-dim hover:text-gray-800" onclick="Utils.copy('${m.content.replace(/'/g, "\\'").replace(/\n/g, '\\n')}')">
        ...
        return `
            <div class="flex flex-col h-full">
                ...
                <div class="flex border-b border-gray-200">
                    ${tabs.map(t => `
                        <button class="flex-1 py-2 text-[10px] font-bold transition-all ${this.currentTab === t.id ? 'text-gray-800 border-b-2 border-accent' : 'text-dim hover:text-gray-800'}" onclick="Modules.web_chat.switchResourceTab('${t.id}')">
    ...
    _renderProjectsTab() {
        return `
            <div class="space-y-3">
                ...
                ` : this.projects.map(p => `
                    <div class="p-3 rounded-lg mb-2 cursor-pointer transition-all ${this.currentProjectId === p.id ? 'bg-gray-200 border border-gray-300' : 'hover:bg-gray-100 border border-transparent'}" onclick="Modules.web_chat.selectProject('${p.id}')">
                        <div class="flex items-center justify-between mb-1">
        ...
        return `
            <div class="space-y-3">
                <div class="flex gap-2">
                    ...
                    <button class="btn h-8 bg-gray-100 text-dim text-xs" onclick="Modules.web_chat.editProjectOutline()">
                ...
                ` : (project.volumes || []).map((v, vi) => `
                    <div class="mb-3">
                        <div class="flex items-center justify-between mb-1 p-2 rounded bg-gray-100">
                            <span class="text-xs font-bold text-gray-800">
                                <i class="fa-solid fa-book-bookmark mr-1 text-blue-400"></i>${v.name || `第${vi + 1}卷`}
                            </span>
                            <div class="flex gap-1">
                                <button class="text-[10px] text-dim hover:text-gray-800" onclick="Modules.web_chat.addChapter(${vi})">
                                    <i class="fa-solid fa-plus"></i>
                                </button>
                                <button class="text-[10px] text-dim hover:text-gray-800" onclick="Modules.web_chat.editVolume(${vi})">
                                    <i class="fa-solid fa-edit"></i>
                                </button>
                        ...
                        <div class="pl-4 space-y-1">
                            ...
                            ` : (v.chapters || []).map((c, ci) => `
                                <div class="p-2 rounded bg-black/2 text-[10px] flex items-center justify-between">
                                    <span class="text-dim truncate">
                                        <i class="fa-solid fa-file-lines mr-1 text-green-400"></i>${c.title || `第${ci + 1}章`}
                                    </span>
                                    <div class="flex gap-1 shrink-0">
                                        <button class="text-dim hover:text-gray-800" onclick="Modules.web_chat.editChapter(${vi}, ${ci})">
                                            <i class="fa-solid fa-edit"></i>
                                        </button>
                                    </div>
                                </div>
    ...
    _renderChaptersTab() {
        ...
        return `
            <div class="space-y-3">
                ...
                ` : allChapters.map(c => `
                    <div class="p-3 rounded-lg mb-2 bg-gray-100 border border-gray-200 cursor-pointer hover:bg-gray-200" onclick="Modules.web_chat.editChapter(${c.volumeIndex}, ${c.chapterIndex})">
                        <div class="flex items-center justify-between mb-1">
                            <span class="text-xs font-bold text-gray-800 truncate">
                                ${c.title || '未命名章节'}
                            </span>
            ...
            <div class="space-y-3">
                ...
                <div class="p-2 bg-gray-100 rounded border border-gray-200">
                    ...
                    <div class="grid grid-cols-3 gap-2 text-center">
                        <div>
                            <div class="text-sm font-bold text-gray-800">${allChapters.length}</div>
                ...
                ` : allChapters.map(c => `
                    <div class="p-3 rounded-lg mb-2 bg-gray-100 border border-gray-200 cursor-pointer hover:bg-gray-200" onclick="Modules.web_chat.editChapterContent(${c.volumeIndex}, ${c.chapterIndex})">
                        <div class="flex items-center justify-between mb-1">
                            <span class="text-xs font-bold text-gray-800 truncate">
    ...
    _renderEditorModal() {
        ...
        return `
        <div class="fixed inset-0 bg-gray-300 flex items-center justify-center z-50" id="wc-editor-modal" onclick="if(event.target.id==='wc-editor-modal')Modules.web_chat.closeEditorModal()">
            <div class="bg-white rounded-xl border border-gray-300 w-full max-w-4xl max-h-[90vh] flex flex-col m-4">
                <div class="flex items-center justify-between p-4 border-b border-gray-300">
                    ...
                    <button class="btn btn-xs bg-gray-100 text-dim" onclick="Modules.web_chat.closeEditorModal()">
                    ...
                    ${type === 'outline' ? `
                        <div class="space-y-3">
                            <div>
                                ...
                                <input type="text" id="wc-edit-title" value="${chapter.title}" class="w-full bg-gray-100 border border-gray-300 rounded-lg p-2 text-sm text-gray-700 focus:border-accent/30">
                            </div>
                            <div>
                                <label class="text-xs text-dim mb-1 block">章节细纲</label>
                                <textarea id="wc-edit-outline" class="w-full bg-gray-100 border border-gray-300 rounded-lg p-3 text-sm text-gray-700 resize-none h-64 focus:border-accent/30">${chapter.outline || ''}</textarea>
                            </div>
                        </div>
                    ` : `
                        <div class="space-y-3">
                            <div>
                                ...
                                <textarea id="wc-edit-content" class="w-full bg-gray-100 border border-gray-300 rounded-lg p-3 text-sm text-gray-700 resize-none h-96 focus:border-accent/30">${chapter.content || ''}</textarea>
                            </div>
                        </div>
                    `}
                </div>
                <div class="flex items-center justify-end gap-2 p-4 border-t border-gray-300">
                    <button class="btn h-9 bg-gray-100 text-dim" onclick="Modules.web_chat.closeEditorModal()">取消</button>
...
请用清晰的格式输出诊断报告。`;
    ...
    openSoloPromptModal() {
        ...
        const promptList = Object.entries(savedPrompts).map(([k, v]) => `
            <div class="flex items-center gap-2 p-2 rounded bg-gray-100 border border-gray-200">
                <span class="flex-1 text-xs text-gray-800 truncate">${k}</span>
        ...
        modal.innerHTML = `
            <div class="bg-white rounded-2xl border border-gray-300 w-[600px] max-h-[80vh] flex flex-col shadow-2xl">
                <div class="flex items-center justify-between px-5 py-3 border-b border-gray-200">
                    ...
                    <button class="text-dim hover:text-gray-800" onclick="this.closest('#wc-solo-prompt-modal').remove()"><i class="fa-solid fa-xmark"></i></button>
                </div>
                <div class="flex-1 overflow-y-auto p-4 space-y-4">
                    <div>
                        <div class="text-[10px] text-dim font-bold uppercase mb-2">提示词名称</div>
                        <input id="wc-solo-name" class="w-full bg-gray-100 border border-gray-300 rounded-lg p-2 text-sm text-gray-800" placeholder="例如：角色分析、情节推演...">
                    </div>
                    <div>
                        <div class="text-[10px] text-dim font-bold uppercase mb-2">提示词内容</div>
                        <textarea id="wc-solo-content" class="w-full bg-gray-100 border border-gray-300 rounded-lg p-3 text-sm text-gray-600 resize-none h-40" placeholder="输入自定义提示词，使用 {{content}} 作为内容占位符...
