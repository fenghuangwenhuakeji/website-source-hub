...
Modules.tools_center = {
    ...
    render() {
        ...
        return `
        <div class="flex h-full bg-white text-gray-700 overflow-hidden">
            <!-- 左侧导航 (可折叠) -->
            <div class="${collapsed ? 'w-10' : 'w-64'} shrink-0 bg-white border-r border-gray-200 flex flex-col z-20 transition-all duration-300" id="tc-sidebar">
    ...
    _renderCollapsedSidebar() {
        return `
            <button class="w-10 h-10 flex center text-dim hover:text-gray-800 hover:bg-gray-200 border-b border-gray-200" onclick="Modules.tools_center.toggleSidebar()" title="展开面板"><i class="fa-solid fa-angles-right text-xs"></i></button>
            <button class="w-10 h-10 flex center ${this.currentTab==='workflow' ? 'text-indigo-400 bg-indigo-500/10' : 'text-dim hover:text-gray-800 hover:bg-gray-200'}" onclick="Modules.tools_center.switchTab('workflow')" title="工作流"><i class="fa-solid fa-diagram-project text-xs"></i></button>
            <button class="w-10 h-10 flex center ${this.currentTab==='agents' ? 'text-blue-400 bg-blue-500/10' : 'text-dim hover:text-gray-800 hover:bg-gray-200'}" onclick="Modules.tools_center.switchTab('agents')" title="智能体"><i class="fa-solid fa-robot text-xs"></i></button>
            ...
            <button class="w-10 h-10 flex center text-dim hover:text-gray-800 hover:bg-gray-200" onclick="Modules.tools_center.saveWorkflow()" title="保存"><i class="fa-solid fa-floppy-disk text-xs"></i></button>
            <button class="w-10 h-10 flex center text-dim hover:text-gray-800 hover:bg-gray-200" onclick="Modules.tools_center.importWorkflowJSON()" title="导入"><i class="fa-solid fa-upload text-xs"></i></button>`;
    },

    _renderExpandedSidebar() {
        const TC = this;
        return `
            <div class="p-3 border-b border-gray-200 flex items-center gap-2 bg-gradient-to-r from-indigo-900/20 to-transparent">
                ...
                <div class="flex-1 min-w-0">
                    <span class="font-bold text-base text-gray-800">工具中心</span>
                    <div class="text-xs text-gray-600 font-bold">工作流 · 智能体</div>
                </div>
                <button class="w-6 h-6 flex center text-dim hover:text-gray-800 hover:bg-gray-200 rounded" onclick="Modules.tools_center.toggleSidebar()"><i class="fa-solid fa-angles-left text-xs"></i></button>
            </div>
            <div class="flex border-b border-gray-200">
                ${[['workflow','工作流','fa-diagram-project'],['agents','智能体','fa-robot']].map(([k,v,i]) => `
                    <button class="flex-1 py-3 text-xs font-bold transition-all ${TC.currentTab===k ? 'text-indigo-600 border-b-2 border-indigo-400' : 'text-gray-600 hover:text-gray-800'}" onclick="Modules.tools_center.switchTab('${k}')">
                        <i class="fa-solid ${i} mr-1"></i>
                        <span>${v}</span>
                    </button>
                `).join('')}
            </div>
    ...
    _renderBottomActions() {
        if (this.currentTab === 'workflow') return `
            <button class="btn w-full py-2.5 bg-gradient-to-r from-indigo-500 to-purple-500 hover:from-indigo-400 hover:to-purple-400 text-white border-none shadow-lg shadow-indigo-500/30 hover:shadow-indigo-500/50 transition-all hover:scale-105 rounded-lg" onclick="Modules.tools_center.runWorkflow()">
                    <i class="fa-solid fa-play mr-2"></i>
                    <span class="text-sm font-bold">运行工作流</span>
                </button>
            <div class="flex gap-1">
                <button class="btn flex-1 py-2 bg-gradient-to-r from-gray-500 to-gray-600 hover:from-gray-400 hover:to-gray-500 text-white border-none shadow-lg shadow-gray-500/30 hover:shadow-gray-500/50 transition-all hover:scale-105 rounded-lg" onclick="Modules.tools_center.saveWorkflow()">
                    <i class="fa-solid fa-floppy-disk mr-1.5"></i>
                    <span class="text-xs font-bold">保存</span>
                </button>
                <button class="btn flex-1 py-2 bg-gradient-to-r from-blue-500 to-cyan-500 hover:from-blue-400 hover:to-cyan-400 text-white border-none shadow-lg shadow-blue-500/30 hover:shadow-blue-500/50 transition-all hover:scale-105 rounded-lg" onclick="Modules.tools_center.importWorkflowJSON()">
                    <i class="fa-solid fa-upload mr-1.5"></i>
                    <span class="text-xs font-bold">导入</span>
                </button>
                <button class="btn flex-1 py-2 bg-gradient-to-r from-green-500 to-emerald-500 hover:from-green-400 hover:to-emerald-400 text-white border-none shadow-lg shadow-green-500/30 hover:shadow-green-500/50 transition-all hover:scale-105 rounded-lg" onclick="Modules.tools_center.exportWorkflowJSON()">
                    <i class="fa-solid fa-download mr-1.5"></i>
                    <span class="text-xs font-bold">导出</span>
                </button>
            </div>
            <div class="flex gap-1">
                <button class="btn flex-1 py-2 bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-400 hover:to-pink-400 text-white border-none shadow-lg shadow-purple-500/30 hover:shadow-purple-500/50 transition-all hover:scale-105 rounded-lg" onclick="Modules.tools_center.toggleBatchMode()">
                    <i class="fa-solid fa-layer-group mr-1.5"></i>
                    <span class="text-xs font-bold">批量</span>
                </button>
                <button class="btn flex-1 py-2 bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-400 hover:to-orange-400 text-white border-none shadow-lg shadow-amber-500/30 hover:shadow-amber-500/50 transition-all hover:scale-105 rounded-lg" onclick="document.getElementById('tc-global-io')?.classList.toggle('hidden')">
                    <i class="fa-solid fa-terminal mr-1.5"></i>
                    <span class="text-xs font-bold">IO</span>
                </button>
            </div>`;
        if (this.currentTab === 'agents') return `
            <button class="btn w-full py-2.5 bg-gradient-to-r from-blue-500 to-cyan-500 hover:from-blue-400 hover:to-cyan-400 text-white border-none shadow-lg shadow-blue-500/30 hover:shadow-blue-500/50 transition-all hover:scale-105 rounded-lg" onclick="Modules.tools_center.createAgent()">
                    <i class="fa-solid fa-plus mr-2"></i>
                    <span class="text-sm font-bold">部署智能体</span>
                </button>
            ...
            return `
                <div class="p-3 space-y-3">
                    ...
                    <div class="grid grid-cols-2 gap-1.5">
                        ${[['input','输入源','fa-keyboard','green'],['output','输出口','fa-print','red'],['merger','融合器','fa-code-merge','orange'],['splitter','分流器','fa-code-branch','teal'],['variable','变量','fa-database','slate'],['const','常量','fa-lock','gray']].map(([t,n,i,c]) => `
                            <div class="p-2.5 bg-white rounded-lg border-2 border-gray-200 cursor-grab hover:bg-${c}-50 hover:border-${c}-400 flex items-center gap-2 text-xs transition-all shadow-sm hover:shadow-md" draggable="true" ondragstart="Modules.tools_center.dragStart(event,'${t}')">
                                <i class="fa-solid ${i} text-${c}-400 text-sm"></i>
                    ...
                    <div class="grid grid-cols-2 gap-1.5">
                        ${[['llm','LLM生成','fa-brain','indigo'],['polish','润色','fa-wand-magic-sparkles','pink'],['translate','翻译','fa-language','blue'],['summary','摘要','fa-compress','yellow'],['expand','扩写','fa-expand','green'],['rewrite','改写','fa-arrows-rotate','purple'],['judge','评审','fa-gavel','amber'],['extract','提取','fa-filter','cyan'],['style','风格化','fa-palette','rose'],['tone','语气调整','fa-comment','violet']].map(([t,n,i,c]) => `
                            <div class="p-2.5 bg-white rounded-lg border-2 border-gray-200 cursor-grab hover:bg-${c}-50 hover:border-${c}-400 flex items-center gap-2 text-xs transition-all shadow-sm hover:shadow-md" draggable="true" ondragstart="Modules.tools_center.dragStart(event,'${t}')">
                                <i class="fa-solid ${i} text-${c}-400 text-sm"></i>
                                <span class="font-bold text-gray-700">${n}</span>
                            </div>
                    ...
                    <div class="grid grid-cols-2 gap-1.5">
                        ${[['api','API调用','fa-cloud','blue'],['code','代码执行','fa-code','green'],['template','模板','fa-file-lines','purple'],['counter','计数器','fa-calculator','yellow'],['random','随机','fa-dice','pink'],['validate','验证','fa-check-circle','teal']].map(([t,n,i,c]) => `
                            <div class="p-2.5 bg-white rounded-lg border-2 border-gray-200 cursor-grab hover:bg-${c}-50 hover:border-${c}-400 flex items-center gap-2 text-xs transition-all shadow-sm hover:shadow-md" draggable="true" ondragstart="Modules.tools_center.dragStart(event,'${t}')">
                                <i class="fa-solid ${i} text-${c}-400 text-sm"></i>
                                <span class="font-bold text-gray-700">${n}</span>
                            </div>
        ...
        if (TC.currentTab === 'agents') {
            return `
                <div class="p-3 flex flex-col gap-3 flex-1">
                    ...
                    <input id="tc-agent-name" class="epic-input h-8 px-3 rounded text-xs text-gray-800" placeholder="名称 (如: 文本润色专家)">
                    <div class="flex gap-2">
                        <input id="tc-agent-icon" class="epic-input h-8 px-3 rounded text-xs text-gray-800 flex-1" placeholder="图标 (fa-solid fa-magic)" oninput="document.getElementById('tc-icon-preview').className=this.value">
                        <div class="w-8 h-8 center bg-blue-500/10 border border-blue-500/30 rounded text-blue-400 flex center"><i id="tc-icon-preview" class="fa-solid fa-cube"></i></div>
                    </div>
                    <textarea id="tc-agent-prompt" class="epic-input flex-1 rounded-lg p-3 text-sm font-mono text-gray-700 resize-none min-h-[100px] border-2 border-gray-300 focus:border-blue-400 focus:outline-none focus:ring-2 focus:ring-blue-200" placeholder="系统提示词..."></textarea>
                    <div class="flex gap-1">
                        <select id="tc-agent-workflow" class="epic-input h-7 px-2 rounded text-[10px] text-gray-800 flex-1"><option value="">绑定工作流(可选)</option></select>
                        <button class="btn btn-xs bg-gray-100 text-dim" onclick="Modules.tools_center._loadAgentWorkflowOptions()"><i class="fa-solid fa-sync"></i></button>
                    </div>
                    <div class="p-2 bg-gray-100 rounded border border-gray-200 text-[9px] text-dim leading-relaxed">
                        <strong class="text-blue-400 block mb-1">💡 网页对话调用</strong>
                        在网页对话中输入 <code class="text-green-400">/agent 名称 消息</code> 即可调用<br>
                        输入 <code class="text-green-400">/workflow 名称</code> 调用工作流<br>
                        输入 <code class="text-green-400">/rag 关键词</code> RAG检索
                    </div>
                    <div class="text-xs font-bold text-gray-700 flex items-center gap-2 mt-2">
    ...
    _renderWorkflowCanvas() {
        ...
        <div class="flex flex-col h-full">
            <!-- 顶部工具栏 -->
            <div class="h-11 bg-white border-b border-gray-200 flex items-center px-4 gap-3 shrink-0">
                ...
                <div class="flex items-center gap-1.5">
                    <button class="btn h-7 px-3 bg-gray-100 hover:bg-gray-200 text-dim text-[10px] rounded-lg border border-gray-200 hover:border-gray-300 transition-all" onclick="Modules.tools_center.resetPan()"><i class="fa-solid fa-crosshairs mr-1 text-blue-400/60"></i>重置视图</button>
                    <button class="btn h-7 px-3 bg-gray-100 hover:bg-gray-200 text-dim text-[10px] rounded-lg border border-gray-200 hover:border-gray-300 transition-all" onclick="Modules.tools_center.autoLayout()"><i class="fa-solid fa-wand-magic mr-1 text-amber-400"></i>自动排列</button>
                    <button class="btn h-7 px-3 bg-gray-100 hover:bg-gray-200 text-dim text-[10px] rounded-lg border border-gray-200 hover:border-gray-300 transition-all" onclick="Modules.tools_center.clearCanvas()"><i class="fa-solid fa-trash mr-1 text-red-400/60"></i>清空</button>
                    <div class="h-5 w-px bg-gray-200"></div>
                    ...
                    <div class="h-5 w-px bg-gray-200"></div>
                    <select class="bg-white border border-gray-300 rounded-lg px-3 py-1 text-[10px] text-gray-800 hover:border-indigo-500/30 transition-all cursor-pointer" onchange="Modules.tools_center.addPresetWorkflow(this.value);this.value=''">
            ...
            <div class="flex-1 relative overflow-hidden" id="tc-canvas"
                 ...
                 style="background:#F8F9FA;background-image:radial-gradient(circle,rgba(255,255,255,0.03) 1px,transparent 1px);background-size:24px 24px;cursor:grab">
                ...
                <div class="absolute inset-0 flex flex-col items-center justify-center z-5 pointer-events-none">
                    <i class="fa-solid fa-diagram-project text-5xl text-gray-800/5 mb-4"></i>
                    <div class="text-sm text-gray-800/10 font-bold mb-1">拖拽节点到画布开始构建工作流</div>
                    <div class="text-[10px] text-gray-800/5">或从右上角加载预设工作流</div>
                </div>` : ''}
                <!-- 节点设置面板 -->
                <div id="tc-node-settings" class="hidden absolute top-3 right-3 w-80 bg-white/95 backdrop-blur-xl border border-gray-300 rounded-xl shadow-2xl z-30 max-h-[85%] overflow-y-auto"></div>
            </div>
            <!-- IO面板 -->
            <div id="tc-global-io" class="hidden border-t border-gray-200 bg-[#F1F3F5] flex flex-col shrink-0" style="height:45vh;min-height:280px">
                <div class="flex items-center px-3 py-1.5 border-b border-gray-200 bg-gray-100">
                    <span class="text-[10px] font-bold text-dim"><i class="fa-solid fa-terminal mr-1 text-green-400/50"></i>IO 调试</span>
                    <div class="flex-1"></div>
                    <button class="btn btn-xs text-dim hover:text-gray-800" onclick="document.getElementById('tc-global-io').classList.add('hidden')"><i class="fa-solid fa-xmark"></i></button>
                </div>
                <div class="flex-1 grid grid-cols-2 gap-2 p-2 min-h-0">
                    <textarea id="tc-io-in" class="bg-gray-100 border border-gray-200 rounded-lg p-2 text-[9px] text-gray-400 font-mono resize-none" readonly placeholder="输入"></textarea>
                    <textarea id="tc-io-out" class="bg-gray-100 border border-gray-200 rounded-lg p-2 text-[9px] text-green-400 font-mono resize-none" readonly placeholder="输出"></textarea>
                </div>
            </div>
        </div>`;
    },
        ...
        <div class="flex flex-col h-full">
            <div class="flex-1 flex">
                <div class="flex-1 flex flex-col border-r border-gray-200">
                    <div class="h-10 bg-white border-b border-gray-200 flex items-center px-3 gap-2 shrink-0">
                    ...
                    <div class="p-3 border-t border-gray-200 flex gap-2 shrink-0">
                        <input id="tc-agent-chat-input" class="epic-input flex-1 h-9 px-3 rounded-lg text-sm text-gray-800" placeholder="输入消息..." onkeydown="if(event.key==='Enter')Modules.tools_center.sendAgentChat()">
                        <button class="btn bg-blue-600 hover:bg-blue-500 text-gray-800 px-4 rounded-lg text-xs font-bold" onclick="Modules.tools_center.sendAgentChat()"><i class="fa-solid fa-paper-plane"></i></button>
                    </div>
                </div>
                <div class="w-72 shrink-0 bg-white p-3 overflow-y-auto" id="tc-agent-detail">
    ...
    _renderChatMsg(m) {
        ...
        <div class="flex ${isUser ? 'justify-end' : 'justify-start'}">
            <div class="max-w-[80%] rounded-xl px-4 py-2.5 text-sm leading-relaxed ${isUser ? 'bg-violet-600/20 text-violet-100 border border-violet-500/20' : 'bg-gray-100 text-gray-700 border border-gray-200'}">
                ${!isUser && m.agent ? `<div class="text-[9px] text-blue-400 font-bold mb-1"><i class="fa-solid fa-robot mr-1"></i>${m.agent}</div>` : ''}
    ...
    _nodeTypes: {
        ...
        merger:      { name:'融合器', icon:'fa-code-merge', color:'orange', ports:{ in:2, out:1 } },
        splitter:    { name:'分流器', icon:'fa-code-branch', color:'teal', ports:{ in:1, out:2 } },
        variable:    { name:'变量', icon:'fa-database', color:'slate', ports:{ out:1 }, params:{ value:'' } },
        const:       { name:'常量', icon:'fa-lock', color:'gray', ports:{ out:1 }, params:{ value:'' } },
        ...
        judge:       { name:'评审', icon:'fa-gavel', color:'amber', ports:{ in:1, out:1 } },
        extract:     { name:'提取', icon:'fa-filter', color:'cyan', ports:{ in:1, out:1 } },
        style:       { name:'风格化', icon:'fa-palette', color:'rose', ports:{ in:1, out:1 } },
        tone:        { name:'语气调整', icon:'fa-comment', color:'violet', ports:{ in:1, out:1 } },
        ...
        delay:       { name:'延迟', icon:'fa-clock', color:'amber', ports:{ in:1, out:1 }, params:{ seconds:1 } },
        ...
        api:         { name:'API调用', icon:'fa-cloud', color:'blue', ports:{ in:1, out:1 }, params:{ url:'', method:'GET' } },
        code:        { name:'代码执行', icon:'fa-code', color:'green', ports:{ in:1, out:1 }, params:{ code:'' } },
        template:    { name:'模板', icon:'fa-file-lines', color:'purple', ports:{ in:1, out:1 }, params:{ template:'' } },
        counter:     { name:'计数器', icon:'fa-calculator', color:'yellow', ports:{ in:1, out:1 }, params:{ start:0, step:1 } },
        random:      { name:'随机', icon:'fa-dice', color:'pink', ports:{ in:1, out:1 }, params:{ min:0, max:100 } },
        validate:    { name:'验证', icon:'fa-check-circle', color:'teal', ports:{ in:1, out:2 }, outLabels:['通过','失败'] }
    },
    ...
    renderNode(node) {
        ...
        if (node.type === 'agent_node') bodyHtml += `<div class="flex items-center gap-1 mt-0.5"><span class="text-[9px] ${node.data.agentId ? 'text-sky-400/70 bg-sky-500/10' : 'text-dim bg-gray-100'} px-1.5 py-0.5 rounded">${node.data.agentId ? '🤖 已绑定智能体' : '⚠ 未选择智能体'}</span></div>`;
        if (node.type === 'subworkflow') bodyHtml += `<div class="flex items-center gap-1 mt-0.5"><span class="text-[9px] ${node.data.workflowId ? 'text-fuchsia-400/70 bg-fuchsia-500/10' : 'text-dim bg-gray-100'} px-1.5 py-0.5 rounded">${node.data.workflowId ? '📋 已绑定工作流' : '⚠ 未选择工作流'}</span></div>`;
        ...
        if (['llm','polish','translate','summary','expand','rewrite','judge','extract'].includes(node.type) && node.data.model) {
            bodyHtml += `<div class="text-[8px] text-dim mt-0.5 bg-gray-100 px-1.5 py-0.5 rounded inline-block">模型: ${node.data.model}</div>`;
        }
        if (!bodyHtml && !['input','output','merger','splitter'].includes(node.type)) {
        ...
        <div class="tc-node absolute select-none cursor-move group" style="left:${node.x}px;top:${node.y}px;min-width:200px;max-width:260px;pointer-events:auto" id="node-${node.id}" onmousedown="Modules.tools_center.nodeMouseDown(event,'${node.id}')" ondblclick="Modules.tools_center._openNodeSettings('${node.id}')">
            <div class="bg-white border-2 ${selected ? 'border-gray-400 shadow-xl' : 'border-'+c+'-500/25 hover:border-'+c+'-400/50'} rounded-xl shadow-lg transition-all">
                <!-- 节点头部 -->
                <div class="flex items-center gap-2 px-3 py-2 border-b border-gray-200 bg-gradient-to-r from-${c}-500/10 to-transparent rounded-t-xl">
                    ...
                    <span class="text-[11px] font-bold text-gray-800 flex-1 truncate">${node.data.label || def.name}</span>
                    <button class="opacity-0 group-hover:opacity-100 w-5 h-5 rounded flex center text-dim hover:text-gray-800 hover:bg-gray-200 text-[9px] transition-all" onclick="event.stopPropagation();Modules.tools_center._openNodeSettings('${node.id}')" title="设置"><i class="fa-solid fa-gear"></i></button>
    ...
    _openNodeSettings(id) {
        ...
        fieldsHtml += `
            <div class="space-y-1">
                ...
                <input class="epic-input w-full h-7 px-2 rounded text-xs text-gray-800" value="${node.data.label||''}" onchange="Modules.tools_center.updateNodeData('${id}','label',this.value)">
            </div>`;
        // AI处理节点: prompt + model + temperature
        if (['llm','polish','translate','summary','expand','rewrite','judge','extract'].includes(node.type)) {
            fieldsHtml += `
            <div class="space-y-1">
                <label class="text-[9px] text-dim font-bold">自定义提示词</label>
                <textarea class="epic-input w-full rounded p-2 text-[10px] text-gray-800 resize-none h-24 font-mono" onchange="Modules.tools_center.updateNodeData('${id}','prompt',this.value)">${node.data.prompt||''}</textarea>
            </div>
            <div class="grid grid-cols-2 gap-2">
                <div class="space-y-1">
                    <label class="text-[9px] text-dim font-bold">模型(可选)</label>
                    <input class="epic-input w-full h-7 px-2 rounded text-[10px] text-gray-800" value="${node.data.model||''}" placeholder="默认" onchange="Modules.tools_center.updateNodeData('${id}','model',this.value)">
                </div>
                <div class="space-y-1">
                    <label class="text-[9px] text-dim font-bold">温度(0-2)</label>
                    <input type="number" step="0.1" min="0" max="2" class="epic-input w-full h-7 px-2 rounded text-[10px] text-gray-800" value="${node.data.temperature||''}" placeholder="默认" onchange="Modules.tools_center.updateNodeData('${id}','temperature',this.value)">
                </div>
            </div>`;
        }
        // 条件分支
        if (node.type === 'condition') {
            ...
            <div class="space-y-1">
                ...
                <textarea class="epic-input w-full rounded p-2 text-[10px] text-gray-800 resize-none h-16 font-mono" placeholder="例: 文本质量是否达到发表标准" onchange="Modules.tools_center.updateNodeData('${id}','prompt',this.value)">${node.data.prompt||''}</textarea>
        ...
        if (node.type === 'loop') {
            fieldsHtml += `
            <div class="space-y-1">
                <label class="text-[9px] text-dim font-bold">循环次数</label>
                <input type="number" min="1" max="20" class="epic-input w-full h-7 px-2 rounded text-[10px] text-gray-800" value="${node.data.count||3}" onchange="Modules.tools_center.updateNodeData('${id}','count',this.value)">
            </div>
            <div class="space-y-1">
                <label class="text-[9px] text-dim font-bold">迭代提示词(可选)</label>
                <textarea class="epic-input w-full rounded p-2 text-[10px] text-gray-800 resize-none h-16 font-mono" placeholder="每次迭代的优化指令" onchange="Modules.tools_center.updateNodeData('${id}','prompt',this.value)">${node.data.prompt||''}</textarea>
            </div>`;
        }
        // 智能体节点
        if (node.type === 'agent_node') {
            ...
            <div class="space-y-1">
                ...
                <select class="epic-input w-full h-7 px-2 rounded text-[10px] text-gray-800 tc-settings-agent-sel" onchange="Modules.tools_center.updateNodeData('${id}','agentId',this.value)"><option value="">选择...</option></select>
            </div>`;
        }
        // 子工作流
        if (node.type === 'subworkflow') {
            ...
            <div class="space-y-1">
                ...
                <select class="epic-input w-full h-7 px-2 rounded text-[10px] text-gray-800 tc-settings-wf-sel" onchange="Modules.tools_center.updateNodeData('${id}','workflowId',this.value)"><option value="">选择...</option></select>
            </div>`;
        }
        // RAG节点
        if (node.type === 'rag_node') {
            ...
            <div class="space-y-1">
                ...
                <input class="epic-input w-full h-7 px-2 rounded text-[10px] text-gray-800" value="${node.data.query||''}" placeholder="留空=自动从输入提取" onchange="Modules.tools_center.updateNodeData('${id}','query',this.value)">
            </div>
            <div class="space-y-1">
                <label class="text-[9px] text-dim font-bold">检索数量</label>
                <input type="number" min="1" max="30" class="epic-input w-full h-7 px-2 rounded text-[10px] text-gray-800" value="${node.data.limit||8}" onchange="Modules.tools_center.updateNodeData('${id}','limit',this.value)">
            </div>`;
        }
        // 输入源
        if (node.type === 'input') {
            ...
            <div class="space-y-1">
                ...
                <textarea class="epic-input w-full rounded p-2 text-[10px] text-gray-800 resize-none h-24 font-mono" placeholder="留空=运行时弹窗输入" onchange="Modules.tools_center.updateNodeData('${id}','prompt',this.value)">${node.data.prompt||''}</textarea>
            </div>`;
        }

        panel.innerHTML = `
            <div class="p-3 border-b border-gray-200 flex items-center gap-2 bg-${c}-500/5">
                <i class="fa-solid ${def.icon} text-${c}-400"></i>
                <span class="text-xs font-bold text-gray-800 flex-1">${def.name} 设置</span>
                <button class="text-dim hover:text-gray-800" onclick="Modules.tools_center._closeNodeSettings()"><i class="fa-solid fa-xmark"></i></button>
            </div>
    ...
    async _executeNode(node, input) {
        ...
        const defaultPrompts = {
            ...
            judge: '请从逻辑性、文笔、创意三个维度评审以下内容，给出详细评价和改进建议：',
        ...
        if (type === 'merger' || type === 'splitter' || type === 'output') return input;
    ...
    async loadSavedWorkflows() {
        ...
        el.innerHTML = wfs.length === 0 ? '<div class="text-[9px] text-dim px-1">暂无保存的工作流</div>' :
            wfs.map(w => `
                <div class="flex items-center gap-1 px-2 py-1 rounded text-[10px] bg-gray-100 hover:bg-indigo-500/10 cursor-pointer group transition-all" onclick="Modules.tools_center.loadWorkflow('${w.id}')">
                    <i class="fa-solid fa-diagram-project text-indigo-400/50"></i>
                    <span class="flex-1 truncate text-dim group-hover:text-gray-800">${w.name}</span>
    ...
    addPresetWorkflow(preset) {
        ...
        const presets = {
            ...
            polish_chain: () => {
                ...
                const c = _n('judge', 620, 120, { label:'质量评审', prompt:'请评审润色后的文本质量，给出评分和改进建议：' });
                const d = _n('output', 900, 120, { label:'最终结果' });
            ...
            // 4. 循环精炼
            loop_refine: () => {
                const a = _n('input', 60, 120, { label:'初稿' });
                const b = _n('loop', 340, 120, { label:'3次精炼', count:3, prompt:'请在保持核心内容的基础上进一步优化文笔和表达' });
                const c = _n('polish', 620, 120, { label:'最终润色' });
                const d = _n('output', 900, 120, { label:'精炼结果' });
                _c(a,b); _c(b,c); _c(c,d);
            },
            // 5. 双语翻译对比
            translate_compare: () => {
                const a = _n('input', 60, 180, { label:'原文' });
                const b = _n('translate', 360, 60, { label:'翻译为英文', prompt:'请将以下中文翻译为地道的英文：' });
                const c = _n('translate', 360, 300, { label:'翻译为日文', prompt:'请将以下中文翻译为地道的日文：' });
                const d = _n('merger', 660, 180, { label:'合并结果' });
                const e = _n('output', 940, 180, { label:'多语言对比' });
                _c(a,b); _c(a,c); _c(b,d); _c(c,d,'out_0','in_1'); _c(d,e);
            },
            // 6. 智能体对话流
            agent_chat_flow: () => {
                const a = _n('input', 60, 140, { label:'用户问题' });
                const b = _n('agent_node', 360, 60, { label:'智能体处理' });
                const c = _n('chat_node', 360, 260, { label:'人工审核' });
                const d = _n('merger', 660, 140, { label:'合并' });
                const e = _n('output', 940, 140, { label:'最终回复' });
                _c(a,b); _c(a,c); _c(b,d); _c(c,d,'out_0','in_1'); _c(d,e);
            },
            // 7. 分流融合
            split_merge: () => {
                const a = _n('input', 60, 180, { label:'长文本' });
                const b = _n('splitter', 320, 180, { label:'分流' });
                const c = _n('summary', 580, 60, { label:'摘要分支' });
                const d = _n('extract', 580, 300, { label:'提取分支' });
                const e = _n('merger', 840, 180, { label:'融合' });
                const f = _n('output', 1100, 180, { label:'综合结果' });
                _c(a,b); _c(b,c); _c(b,d,'out_1','in_0'); _c(c,e); _c(d,e,'out_0','in_1'); _c(e,f);
            },
            // 8. 大纲→章节
            outline_to_chapter: () => {
            ...
            // 9. 角色构建器
            character_builder: () => {
                const a = _n('input', 60, 180, { label:'角色灵感' });
                const b = _n('llm', 340, 60, { label:'性格塑造', prompt:'请根据以下灵感设计一个立体的角色性格，包含MBTI、核心动机、致命弱点、口头禅：' });
                const c = _n('llm', 340, 300, { label:'背景故事', prompt:'请为以下角色设计一段引人入胜的背景故事(500字)：' });
                const d = _n('merger', 620, 180, { label:'合并' });
                const e = _n('llm', 900, 180, { label:'角色卡生成', prompt:'请将以下角色信息整理为一张完整的角色卡，格式清晰：' });
                const f = _n('output', 1160, 180, { label:'角色卡' });
                _c(a,b); _c(a,c); _c(b,d); _c(c,d,'out_0','in_1'); _c(d,e); _c(e,f);
            },
            // 10. 完整创作流水线
            full_pipeline: () => {
    ...
    async createAgent() {
        ...
        const icon = document.getElementById('tc-agent-icon')?.value || 'fa-solid fa-robot';
        const prompt = document.getElementById('tc-agent-prompt')?.value;
        const workflowId = document.getElementById('tc-agent-workflow')?.value || '';
        if (!name || !prompt) return UI.toast('请填写名称和提示词');
        const agent = { id: 'agent_' + Date.now(), name, icon, prompt, workflowId, ts: Date.now() };
        const store = await DB.get('settings', 'tc_agents') || { id: 'tc_agents', items: [] };
        store.items.push(agent);
        await DB.put('settings', store);
        UI.toast('智能体已部署: ' + name);
        ['tc-agent-name','tc-agent-prompt'].forEach(id => { const el = document.getElementById(id); if (el) el.value = ''; });
        this.loadAgentList();
    },
    ...
    async loadAgentList() {
        ...
        el.innerHTML = agents.length === 0 ? '<div class="text-[9px] text-dim">暂无智能体</div>' :
            agents.map(a => `
                <div class="flex items-center gap-2 px-2 py-1.5 rounded bg-gray-100 hover:bg-blue-500/10 cursor-pointer group transition-all" onclick="Modules.tools_center.chatWithAgent('${a.id}')">
                    <i class="${a.icon || 'fa-solid fa-robot'} text-blue-400 text-xs"></i>
                    <div class="flex-1 min-w-0">
                        <div class="text-[10px] font-bold text-gray-800 truncate">${a.name}</div>
                        <div class="text-[9px] text-dim truncate">${a.workflowId ? '🔗工作流' : ''} ${(a.prompt||'').slice(0,30)}</div>
                    </div>
            ...
            if (detail) detail.innerHTML = `
                <div class="space-y-3">
                    <div class="text-center">
                        <div class="w-12 h-12 rounded-xl bg-blue-500/10 border border-blue-500/30 flex center text-blue-400 text-xl mx-auto mb-2"><i class="${agent.icon || 'fa-solid fa-robot'}"></i></div>
                    ...
                    <div class="text-[10px] text-dim bg-gray-100 rounded-lg p-2 border border-gray-200 max-h-40 overflow-y-auto">${agent.prompt.slice(0,500)}</div>
                </div>`;
            return;
        }
        ...
        if (detail) detail.innerHTML = `
            <div class="space-y-3">
                <div class="text-center">
                    <div class="w-12 h-12 rounded-xl bg-blue-500/10 border border-blue-500/30 flex center text-blue-400 text-xl mx-auto mb-2"><i class="${agent.icon || 'fa-solid fa-robot'}"></i></div>
                    <div class="font-bold text-white text-sm">${agent.name}</div>
                    ${agent.workflowId ? '<div class="text-[9px] text-indigo-400 mt-1"><i class="fa-solid fa-diagram-project mr-1"></i>已绑定工作流</div>' : ''}
                </div>
                <div class="text-[10px] text-dim bg-gray-100 rounded-lg p-2 border border-gray-200 max-h-40 overflow-y-auto">${agent.prompt.slice(0,500)}</div>
            </div>`;
